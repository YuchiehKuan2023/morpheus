...
class DFEncoderDataLoader(DataLoader):
    ...
    def __iter__(self):
        ...
        for data_d in super().__iter__():
            if self.batch_size == 1:
                ...
                data_d["data"] = {
                    k: v[0] if type(v) != list else [_v[0] for _v in v] for k, v in data_d["data"].items()
                }
            yield data_d
    ...
    def get_distributed_training_dataloader_from_path(
        model, data_folder, rank, world_size, load_data_fn=pd.read_csv, pin_memory=False, num_workers=0
    ):
