...
# Maximum travel_speed_kmph fed to the DFP AutoEncoder.
# Simulation events arrive seconds apart; an inter-city location change at that
# cadence produces speeds in the millions of km/h.  Feeding such values to the
# autoencoder saturates its shared hidden layers, corrupting the latent vector
# and causing ALL continuous features to produce astronomical reconstruction
# losses — the z-score explosion seen in debug-inference.log.
# Capping here preserves full anomaly-detection capability: values above 800
# still exceed the impossible-travel threshold used by detect_impossible_travel(),
# while the autoencoder no longer receives catastrophic out-of-distribution inputs.
# 950 km/h ≈ just above commercial airliner cruising speed (~900 km/h), which
# is the fastest any normal user could legitimately travel between events.
TRAVEL_SPEED_MODEL_CAP: float = 950.0  # km/h  (just above commercial aircraft cruising speed)
...
def calculate_travel_features(
    ...
    for user in df[user_col].unique():
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            ...
            raw_speed = speed.replace([np.inf, -np.inf, np.nan], 0)
            # Cap to TRAVEL_SPEED_MODEL_CAP to prevent autoencoder numerical overflow.
            # Sub-second time deltas (simulation) can produce millions-of-km/h values
            # that saturate hidden layers and corrupt reconstruction of ALL features.
            # detect_impossible_travel() threshold (800 km/h) still fires correctly
            # because 2000 > 800.
            df.loc[user_mask, "travel_speed_kmph"] = raw_speed.clip(upper=TRAVEL_SPEED_MODEL_CAP)
...
def detect_impossible_travel(
    ...
    Alert Examples:
        CRITICAL: alice@contoso.com traveled 38,244 km/h
           London → Tokyo in 15 minutes (9,561 km)

        CRITICAL: bob@contoso.com traveled 16,000 km/h
    ...
    if impossible_count > 0:
        logger.warning(
            f"CRITICAL: Detected {impossible_count} impossible travel events "
