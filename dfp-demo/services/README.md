...
The services required for NVIDIA Morpheus DFP streaming inference and AI intelligence layer are:
...
- **Qdrant** (ports 6333 REST, 6334 gRPC) - Vector database for semantic search and embeddings
- **Neo4j** (ports 7474 HTTP, 7687 Bolt) - Knowledge graph for entity relationships
- **Redis** (port 6379) - High-speed cache for AI features and session data
- **PostgreSQL** (port 5432) - AI metadata storage (enrichments, labels, feedback)
...
- ✅ Check and install prerequisites (Kafka, MLflow, tmux, AI services)
- ✅ Create necessary directories (including AI data directories)
- ✅ Configure Kafka in KRaft mode (no Zookeeper needed)
- ✅ Start AI infrastructure services (Qdrant, Neo4j, Redis, PostgreSQL)
...
### AI Infrastructure (Auto-installed)

- **Qdrant** - Vector database (`brew install qdrant`)
- **Neo4j** - Knowledge graph (`brew install neo4j`)
- **Redis** - Cache layer (`brew install redis`)
- **PostgreSQL@16** - AI metadata storage (`brew install postgresql@16`)
...
brew install neo4j redis postgresql@16

# Install Qdrant (via tap)
brew tap qdrant/qdrant
brew install qdrant

# Install Python dependencies
pip install mlflow==3.5.1 psycopg2-binary confluent-kafka qdrant-client neo4j redis
```
...
The AI Intelligence Layer uses four specialized databases that start automatically with `start_services.sh`:
...
- **Data**: `data/ai/qdrant/`
- **Log**: `data/logs/qdrant.log`
- **Features**:
  - Semantic similarity search
  - Anomaly clustering
  - Similar case retrieval
  - Embedding storage (sentence-transformers, OpenAI)
...
# Stop Qdrant standalone
./services/stop_qdrant.sh
...
- **Purpose**: Store entity relationships and attack patterns
- **Data**: `/opt/homebrew/var/neo4j/data/`
- **Features**:
  - Entity extraction (users, IPs, applications)
...
- **Port**: 6379
- **Purpose**: High-speed caching for AI features and session data
- **Data**: `data/redis/`
...
- **Port**: 5432
- **Database**: `dfp_ai`
- **User**: `dfp_ai` / `dfp-ai-2026`
- **Purpose**: Store AI enrichments, labels, and feedback
- **Service**: Managed by Homebrew (`postgresql@16`)
- **Features**:
  - Enriched anomaly storage
...
```bash
python tests/services/test_ai_services.py
```
...
services/
├── README.md                   # This file
├── start_services.sh           # Start all services (DFP + AI + monitoring)
├── stop_services.sh            # Stop all services (DFP + AI + monitoring)
├── restart_services.sh         # Restart all services (convenience wrapper)
├── check_services.sh           # Health check (includes monitoring)
├── start_qdrant.sh             # Start Qdrant standalone (optional)
├── stop_qdrant.sh              # Stop Qdrant standalone (optional)
...
data/
├── mlflow/                     # MLflow database & model artifacts
...
tests/
└── services/
    └── test_ai_services.py     # AI services validation
