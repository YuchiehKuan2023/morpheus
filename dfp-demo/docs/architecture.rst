...
The platform includes a ReAct-based agentic conversational AI system
that enables security analysts to investigate anomalies through natural
language dialogue.
...
.. code-block:: text
   ...
   ┌──────────────────▼──────────────────────────────────────────┐
   │              Entity Resolution                               │
   │  • Resolve anaphora ("her" → "alice@co.com")                │
   │  • Track entities across conversation turns                  │
   └──────────────────┬──────────────────────────────────────────┘
                      │
   ┌──────────────────▼──────────────────────────────────────────┐
   │              Query Planner                                   │
   │  • Decides if planning is needed (comparisons, complex)     │
   │  • Generates step-by-step plan with dependencies            │
   └──────────────────┬──────────────────────────────────────────┘
                      │
   ┌──────────────────▼──────────────────────────────────────────┐
   │              ReAct Loop (Agent Core)                         │
   │  ┌──────────┐  ┌────────────┐  ┌────────────┐              │
   │  │ THOUGHT  │→│  ACTION    │→│OBSERVATION │──┐            │
   │  │(Router   │  │(Tool Call) │  │(Tool Result)│  │            │
   │  │ Model)   │  └────────────┘  └────────────┘  │            │
   │  └──────────┘          ↑                        │            │
   │       ↑                └────────────────────────┘            │
   │       │                                                      │
   │  ┌────▼─────────┐                                           │
   │  │ Guard Rails  │  • Iteration budget (8)                   │
   │  │              │  • Tool-call budget (15)                   │
   │  │              │  • Token budget (12K)                      │
   │  │              │  • Blocked tools / duplicates              │
   │  └──────────────┘                                           │
   └──────────────────┬──────────────────────────────────────────┘
                      │
   ┌──────────────────▼──────────────────────────────────────────┐
   ...
   │  • Rejects insufficient answers → loops back                │
   └──────────────────┬──────────────────────────────────────────┘
                      │
   ┌──────────────────▼──────────────────────────────────────────┐
   │              Final Answer (Answer Model)                     │
   │  • Synthesises grounded response from observations          │
...
- **AgentCore**: ReAct loop orchestrator with trace capture
- **ToolRegistry**: 14 tools across PostgreSQL, Qdrant, Neo4j, and LLM
- **QueryPlanner**: Generates multi-step plans for complex queries
- **Reflector**: Evaluates answer quality and triggers retries
- **GuardRails**: Safety limits on iterations, tokens, and tool calls
- **WorkingMemory**: Scratchpad for thoughts and observations
- **EntityTracker**: Tracks entities across conversation turns
- **EpisodicMemory**: Long-term memory of past conversations (PostgreSQL)
- **HybridRetriever**: Dense + sparse + graph + structured retrieval (Week 27)
- **ContextCompressor**: Compresses retrieved context for LLM consumption
...
.. code-block:: text
   ...
   │  │ 9 tools    │  │ 1 tool   │  │ 1 tool   │            │
   │  │ (anomalies,│  │(semantic │  │(knowledge│            │
   │  │  users,    │  │ search)  │  │  graph)  │            │
   │  │  baselines)│  └──────────┘  └──────────┘            │
   │  └────────────┘                                         │
   │  ┌────────────┐  ┌──────────┐                          │
...
The agent system provides full observability:

- **Reasoning Trace**: Every thought, action, and observation is recorded
- **SSE Streaming**: Real-time trace display in the frontend
- **Agent Metrics**: Aggregate statistics (steps, latency, tool distribution)
- **Episodic Memory**: Past conversations and entity interactions
