...
class TestCalculateTravelFeatures:
    ...
    def test_single_user_two_events(self):
        ...
        # Second event: London to Tokyo in 15 minutes — raw speed ~38,244 km/h,
        # capped at TRAVEL_SPEED_MODEL_CAP to prevent autoencoder saturation.
        assert 9461 < result["distance_km"].iloc[1] < 9661  # ~9,561 km
        assert 0.24 < result["ts_delta_hour"].iloc[1] < 0.26  # ~0.25 hours (15 min)
        assert result["travel_speed_kmph"].iloc[1] == TRAVEL_SPEED_MODEL_CAP
    ...
    def test_multiple_users(self):
        ...
        # Bob second event: NY → Sydney in 0.5 hours — raw speed ~32,000 km/h,
        # capped at TRAVEL_SPEED_MODEL_CAP to prevent autoencoder saturation.
        bob_row = result[
        ...
        assert bob_row["travel_speed_kmph"] == TRAVEL_SPEED_MODEL_CAP
    ...
    def test_extreme_speed_capped(self):
        """Extreme speeds are capped at TRAVEL_SPEED_MODEL_CAP before model input.

        Sub-second simulation events can produce millions-of-km/h values that
        saturate the autoencoder hidden layers.  The cap keeps model inputs
        within a safe numerical range while still exceeding the impossible-travel
        detection threshold (800 km/h < 2000 km/h).
        """
        # Events 1 second apart, inter-city distance → millions km/h raw
        df = pd.DataFrame(
            {
                "username": ["alice@contoso.com", "alice@contoso.com"],
                "timestamp": ["2025-11-25 10:00:00", "2025-11-25 10:00:01"],
                "location_geoCoordinates_latitude": [51.5074, 35.6762],
                "location_geoCoordinates_longitude": [-0.1278, 139.6503],
            }
        )
    ...
    def test_speed_just_below_cap_not_clipped(self):
        """Speeds below TRAVEL_SPEED_MODEL_CAP are stored as-is."""
        # London to Paris in 1.5 hours → ~98 km/h — well below cap
        df = pd.DataFrame(
            {
                "username": ["alice@contoso.com", "alice@contoso.com"],
                "timestamp": ["2025-11-25 08:00:00", "2025-11-25 09:30:00"],
                "location_geoCoordinates_latitude": [51.5074, 48.8566],
                "location_geoCoordinates_longitude": [-0.1278, 2.3522],
            }
        )
        result = calculate_travel_features(df)
        speed = result["travel_speed_kmph"].iloc[1]
        assert speed < TRAVEL_SPEED_MODEL_CAP
        assert speed > 0
...
class TestDetectImpossibleTravel:
    ...
    def test_capped_speed_still_triggers_impossible_travel(self):
        """detect_impossible_travel() flags events whose speed was capped at TRAVEL_SPEED_MODEL_CAP.

        The cap (2000 km/h) is above the impossible-travel threshold (800 km/h),
        so capping must not suppress the security alert.
        """
        df = pd.DataFrame(
            {
                "username": ["alice@contoso.com", "alice@contoso.com"],
        ...
        df = calculate_travel_features(df)
        # Confirm capping happened
        assert df["travel_speed_kmph"].iloc[1] == TRAVEL_SPEED_MODEL_CAP
        df = detect_impossible_travel(df)
        # Capped value (2000) > threshold (800) → must still be flagged
        assert df["impossible_travel"].iloc[1] == True  # noqa: E712 - explicit True check for clarity
