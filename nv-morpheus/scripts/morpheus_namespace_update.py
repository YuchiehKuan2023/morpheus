...
def replace_imports_in_file(file_path, old_module, new_module):
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        ...
        if re.findall(rf"(import {old_module})(\W+)", content):
            do_write = True
            content = re.sub(
                rf"(import {old_module})(\W+)", rf"import {new_module}\2", content
            )
        ...
        if re.findall(rf"(from {old_module})(\S+)", content):
            do_write = True
            content = re.sub(
                rf"(from {old_module})(\S+)", rf"from {new_module}\2", content
            )
...
def replace_llm_imports_in_file(file_path):
    ...
    if re.findall(rf"(from morpheus\.)({module})", content):
        do_write = True
        content = re.sub(
            rf"(from morpheus\.)({module})", r"from morpheus_llm.\2", content
        )
    ...
    if re.findall(rf"(from morpheus\.)(\w+)(\.{module})", content):
        do_write = True
        content = re.sub(
            rf"(from morpheus\.)(\w+)(\.{module})", r"from morpheus_llm.\2\3", content
        )
    ...
    if re.findall(rf"(from morpheus\.)(\S+)(\.{module})", content):
        do_write = True
        content = re.sub(
            rf"(from morpheus\.)(\S+)(\.{module})", r"from morpheus_llm.\2\3", content
        )
    ...
    if re.findall(rf"(from morpheus\.)(\S+)( import {module})", content):
        do_write = True
        content = re.sub(
            rf"(from morpheus\.)(\S+)( import {module})",
            r"from morpheus_llm.\2\3",
            content,
        )
    ...
    if re.findall(rf"(import morpheus\.)(\S+)(\.{module})", content):
        do_write = True
        content = re.sub(
            rf"(import morpheus\.)(\S+)(\.{module})",
            r"import morpheus_llm.\2\3",
            content,
        )
