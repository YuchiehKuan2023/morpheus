...
def parse_args():
    argparser = argparse.ArgumentParser(
        "Compares two data files which are parsable as Pandas dataframes"
    )
    ...
    argparser.add_argument(
        ...
        nargs="*",
        help=(
            "Which columns to include in the validation. "
    ...
    argparser.add_argument(
        ...
        default=[r"^ID$", r"^_ts_"],
        help=(
            "Which columns to exclude from the validation. "
    ...
    argparser.add_argument(
        "--index_col",
        help=(
            "Specifies a column which will be used to align messages with rows in the validation dataset."
        ),
    )
    argparser.add_argument(
        "--abs_tol",
...
def main():
    ...
    df_a = read_file_to_df(
        args.data_files[0], file_type=FileTypes.Auto, df_type="pandas"
    )
    df_b = read_file_to_df(
        args.data_files[1], file_type=FileTypes.Auto, df_type="pandas"
    )
    results = compare_df(
        df_a,
        df_b,
