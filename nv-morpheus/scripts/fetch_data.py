    ...
    with subprocess.Popen(
        ...
        if returncode != 0:
            ...
            raise subprocess.CalledProcessError(
                returncode=returncode, cmd=cmd, output=output
            )
...
def lfs_check(list_all=False):
    output = subprocess.check_output(
        "git lfs ls-files", shell=True, universal_newlines=True
    )
    ...
    if not list_all:
        if missing_files:
            logging.error(
                "Missing the following LFS files:\n%s", "\n".join(missing_files)
            )
        else:
...
def parse_args():
    argparser = argparse.ArgumentParser(
        "Fetches data not included in the repository by default"
    )
    ...
    check_parser = subparsers.add_parser(
        "check",
        help=(
            "Check download status of large files. Exits with a status of 0 if all "
