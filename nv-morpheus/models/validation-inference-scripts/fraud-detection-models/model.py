...
class BaseHeteroGraph(nn.Module):
    ...
    def __init__(
        self, input_graph: dgl.DGLHeteroGraph, embedding_size: int, target: str
    ):
        super().__init__()
        self._target = target
    ...
    def forward(
        self, input_graph: dgl.DGLHeteroGraph, features: torch.tensor
    ...
    def inference(
        ...
        _, test_seed, test_embedding = self.evaluate(
            test_dataloader, feature_tensors, target_node
        )
...
class HeteroRGCN(BaseHeteroGraph):
    ...
    def __init__(
        ...
        super().__init__(
            input_graph=input_graph, embedding_size=embedding_size, target=target
        )
        ...
        self.layers.append(
            dglnn.HeteroGraphConv(
                {
                    rel: dglnn.GraphConv(in_sizes[rel], hidden_size)
...
class HinSAGE(BaseHeteroGraph):
    ...
    def __init__(
        ...
        in_feats = {
            rel: embedding_size if rel != self._target else in_size
            for rel in input_graph.ntypes
        }
        ...
        self.layers.append(
            dglnn.HeteroGraphConv(
                {
                    rel: dglnn.SAGEConv(
                        (in_feats[src_type], in_feats[v_type]),
                        hidden_size,
                        aggregator_type=aggregator_type,
                    )
                    for src_type, rel, v_type in input_graph.canonical_etypes
                },
                aggregate="sum",
            )
        )
...
def load_model(
    model_dir: str, gnn_model: BaseHeteroGraph = HinSAGE, device: torch.device = None
) -> (BaseHeteroGraph, dgl.DGLHeteroGraph, dict):
    ...
    model.load_state_dict(
        torch.load(os.path.join(model_dir, "model.pt"), weights_only=False)
    )
...
def build_fsi_graph(
    train_data: cf.DataFrame, col_drop: list[str]
...
def prepare_data(
    ...
    cf.DataFrame, cf.DataFrame, cf.Series, cf.Series, cupy.ndarray, cf.DataFrame
]:
    ...
    train_data, test_data, train_index, test_index, all_data = (
        cdf.iloc[:train_size, :],
        cdf.iloc[train_size:, :],
