...
def run_benchmark_pipeline(
    ...
    for _, sample in tqdm(dataset.iterrows(), total=len(dataset)):
        ...
        hybrid_results.append(
            json.dumps(gliner_processor.filter_entities(hybrid_findings))
        )
        ...
        if not model_skip:
            ...
            gliner_results.append(
                json.dumps(gliner_processor.filter_entities(gliner_findings))
            )
...
def evaluate_results(
    ...
    ground_truth = true_samples[true_samples["source"] == dataset][
        "privacy_mask"
    ].tolist()
