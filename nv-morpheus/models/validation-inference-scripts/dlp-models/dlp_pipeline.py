    ...
    def preprocess(self, text: str) -> list[str]:
        ...
        if self.split_by_paragraphs:
            ...
            for para in paragraphs:
                if (
                    len(current_chunk) + len(para) > self.chunking_size
                    and current_chunk
                ):
...
class DLPPipeline:
    ...
    def inference(self, document: str, failback: bool = False) -> list:
        ...
        if len(regex_findings) > 0:
            return self.gliner_processor.process(
                document, regex_findings, failback=failback
            )
        return []
    ...
    def process(self, document: str) -> dict:
        ...
        performance_metrics = {
            ...
            "regex_percentage": (total_regex_time / total_processing_time) * 100
            if total_processing_time > 0
            else 0,
            "gliner_percentage": (total_gliner_time / total_processing_time) * 100
            if total_processing_time > 0
            else 0,
            "throughput": len(document) / total_processing_time
            if total_processing_time > 0
            else 0,
        }
