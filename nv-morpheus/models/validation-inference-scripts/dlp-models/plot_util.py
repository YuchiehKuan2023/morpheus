...
def make_metrics_per_entity(results_per_tag: list, metric: str = "partial"):
    ...
    for entity in results_per_tag:
        recall, precision, f1 = (
            results_per_tag[entity][metric]["recall"],
...
def plot_detail_metrics(df, only_heatmap=True):
    ...
    heatmap_data = df_sorted[["entity", "recall", "precision", "f1"]].set_index(
        "entity"
    )
    sns.heatmap(
        heatmap_data, annot=True, cmap="YlGnBu", fmt=".3f", cbar_kws={"label": "Score"}
    )
    plt.title("Recall, Precision, and F1 Scores by Entity")
    plt.tight_layout()
    plt.show()
...
def visualize_throughput_latency(timing_metrics, output_path=None, dataset_name=None):
    ...
    ax3.bar(
        models, tokens_per_second, color=["#1f77b4", "#ff7f0e", "#2ca02c", "#d62728"]
    )
...
def visualize_latency(
    ...
    """
    if columns is None:
        columns = ["total_time", "throughput", "Average per row"]

    # Calculate speedup factors relative to GliNER model
    hybrid_total_time = latency_df[latency_df["model"] == speedup_model_factor][
        columns[2]
    ].iloc[0]
    hybrid_avg_time = latency_df[latency_df["model"] == speedup_model_factor][
        columns[1]
    ].iloc[0]
    ...
    for i, v in enumerate(latency_df[columns[2]]):
        speedup = hybrid_total_time / v
        if latency_df.iloc[i]["model"] == speedup_model_factor:
            ax1.text(
                i, v + v * 0.05, f"{v:.4f}s", ha="center", va="top", fontweight="bold"
            )
        else:
            ax1.text(
                i,
...
def visualize_risk_assessment(
    ...
    if risk_assessment["data_types_found"]:
        ...
        # Sort by weight
        data_types_sorted = [
            x for _, x in sorted(zip(weights, data_types), reverse=True)
        ]
        weights_sorted = sorted(weights, reverse=True)

        # Create color map based on weights
        colors = [
            "#FF0000" if w >= 80 else "#FFCC00" if w >= 50 else "#00CC00"
