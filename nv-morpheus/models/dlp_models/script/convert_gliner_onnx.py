...
def convert_bi_encoder_to_onnx(
    model, onnx_path, example_texts, example_labels, opset_version=14
):
    ...
    model_input, _ = model.prepare_model_inputs(
        example_texts, example_labels, prepare_entities=False
    )
    ...
    dynamic_axes = {
        ...
        "words_mask": {0: "batch_size", 1: "seq_length"},
    ...
    labels_path = os.path.join(
        os.path.dirname(onnx_path), "example_labels_embeddings.pt"
    )
...
def test_onnx_model(onnx_model_dir, test_texts, test_labels):
    ...
    model_input, raw_batch = gliner_model.prepare_model_inputs(
        test_texts, test_labels, prepare_entities=False
    )
...
def convert_gliner_to_onnx(
    model_name_or_path, onnx_output_dir, example_texts, example_labels
):
