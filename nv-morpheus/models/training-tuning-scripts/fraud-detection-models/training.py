...
def build_fsi_graph(train_data, col_drop):
    ...
    feature_tensors = (feature_tensors - feature_tensors.mean(0, keepdim=True)) / (
        0.0001 + feature_tensors.std(0, keepdim=True)
    )
    ...
    client_tensor, merchant_tensor, transaction_tensor = (
        client_tensor.view(-1),
...
def load_model(model_dir, gnn_model=HinSAGE):
    ...
    xgb_model = ForestInference.load(
        os.path.join(model_dir, "xgb.pt"), output_class=True
    )
...
def train(
    ...
    for _, (_, output_nodes, blocks) in enumerate(train_dataloader):
        ...
        train_acc = accuracy_score(
            logits.argmax(1).cpu(), labels[seed].cpu().long()
        ).item()
...
@click.command()
@click.option(
    "--training-data", help="Path to training data ", default="data/training.csv"
)
@click.option(
    "--validation-data", help="Path to validation data", default="data/validation.csv"
)
...
@click.option(
    "--output-file", help="Path to csv inference result", default="debug/out.csv"
)
...
def train_model(
    ...
    train_data, _, train_idx, inductive_idx, labels, df = prepare_data(
    ...
    scale_pos_weight = torch.FloatTensor([scale_pos_weight, 1 - scale_pos_weight]).to(
        device
    )
    ...
    train_loader, val_loader, test_loader = init_loaders(
        train_graph,
        train_idx,
        test_idx=inductive_idx,
        val_idx=inductive_idx,
        g_test=whole_graph,
        batch_size=batch_size,
    )
    ...
    # Set model variables
    model = gnn_model(
        whole_graph, in_size, hidden_size, out_size, n_layers, embedding_size
    ).to(device)
    ...
    for epoch in trange(epochs):
        ...
        print(
            f"Epoch {epoch}/{epochs} | Train Accuracy: {train_acc} | Train Loss: {loss}"
        )
        val_logits, val_seed, _ = evaluate(
            model, val_loader, feature_tensors, target_node
        )
        val_accuracy = accuracy(
            val_logits.argmax(1), labels.long()[val_seed].cpu(), "binary"
        ).item()
    ...
    # Create embeddings
    _, train_seeds, train_embedding = evaluate(
        model, train_loader, feature_tensors, target_node
    )
    test_logits, test_seeds, test_embedding = evaluate(
        model, test_loader, feature_tensors, target_node
    )

    # compute metrics
    test_acc = accuracy(
        test_logits.argmax(dim=1), labels.long()[test_seeds].cpu(), "binary"
    ).item()
    test_auc = roc_auc_score(
        labels.long()[test_seeds].cpu().numpy(), test_logits[:, 1].numpy()
    )
