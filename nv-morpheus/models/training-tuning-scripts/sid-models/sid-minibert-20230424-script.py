...
def data_preprocessing(training_data):
    ...
    cased_tokenizer = SubwordTokenizer(
        "resources/bert-base-uncased-hash.txt", do_lower_case=True
    )

    tokenizer_output = cased_tokenizer(
        df.data,
        max_length=256,
...
def model_eval(model, val_dataloader, idx2label):
    ...
    for label, cfm in zip(
        list(idx2label.values()), multilabel_confusion_matrix(true_bools, pred_bools)
    ):
        print(label)
        print(cfm)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--tokenizer-hash-filepath", required=True, help="hash file for tokenizer vocab"
    )
    parser.add_argument(
        "--output-file", required=True, help="output file to save new model"
    )
