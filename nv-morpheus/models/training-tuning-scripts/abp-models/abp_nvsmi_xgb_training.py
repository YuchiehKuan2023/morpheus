...
def train_val_split(df):
    (x_train, x_test, y_train, y_test) = train_test_split(
        df.drop(["label"], axis=1), df["label"], train_size=0.8, random_state=1
    )
...
def main(trainingdata):
    print("Preprocessing...")
    (x_train, x_test, y_train, y_test) = train_val_split(preprocess(trainingdata))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--trainingdata", required=True, help="Labelled data in JSON format"
    )
