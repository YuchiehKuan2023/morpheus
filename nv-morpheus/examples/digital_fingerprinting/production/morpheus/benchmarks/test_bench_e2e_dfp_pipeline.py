...
def dfp_modules_pipeline(
    ...
    source_stage = pipeline.add_stage(
        ControlMessageFileSourceStage(pipe_config, filenames=filenames)
    )
...
def dfp_training_pipeline_stages(
    pipe_config: Config,
    ...
    pipeline.add_stage(
        DFPPreprocessingStage(pipe_config, input_schema=preprocess_schema)
    )
...
def dfp_inference_pipeline_stages(
    pipe_config: Config,
    stages_conf: typing.Dict[str, typing.Any],
    source_schema: DataFrameInputSchema,
    preprocess_schema: DataFrameInputSchema,
    ...
    pipeline.add_stage(
        DFPInferenceStage(
            pipe_config, model_name_formatter=stages_conf["model_name_formatter"]
        )
    )
    pipeline.add_stage(
    ...
    pipeline.add_stage(
        WriteToFileStage(pipe_config, filename=output_filepath, overwrite=True)
    )
...
def test_dfp_modules_azure_payload_inference_e2e(benchmark: typing.Any):
    ...
    benchmark(
        dfp_modules_pipeline, pipe_config, module_config, filenames=input_filenames
    )
