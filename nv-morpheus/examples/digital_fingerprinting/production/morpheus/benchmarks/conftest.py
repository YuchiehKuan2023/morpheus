...
def pytest_benchmark_update_json(config, benchmarks, output_json):  # pylint:disable=unused-argument
    ...
    for gpu in device_query["gpu"]:
        ...
        output_json["machine_info"]["gpu_" + gpu_num]["utilization"] = (
            f"{gpu['utilization']['gpu_util']}{gpu['utilization']['unit']}"
        )
        output_json["machine_info"]["gpu_" + gpu_num]["total_memory"] = (
            f"{gpu['fb_memory_usage']['total']} {gpu['fb_memory_usage']['unit']}"
        )
        output_json["machine_info"]["gpu_" + gpu_num]["used_memory"] = (
            f"{gpu['fb_memory_usage']['used']} {gpu['fb_memory_usage']['unit']}"
        )
        output_json["machine_info"]["gpu_" + gpu_num]["free_memory"] = (
            f"{gpu['fb_memory_usage']['free']} {gpu['fb_memory_usage']['unit']}"
        )
        output_json["machine_info"]["gpu_" + gpu_num]["temperature"] = (
            f"{gpu['temperature']['gpu_temp']} {gpu['temperature']['unit']}"
        )
    ...
    for bench in output_json["benchmarks"]:
        ...
        if "file_path" in PIPELINES_CONF[bench["name"]]:
            ...
            source_files_glob = path.join(
                curr_dir, PIPELINES_CONF[bench["name"]]["glob_path"]
            )
            ...
            source_message_glob = path.join(
                curr_dir, PIPELINES_CONF[bench["name"]]["message_path"]
            )
            for message_fn in glob.glob(source_message_glob):
                ...
                for inpt in inputs:
                    ...
                    if non_load_task is not None:
                        ...
                        bench["stats"][non_load_task]["input_lines"] = (
                            line_count_per_task
                        )
                        bench["stats"][non_load_task]["input_bytes"] = (
                            byte_count_per_task
                        )

        else:
            raise KeyError(
                "Configuration requires either 'glob_path' or 'file_path' attribute."
            )

        bench["morpheus_config"] = {}
        bench["morpheus_config"]["num_threads"] = PIPELINES_CONF[bench["name"]][
            "num_threads"
        ]
        bench["morpheus_config"]["pipeline_batch_size"] = PIPELINES_CONF[bench["name"]][
            "pipeline_batch_size"
        ]
        bench["morpheus_config"]["edge_buffer_size"] = PIPELINES_CONF[bench["name"]][
            "edge_buffer_size"
        ]
        bench["morpheus_config"]["start_time"] = PIPELINES_CONF[bench["name"]][
            "start_time"
        ]
        ...
        bench["stats"]["median_throughput_lines"] = (
            line_count / bench["stats"]["median"]
        )
        ...
        bench["stats"]["median_throughput_bytes"] = (
            byte_count / bench["stats"]["median"]
        )
