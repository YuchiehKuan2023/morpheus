...
class BenchmarkConfGenerator:
    ...
    def _create_config(self):
        config = generate_ae_config(
            source=(self._pipe_conf.get("source")),
        ...
        if "glob_path" in self._pipe_conf:
            ...
            if "file_path" in self._pipe_conf:
                ...
                if "message_path" in self._pipe_conf:
                    ...
                    raise KeyError(
                        "Configuration needs the glob path or file path attribute."
                    )
    ...
    def get_module_conf(self):
        ...
        dfp_arg_parser = DFPArgParser(
            ...
            only_user=[],
            start_time=(
                datetime.strptime(self._pipe_conf.get("start_time"), "%Y-%m-%d")
            ),
        ...
        config_generator = ConfigGenerator(
            self.pipe_config, dfp_arg_parser, self.get_schema()
        )
