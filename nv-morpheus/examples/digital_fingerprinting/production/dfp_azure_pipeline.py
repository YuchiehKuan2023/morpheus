...
@click.option(
    ...
    help=(
        "Indicates whether or not to train per user or a generic model for all users. "
...
@click.option(
    ...
    multiple=True,
    help=(
        "List of files to process. Can specify multiple arguments for multiple files. "
...
@click.option(
    ...
    callback=lambda _, __, value: None
    if value is None
    else _file_type_name_to_enum(value),
)
@click.option(
    "--watch_inputs",
    type=bool,
    ...
    default=False,
    help=(
        "Instructs the pipeline to continuously check the paths specified by `--input_file` for new files. "
...
@click.option(
    ...
    default=1.0,
    help=(
        "Amount of time, in seconds, to wait between checks for new files. "
...
@click.option(
    "--use_postproc_schema",
    is_flag=True,
    help="Assume that input data has already been preprocessed.",
)
@click.option(
    "--inference_detection_file_name", type=str, default="dfp_detections_azure.csv"
)
def run_pipeline(
    train_users,
    ...
    **kwargs,
    ...
    if len(skip_users) > 0 and len(only_users) > 0:
        logging.error(
            "Option --skip_user and --only_user are mutually exclusive. Exiting"
        )
    ...
    config.ae.feature_columns = load_labels_file(
        get_package_relative_file("data/columns_ae_azure.txt")
    )
    ...
    if use_postproc_schema:
        source_column_info = [
            ...
            DateTimeColumn(
                name=config.ae.timestamp_column_name, dtype=datetime, input_name="time"
            ),
            ColumnInfo(name="appDisplayName", dtype=str),
            ColumnInfo(name="clientAppUsed", dtype=str),
            RenameColumn(
                name=config.ae.userid_column_name,
                dtype=str,
                input_name="userPrincipalName",
            ),
            RenameColumn(
                name="deviceDetailbrowser", dtype=str, input_name="deviceDetail_browser"
            ),
            RenameColumn(
                name="deviceDetaildisplayName",
                dtype=str,
                input_name="deviceDetail_displayName",
            ),
            RenameColumn(
                name="deviceDetailoperatingSystem",
                dtype=str,
                input_name="deviceDetail_operatingSystem",
            ),
        ...
        preprocess_schema = DataFrameInputSchema(
            column_info=preprocess_column_info, preserve_columns=["_batch_id"]
        )
        ...
        config.ae.feature_columns = [
            name
            for (name, dtype) in preprocess_schema.output_columns
            if name not in exclude_from_training
        ]
    else:
        source_column_info = [
            DateTimeColumn(
            ...
            RenameColumn(
                name="appDisplayName", dtype=str, input_name="properties.appDisplayName"
            ),
            ColumnInfo(name="category", dtype=str),
            RenameColumn(
                name="clientAppUsed", dtype=str, input_name="properties.clientAppUsed"
            ),
            RenameColumn(
        ...
        source_schema = DataFrameInputSchema(
            json_columns=["properties"], column_info=source_column_info
        )
    ...
    pipeline.add_stage(
        DFPFileBatcherStage(
            ...
            date_conversion_func=functools.partial(
                date_extractor, filename_regex=iso_date_regex
            ),
    ...
    if is_training:
        ...
        pipeline.add_stage(
            MonitorStage(config, description="Training rate", smoothing=0.001)
        )
        ...
        pipeline.add_stage(
            DFPInferenceStage(config, model_name_formatter=model_name_formatter)
        )

        pipeline.add_stage(
            MonitorStage(config, description="Inference rate", smoothing=0.001)
        )
        ...
        pipeline.add_stage(
            SerializeStage(
                config, exclude=["batch_count", "origin_hash", "_row_hash", "_batch_id"]
            )
        )
        ...
        pipeline.add_stage(
            WriteToFileStage(
                config, filename=inference_detection_file_name, overwrite=True
            )
        )
