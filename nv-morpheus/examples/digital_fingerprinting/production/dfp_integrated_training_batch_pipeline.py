...
from morpheus.stages.input.control_message_file_source_stage import (
...
@click.option(
    ...
    default=None,
    help=(
        "The MLflow experiment name template to use when logging experiments."
...
@click.option(
    ...
    default=None,
    help=(
        "The MLflow model name template to use when logging models."
...
@click.option(
    ...
    is_flag=True,
    help=(
        "Enabling this option will skip pre-filtering of json messages. "
...
@click.option(
    ...
    multiple=True,
    help=(
        "List of control message defination files to process. Can specify multiple arguments for multiple files."
...
@click.option(
    "--silence_monitors",
    flag_value=True,
    help="Controls whether monitors will be verbose.",
)
def run_pipeline(
    source: str,
    train_users: str,
    ...
    **kwargs,
    ...
    config: Config = generate_ae_config(
        source, userid_column_name, timestamp_column_name
    )
    ...
    source_stage = pipeline.add_stage(
        ControlMessageFileSourceStage(config, filenames=list(kwargs["input_file"]))
    )
