...
def main(args):
    ...
    with open(args.output_file, "w", encoding="UTF-8") as f:
        while True:
            ...
            df.rename(
                columns={"gpu.id": "gpu.@id", "count": "attached_gpus"}, inplace=True
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--interval-ms",
        default=1000,
        help="interval in ms between writes to output file",
    )
    parser.add_argument(
        "--output-file", default="nvsmi.jsonlines", help="output file to save dataset"
    )
