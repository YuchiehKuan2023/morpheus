...
class DatasetsSourceStage(GpuAndCpuMixin, SingleOutputSource):
    ...
    @classmethod
    def process_gretel_dataset(
        cls, df: pd.DataFrame, num_samples: int | None, include_privacy_masks: bool
    ) -> pd.DataFrame:
    ...
    ) -> collections.abc.Iterator[MessageMeta]:
        for dataset_name in self._dataset_names:
            dataset = load_dataset(
                self.AVAILABLE_DATASETS[dataset_name], split="validation"
            )
            df = dataset.to_pandas()

            if dataset_name == "gretel":
                df = self.process_gretel_dataset(
                    df, self._num_samples, self._include_privacy_masks
                )

            if self._include_privacy_masks:
                df["privacy_mask"] = df["privacy_mask"].apply(
                    self.normalize_privacy_masks
                )
