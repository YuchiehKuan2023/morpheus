...
class GliNERTritonInference:
    ...
    def pre_process(self, texts):
        ...
        """
        # === 1. PRE-PROCESSING ===
        model_input, raw_batch = self.model.prepare_model_inputs(
            texts, self.labels, prepare_entities=False
        )
    ...
    def process(self, texts: list[str], callback):
        ...
        for name, data in onnx_inputs:
            triton_input = tritonclient.InferInput(
                name, data.shape, tritonclient.np_to_triton_dtype(data.dtype)
            )
