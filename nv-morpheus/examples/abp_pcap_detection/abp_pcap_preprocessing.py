...
class AbpPcapPreprocessingStage(PreprocessBaseStage):
    ...
    def pre_process_batch(
        msg: ControlMessage,
        fea_len: int,
        fea_cols: typing.List[str],
        req_cols: typing.List[str],
    ) -> ControlMessage:
        meta = msg.payload()
        ...
        df = cudf.DataFrame(
            {rename_cols_dct[i]: flag_array.list.get(i) for i in range(5)},
            index=orig_df.index,
        )
        ...
        df["rollup_time"] = cudf.to_datetime(df["rollup_time"], unit="us").dt.strftime(
            "%Y-%m-%d %H:%M"
        )
        ...
        grouped_df["ackpush/all"] = (
            grouped_df["ack"] + grouped_df["psh"]
        ) / grouped_df["all"]
