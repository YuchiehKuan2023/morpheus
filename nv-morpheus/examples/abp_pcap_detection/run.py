...
@click.option(
    ...
    default=False,
    help=(
        "Iterative mode will emit DataFrames one at a time. Otherwise a list of DataFrames is emitted. "
        "Iterative mode is good for interleaving source stages."
    ),
)
@click.option(
    "--server_url", required=True, help="Tritonserver url.", default="localhost:8000"
)
@click.option(
    "--file_type",
    ...
    default="auto",
    help=(
        "Indicates what type of file to read. "
...
def run_pipeline(
    ...
    pipeline.add_stage(
        TritonInferenceStage(config, model_name=model_name, server_url=server_url)
    )
