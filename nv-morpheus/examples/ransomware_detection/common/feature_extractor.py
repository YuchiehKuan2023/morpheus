...
class FeatureExtractor:
    ...
    def _filter_by_pid_process(
        self, plugin_dict: typing.Dict[str, pd.DataFrame], pid_process: str
    ...
    def _extract_threadlist(self, x: pd.DataFrame):
        ...
        for wait_reason in fc.WAIT_REASON_LIST:
            ...
            self._features["threadlist_df_wait_reason_" + wait_reason] = len(
                wait_reason_df
            )
    ...
    def _extract_cc_vad_page_noaccess(self, commit_charge: pd.Series):
        ...
        if not commit_charge.empty:
            self._features["get_commit_charge_min_vad_page_noaccess"] = (
                commit_charge.min()
            )
            self._features["get_commit_charge_mean_vad_page_noaccess"] = (
                commit_charge.mean()
            )
    ...
    def _extract_vadinfo(self, x: pd.DataFrame):
        ...
        commit_charge = x[
            (x.Protection == fc.PAGE_NOACCESS) & (x.Tag == fc.VAD)
        ].CommitCharge
    ...
    def _page_execute_readwrite(self, x: ProtectionData):
        ...
        if x.commit_charge_size:
            self._features["get_commit_charge_mean_page_execute_readwrite"] = (
                commit_charge.mean()
            )
            self._features["get_commit_charge_min_page_execute_readwrite"] = (
                commit_charge.min()
            )
            self._features["get_commit_charge_max_page_execute_readwrite"] = (
                commit_charge.max()
            )
            self._features["get_commit_charge_sum_page_execute_readwrite"] = (
                commit_charge.sum()
            )
            self._features["get_commit_charge_std_page_execute_readwrite"] = (
                commit_charge.std(ddof=0)
            )
        ...
        if x.protection_df_size:
            ...
            self._features["page_execute_readwrite_ratio"] = (
                x.protection_df_size / x.vadinfo_df_size
            )
        ...
        if x.vads_protection_size:
            ...
            self._features["page_execute_readwrite_vads_ratio"] = (
                x.vads_protection_size / x.vadsinfo_size
            )
    ...
    def _page_noaccess(self, x: ProtectionData):
        ...
        if x.commit_charge_size:
            self._features["get_commit_charge_mean_page_no_access"] = (
                commit_charge.mean()
            )
        ...
        if x.protection_df_size:
            ...
            self._features["page_no_access_ratio"] = (
                x.protection_df_size / x.vadinfo_df_size
            )
        ...
        if x.vads_protection_size:
            self._features["page_no_access_vads_ratio"] = (
                x.vads_protection_size / x.vadsinfo_size
            )

        if x.vad_protection_size:
            self._features["page_no_access_vad_ratio"] = (
                x.vad_protection_size / x.vadinfo_size
            )
    ...
    def _page_execute_writecopy(self, x: ProtectionData):
        ...
        if x.commit_charge_size:
            self._features["get_commit_charge_min_page_execute_writecopy"] = (
                commit_charge.min()
            )
            self._features["get_commit_charge_sum_page_execute_writecopy"] = (
                commit_charge.sum()
            )
        ...
        if x.vad_protection_size:
            self._features["page_execute_writecopy_vad_ratio"] = (
                x.vad_protection_size / x.vadinfo_size
            )
    ...
    def _page_readonly(self, x: ProtectionData):
        ...
        if x.commit_charge_size:
            self._features["get_commit_charge_mean_page_readonly"] = (
                commit_charge.mean()
            )
        ...
        if x.protection_df_size:
            ...
            self._features["page_readonly_ratio"] = (
                x.protection_df_size / x.vadinfo_df_size
            )
        ...
        if x.vads_protection_size:
            self._features["page_readonly_vads_ratio"] = (
                x.vads_protection_size / x.vadsinfo_size
            )

        if x.vad_protection_size:
            self._features["page_readonly_vad_ratio"] = (
                x.vad_protection_size / x.vadinfo_size
            )
    ...
    def _page_readwrite(self, x: ProtectionData):
        ...
        if x.protection_df_size:
            self._features["page_readwrite_ratio"] = (
                x.protection_df_size / x.vadinfo_df_size
            )
        ...
        if x.vads_protection_size:
            self._features["page_readwrite_vads_ratio"] = (
                x.vads_protection_size / x.vadsinfo_size
            )

        if x.vad_protection_size:
            self._features["page_readwrite_vad_ratio"] = (
                x.vad_protection_size / x.vadinfo_size
            )
    ...
    def _extract_protections(
        ...
        for protection in fc.PROTECTIONS:
            p_data = self._get_protection_data(
                x, protection, vadinfo_df_size, vadsinfo_size, vadinfo_size
            )
        ...
        self._features["vads_page_execute_writecopy_ratio"] = vadsinfo_size / (
            page_execute_writecopy_count + 1
        )
    ...
    def _extract_file_handle_dirs(self, file_paths: pd.Series):
        ...
        filepath_split_df = file_paths[
            file_paths.str.split("\\").str.len() > 3
        ].str.split("\\", expand=True)

        if not filepath_split_df.empty:
            # Count the unique directories
            directories_uniques_count = len(
                file_paths.str.extract("^(.*)\\\\.*")[0].unique()
            )
            if len(filepath_split_df) > 3:
                filepath_split_df = filepath_split_df[
                    (~filepath_split_df[4].isna())
                    & (filepath_split_df[1] == "device")
                    & (filepath_split_df[2].str.contains("harddisk"))
                ]

                windows = filepath_split_df[
                    filepath_split_df[3].str.contains("windows")
                ]
            ...
            self._features["count_directories_handles_uniques"] = (
                directories_uniques_count
            )
    ...
    def _extract_handles(self, x: pd.DataFrame):
        ...
        self._features["count_extension_handles_uniques"] = len(
            file_extensions.unique()
        )
        ...
        self._features["handles_df_name_unique_ratio"] = name_unique_count / (
            self._features["handles_df_count"] + 1
        )
        ...
        self._features["handles_df_type_unique_ratio"] = type_unique_count / (
            self._features["handles_df_count"] + 1
        )
    ...
    def extract_features(
        self, x: pd.DataFrame, feas_all_zeros: typing.Dict[str, int]
    ) -> pd.DataFrame:
        """
        ...
        plugin_dict = {
            plugin: x[x.plugin == plugin] for plugin in self._config.interested_plugins
        }
