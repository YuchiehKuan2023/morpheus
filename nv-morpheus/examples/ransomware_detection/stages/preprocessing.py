...
class PreprocessingRWStage(PreprocessBaseStage):
    ...
    def _sliding_window_offsets(
        self, ids: list[int], ids_len: int, window: int
    ) -> list[tuple[int]]:
        """
        ...
        for start in range(ids_len - (window - 1)):
            ...
            consecutive = sorted(sequence) == list(
    ...
    def _rollover_pending_snapshots(
        self,
        ...
        for snapshot_id in snapshot_ids[1 - self._sliding_window :]:
            pending_snapshot_data = snapshot_df[
                snapshot_df.index == snapshot_id
            ].values[0]
    ...
    def _merge_curr_and_prev_snapshots(
        self, snapshot_df: pd.DataFrame, source_pid_process: str
    ) -> pd.DataFrame:
        """
        ...
        for prev_pending_snapshot in prev_pending_snapshots:
            snapshot_df.loc[prev_pending_snapshot.snapshot_id] = (
                prev_pending_snapshot.data
            )
    ...
    def _pre_process_batch(self, msg: ControlMessage) -> ControlMessage:
        ...
        if source_pid_process in self._snapshot_dict:
            snapshot_df = self._merge_curr_and_prev_snapshots(
                snapshot_df, source_pid_process
            )
        ...
        if curr_and_prev_snapshots_size >= self._sliding_window:
            ...
            offsets = self._sliding_window_offsets(
                snapshot_ids, curr_and_prev_snapshots_size, window=self._sliding_window
            )
            ...
            for start, stop in offsets:
                ...
                sequence[start] = (
                    str(snapshot_df.index[start])
                    + "-"
                    + str(snapshot_df.index[stop - 1])
                )
        ...
        memory = InferenceMemoryFIL(
            count=curr_snapshots_size, input__0=data, seq_ids=seq_ids
        )
