...
def _build_engine(llm_service: str):
    ...
    engine.add_node(
        ...
        node=PromptTemplateNode(
            template="What is the capital of {{country}}?", template_format="jinja"
        ),
    )

    engine.add_node(
        "completion", inputs=["/prompts"], node=LLMGenerateNode(llm_client=llm_clinet)
    )
...
def pipeline(
    ...
    if input_file is not None:
        source_df = read_file_to_df(
            input_file, df_type=exec_mode_to_df_type_str(config.execution_mode)
        )
    else:
    ...
    pipe.set_source(
        InMemorySourceStage(config, dataframes=[source_df], repeat=repeat_count)
    )
    ...
    pipe.add_stage(
        LLMEngineStage(config, engine=_build_engine(llm_service=llm_service))
    )

    pipe.add_stage(
        MonitorStage(
            config, description="Inference rate", unit="req", delayed_start=True
        )
    )
    ...
    logger.info(
        "Pipeline complete. Received %s responses\n%s",
        len(messages),
        responses["response"],
    )
