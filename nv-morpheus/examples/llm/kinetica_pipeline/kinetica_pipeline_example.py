    ...
    df = cudf.DataFrame(
        {
            ...
            "embeddings": [
                [random.random() for _ in range(3)] for _ in range(num_input_rows)
            ],
            "metadata": [
                json.dumps({"metadata": f"Sample metadata for row {i}"})
...
def main():
    ...
    kinetica_db_service = KineticaVectorDBService(
        host, user=username, password=password, kinetica_schema=schema
    )
    collection_name = "test_collection"
    collection_name = (
        f"{schema}.{collection_name}"
        if schema is not None and len(schema) > 0
        else f"ki_home.{collection_name}"
    )
    ...
    pipeline.add_stage(
        WriteToVectorDBStage(config, kinetica_db_service, "test_collection")
    )
