...
@run.command(
    help="Runs a simple finite pipeline with a single execution of a LangChain agent from a fixed input"
)
@click.option(
    "--use_cpu_only",
    default=False,
...
@run.command(
    help="Runs a pipeline LangChain agents which pulls inputs from a Kafka message bus"
)
@click.option(
    "--num_threads",
    default=len(os.sched_getaffinity(0)),
