...
def _build_engine(model_name: str) -> LLMEngine:
    ...
    engine.add_node(
        ...
        node=LangChainAgentNode(
            agent_executor=_build_agent_executor(model_name=model_name)
        ),
    )
...
def build_common_pipeline(
    config: Config, pipe: LinearPipeline, task_payload: dict, model_name: str
) -> InMemorySinkStage:
    """
    ...
    pipe.add_stage(
        MonitorStage(
            config, description="Agent rate", unit="events", delayed_start=True
        )
    )
