...
def pipeline(
    ...
    nlp_stage = pipe.add_stage(
        PreprocessNLPStage(pipeline_config, **tokenizer_config.get("model_kwargs", {}))
    )

    monitor_1 = pipe.add_stage(
        MonitorStage(
            pipeline_config,
            description="Tokenize rate",
            unit="events",
            delayed_start=True,
        )
    )
    ...
    embedding_stage = pipe.add_stage(
        TritonInferenceStage(
            pipeline_config, **embeddings_config.get("model_kwargs", {})
        )
    )
    ...
    @stage
    def embedding_tensor_to_df(
        message: ControlMessage, *, embedding_tensor_name="probs"
    ) -> ControlMessage:
        """
    ...
    embedding_tensor_to_df_stage = pipe.add_stage(
        embedding_tensor_to_df(pipeline_config)
    )
