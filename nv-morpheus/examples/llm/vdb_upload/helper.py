...
def setup_rss_source(
    pipe: Pipeline,
    config: Config,
    source_name: str,
    rss_config: typing.Dict[str, typing.Any],
):
    """
...
def setup_filesystem_source(
    ...
    config: Config,
    source_name: str,
    fs_config: typing.Dict[str, typing.Any],
):
    """
    ...
    module_loader = FileSourcePipeLoaderFactory.get_instance(
        module_name=f"file_source_pipe__{source_name}",
        module_config={"file_source_config": fs_config},
    )
    file_pipe = pipe.add_stage(
        LinearModuleSourceStage(
            config, module_loader, output_type=ControlMessage, output_port_name="output"
        )
    )
...
def setup_doca_source(
    pipe: Pipeline,
    config: Config,
    source_name: str,
    stage_config: typing.Dict[str, typing.Any],
):
    """
...
def process_vdb_sources(
    pipe: Pipeline, config: Config, vdb_source_config: typing.List[typing.Dict]
) -> typing.List:
    """
    ...
    for source_info in vdb_source_config:
        ...
        if source_type == "rss":
            vdb_sources.append(
                setup_rss_source(pipe, config, source_name, source_config)
            )
        elif source_type == "filesystem":
            vdb_sources.append(
                setup_filesystem_source(pipe, config, source_name, source_config)
            )
        elif source_type == "custom":
            vdb_sources.append(
                setup_custom_source(pipe, config, source_name, source_config)
            )
        elif source_type == "doca":
            vdb_sources.append(
                setup_doca_source(pipe, config, source_name, source_config)
            )
        else:
