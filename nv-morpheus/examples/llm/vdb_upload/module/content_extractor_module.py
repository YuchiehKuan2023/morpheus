...
ContentExtractorLoaderFactory = ModuleLoaderFactory(
    "file_content_extractor", "morpheus_examples_llm", ContentExtractorSchema
)
...
def process_content(
    docs: str | list[str], file_meta: FileMeta, chunk_size: int, chunk_overlap: int
) -> list[dict]:
    """
...
def file_content_extractor(builder: mrc.Builder):
    ...
    try:
        ...
        log_error_message = (
            f"Invalid configuration for file_content_extractor: {error_messages}"
        )
    ...
    chunk_params = {
        file_type: {
            ...
            "chunk_size": converters_meta.get(file_type, {}).get(
                "chunk_size", chunk_size
            ),
            "chunk_overlap": converters_meta.get(file_type, {}).get(
            ...
            for i in range(0, len(open_files), batch_size):
                ...
                for open_file in batch:
                    ...
                    if not _fs.exists(open_file.path):
                        logger.warning(
                            "File does not exist: %s. Skipping...", open_file.path
                        )
                        continue

                    if _fs.isdir(open_file.path):
                        logger.warning(
                            "File is a directory: %s. Skipping...", open_file.path
                        )
                        continue
                    ...
                    try:
                        ...
                        futures.append(
                            executor.submit(read_file_to_bytesio, file_meta.file_path)
                        )
                ...
                for file_meta, future in zip(files_meta, futures):
                    ...
                    if io_bytes:
                        ...
                        input_info = ConverterInputInfo(
                            io_bytes=io_bytes, meta=converters_meta
                        )
                        result = converter(input_info)
                        # Get chunk params for the file type, default to txt
                        file_type_chunk_params = (
                            chunk_params[file_meta.file_type]
                            if file_meta.file_type in chunk_params
                            else chunk_params["txt"]
                        )
    ...
    node = builder.make_node(
        "text_extractor", ops.map(parse_files), ops.filter(lambda x: x is not None)
    )
