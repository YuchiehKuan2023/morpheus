...
SchemaTransformLoaderFactory = ModuleLoaderFactory(
    "schema_transform", "morpheus_examples_llm", SchemaTransformSchema
)
...
def _schema_transform(builder: mrc.Builder):
        ...
        if op_type == "rename":
            ...
            source_column_info.append(
                RenameColumn(
                    ...
                    input_name=col_config["from"],
                )
            )
        elif op_type == "select":
            # Handling regular columns
            source_column_info.append(
                ColumnInfo(name=col_name, dtype=col_config["dtype"])
            )
        else:
        ...
        with message.mutable_dataframe() as mdf:
            ...
            for col_info in source_schema.column_info:
                try:
                    ...
                    logger.exception(
                        "Failed to process column '%s'. Dataframe: \n%s\n%s",
                        col_info.name,
                        mdf,
                        exc_info,
                    )
    ...
    node = builder.make_node(
        "schema_transform", ops.map(do_transform), ops.filter(lambda x: x is not None)
    )
