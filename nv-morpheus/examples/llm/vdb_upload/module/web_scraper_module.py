...
WebScraperLoaderFactory = ModuleLoaderFactory(
    "web_scraper", "morpheus_examples_llm", WebScraperSchema
)


def download_and_split(
    msg: MessageMeta, text_splitter, link_column, session
) -> MessageMeta:
    """
    ...
    for row in df_dicts:
        ...
        try:
            ...
            logger.error(
                "Error downloading document from URL '%s'. Error: %s", url, exc
            )
            continue
...
def _web_scraper(builder: mrc.Builder):
    ...
    try:
        web_scraper_config = WebScraperSchema(
            **module_config.get("web_scraper_config", {})
        )
    ...
    node = builder.make_node(
        "web_scraper", ops.map(op_func), ops.filter(lambda x: x is not None)
    )
