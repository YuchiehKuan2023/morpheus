...
class ClassificationStage(SinglePortStage):
    def __init__(self, c: Config, model_xgb_file: str):
        ...
        self._xgb_model = cuml.ForestInference.load(
            model_xgb_file, output_class=True, model_type="xgboost"
        )
        self._needed_columns.update(
            {"node_id": TypeId.INT64, "prediction": TypeId.FLOAT32}
        )
    ...
    def _process_message(self, message: ControlMessage) -> ControlMessage:
        inductive_embedding_column_names = message.get_metadata(
