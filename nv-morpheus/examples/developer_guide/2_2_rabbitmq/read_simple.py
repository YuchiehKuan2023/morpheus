...
@click.option(
    ...
    type=bool,
    is_flag=True,
    help=("Whether or not to run in CPU only mode"),
)
@click.option(
    "--num_threads",
...
def run_pipeline(use_source_function: bool, use_cpu_only: bool, num_threads: int):
    ...
    if use_source_function:
        ...
        pipeline.set_source(
            RabbitMQSourceStage(config, host="localhost", exchange="logs")
        )
    ...
    pipeline.add_stage(
        WriteToFileStage(
            config, filename="results.json", file_type=FileTypes.JSON, overwrite=True
        )
    )
