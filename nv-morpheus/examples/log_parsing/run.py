...
@click.option(
    "--server_url", required=True, help="Tritonserver url.", default="localhost:8001"
)
def run_pipeline(
    num_threads,
    pipeline_batch_size,
    ...
    pipeline.set_source(
        FileSourceStage(config, filename=input_file, iterative=False, repeat=1)
    )
    ...
    pipeline.add_stage(
        LogParsingPostProcessingStage(
            config, vocab_path=model_vocab_file, model_config_path=model_config_file
        )
    )
