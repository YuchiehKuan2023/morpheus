...
class LogParsingPostProcessingStage(SinglePortStage):
    def __init__(
        self, c: Config, vocab_path: pathlib.Path, model_config_path: pathlib.Path
    ):
        """
    ...
    def _postprocess(self, msg: ControlMessage):
        ...
        infer_pdf = pd.DataFrame(
            {"doc": src_index, "start": seq_ids[:, 1], "stop": seq_ids[:, 2]}
        )
        ...
        for label in ext_parsed.columns:
            if label[0] == "B":
                ...
                if "I-" + col_name in ext_parsed.columns:
                    parsed_df[col_name] = (
                        ext_parsed[label] + " " + ext_parsed["I-" + col_name].fillna("")
                    )
                else:
