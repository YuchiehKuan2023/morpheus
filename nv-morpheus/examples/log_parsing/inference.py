...
class TritonInferenceLogParsing(TritonInferenceWorker):
    ...
    def build_output_message(self, msg: ControlMessage) -> ControlMessage:
        ...
        memory = TensorMemory(
            ...
            tensors={
                "confidences": cp.zeros(
                    (
                        msg.tensor_count(),
                        self._inputs[list(self._inputs.keys())[0]].shape[1],
                    )
                ),
                "labels": cp.zeros(
                    (
                ...
                "input_ids": cp.zeros(
                    (msg.tensor_count(), msg.tensors().get_tensor("input_ids").shape[1])
                ),
                "seq_ids": seq_ids,
            },
        )
    ...
    def _build_response(
        self, batch: ControlMessage, result: tritonclient.InferResult
    ) -> TensorMemory:
        outputs = {
            output.mapped_name: result.as_numpy(output.name)
        ...
        return TensorMemory(
            count=outputs[list(outputs.keys())[0]].shape[0],
            tensors={
...
class LogParsingInferenceStage(TritonInferenceStage):
    ...
    def _convert_one_response(
        ...
        input_ids[batch_offset : inf.tensor_count() + batch_offset, :] = (
            inf.tensors().get_tensor("input_ids")
        )
        ...
        if inf.payload().count == inf.tensor_count():
            ...
            confidences[batch_offset : inf.tensor_count() + batch_offset, :] = (
                resp_confidences
            )
            ...
            for i, idx in enumerate(mess_ids):
                confidences[idx, :] = cp.maximum(
                    confidences[idx, :], resp_confidences[i, :]
                )
