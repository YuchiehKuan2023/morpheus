...
@click.option(
    ...
    show_default=True,
    help=(
        "Size of the internal buffer channel used by the DocaConvertStage, if None, the value of `--edge_buffer_size`"
        " will be used."
    ),
)
@click.option(
...
def run_pipeline(
    ...
    pipeline.add_stage(
        MonitorStage(
            config, description="Convert rate", unit="pkts", delayed_start=True
        )
    )
