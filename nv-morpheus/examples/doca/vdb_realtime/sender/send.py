...
def send_data(
    ...
    packets = [
        IP(**ip_kwargs)
        / net_type_cls(sport=sport, dport=dport)
        / Raw(load=content.encode("utf-8"))
        for content in data
    ]
...
@click.option(
    "--iface",
    help="Ethernet device to use, useful for systems with multiple NICs",
    required=False,
)
@click.option(
    "--src_ip",
    help="Source IP to send from, useful for systems with multiple IPs",
    required=False,
)
...
@click.option(
    "--net_type", type=click.Choice(["TCP", "UDP"], case_sensitive=False), default="UDP"
)
@click.option(
    "--input_data_glob",
    type=str,
    default=os.path.join(
        MORPHEUS_ROOT, "examples/doca/vdb_realtime/sender/dataset/*.txt"
    ),
