...
def run_pipeline(
    log_level: int, use_cpu_only: bool, in_file: pathlib.Path, out_file: pathlib.Path
):
    ...
    def calculate_totals(
        msg: ControlMessage, *, total_column_name: str = "total"
    ) -> ControlMessage:
        meta = msg.payload()

        with meta.mutable_dataframe() as df:
            logger.debug(
                "Received a ControlMessage with a dataframe of type %s", type(df)
            )
    ...
    logger.info(
        "Running pipeline\tC++ mode = %s\texecution_mode = %s",
