...
class FieldSchemaEncoder(json.JSONEncoder):
    ...
    def load(f_obj: typing.IO) -> "pymilvus.FieldSchema":
        ...
        """
        return pymilvus.FieldSchema.construct_from_dict(
            json.load(f_obj, object_hook=FieldSchemaEncoder.object_hook)
        )
    ...
    def loads(field: str) -> "pymilvus.FieldSchema":
        ...
        return pymilvus.FieldSchema.construct_from_dict(
            json.loads(field, object_hook=FieldSchemaEncoder.object_hook)
        )
...
class MilvusVectorDBResourceService(VectorDBResourceService):
    ...
    def __init__(
        self, name: str, client: "MilvusClient", truncate_long_strings: bool = False
    ) -> None:
        if IMPORT_EXCEPTION is not None:
            raise ImportError(
                IMPORT_ERROR_MESSAGE.format(package="pymilvus")
    ...
    def insert(
        self, data: list[list] | list[dict], **kwargs: dict[str, typing.Any]
    ) -> dict:
        """
    ...
    def insert_dataframe(
        self, df: DataFrameType, **kwargs: dict[str, typing.Any]
    ) -> dict:
        """
        ...
        **kwargs : dict[str, typing.Any]
        ...
        for field_name, dtype in self._fillna_fields_dict.items():
            if dtype in (pymilvus.DataType.VARCHAR, pymilvus.DataType.STRING):
                ...
                logger.info(
                    "Skipped checking 'None' in the field: %s, with datatype: %s",
                    field_name,
                    dtype,
                )
        ...
        if needs_truncate and is_cudf_type(df):
            ...
            needs_truncate = cudf_string_cols_exceed_max_bytes(
                df, self._fields_max_length
            )
        ...
        if needs_truncate:
            truncate_string_cols_by_bytes(
                collection_df, self._fields_max_length, warn_on_truncate=True
            )
    ...
    def query(self, query: str, **kwargs: dict[str, typing.Any]) -> typing.Any:
        ...
        **kwargs : dict
        ...
        logger.debug(
            "Searching in collection: %s, query=%s, kwargs=%s",
            self._name,
            query,
            kwargs,
        )
    ...
    async def similarity_search(
        self, embeddings: list[list[float]], k: int = 4, **kwargs: dict[str, typing.Any]
    ) -> list[dict]:
        """
        ...
        **kwargs : dict[str, typing.Any]
        ...
        assert self._vector_field is not None, (
            "Cannot perform similarity search on a collection without a vector field"
        )
        ...
        for res in response:
            outputs.append(
                [{x: hit.entity.get(x) for x in output_fields} for hit in res]
            )

        return outputs

    def update(
        self, data: list[typing.Any], **kwargs: dict[str, typing.Any]
    ) -> dict[str, typing.Any]:
        """
        ...
        **kwargs : dict[str, typing.Any]
        ...
        result = self._client.upsert(
            collection_name=self._name, entities=data, **kwargs
        )
    ...
    def delete_by_keys(
        self, keys: int | str | list, **kwargs: dict[str, typing.Any]
    ) -> typing.Any:
        """
    ...
    def delete(
        self, expr: str, **kwargs: dict[str, typing.Any]
    ) -> dict[str, typing.Any]:
        """
        ...
        **kwargs :  dict[str, typing.Any]
        ...
        result = self._client.delete_by_expr(
            collection_name=self._name, expression=expr, **kwargs
        )

        return self._update_delete_result_to_dict(result=result)

    def retrieve_by_keys(
        self, keys: int | str | list, **kwargs: dict[str, typing.Any]
    ) -> list[typing.Any]:
        """
    ...
    def _update_delete_result_to_dict(
...
class MilvusVectorDBService(VectorDBService):
    ...
    **kwargs : dict
    ...
    def load_resource(
        self, name: str, **kwargs: dict[str, typing.Any]
    ) -> MilvusVectorDBResourceService:
        return MilvusVectorDBResourceService(
            name=name,
            client=self._client,
    ...
    def create(
        self, name: str, overwrite: bool = False, **kwargs: dict[str, typing.Any]
    ):
        """
        ...
        **kwargs : dict
        ...
        """
        logger.debug(
            "Creating collection: %s, overwrite=%s, kwargs=%s", name, overwrite, kwargs
        )
        ...
        if not self.has_store_object(name) or overwrite:
            ...
            if len(schema_fields_conf) == 0:
                raise ValueError(
                    "Cannot create collection as provided empty schema_fields configuration"
                )

            schema_fields = [
                FieldSchemaEncoder.from_dict(field_conf)
                for field_conf in schema_fields_conf
            ]
    ...
    def _build_schema_conf(self, df: DataFrameType) -> list[dict]:
        ...
        for col_name, col_val in df.iloc[0].items():
            ...
            if (
                field_dict["dtype"] == pymilvus.DataType.FLOAT_VECTOR
            ...
            if field_dict["dtype"] == pymilvus.DataType.UNKNOWN:
                logger.warning(
                    "Could not infer data type for column '%s', with value: %s. Skipping column in schema.",
                    col_name,
                    col_val,
                )
                continue
    ...
    @with_collection_lock
    def insert(
        self, name: str, data: list[list] | list[dict], **kwargs: dict[str, typing.Any]
    ) -> dict[str, typing.Any]:
        """
    ...
    @with_collection_lock
    def insert_dataframe(
        self, name: str, df: DataFrameType, **kwargs: dict[str, typing.Any]
    ) -> dict[str, typing.Any]:
        """
    ...
    @with_collection_lock
    def query(
        self, name: str, query: str = None, **kwargs: dict[str, typing.Any]
    ) -> typing.Any:
        """
        Query data in a collection in the Milvus vector database.
    ...
    @with_collection_lock
    def update(
        self, name: str, data: list[typing.Any], **kwargs: dict[str, typing.Any]
    ) -> dict[str, typing.Any]:
        """
    ...
    @with_collection_lock
    def delete_by_keys(
        self, name: str, keys: int | str | list, **kwargs: dict[str, typing.Any]
    ) -> typing.Any:
        """
        Delete vectors by keys from the collection.
    ...
    @with_collection_lock
    def delete(
        self, name: str, expr: str, **kwargs: dict[str, typing.Any]
    ) -> dict[str, typing.Any]:
        """
        Delete vectors from the collection using expressions.
        ...
        if self.has_store_object(name):
            ...
            if resource == "collection":
                ...
                if "partition_name" not in kwargs:
                    raise ValueError(
                        "Mandatory argument 'partition_name' is required when resource='partition'"
                    )
                partition_name = kwargs["partition_name"]
                if self._client.has_partition(
                    collection_name=name, partition_name=partition_name
                ):
