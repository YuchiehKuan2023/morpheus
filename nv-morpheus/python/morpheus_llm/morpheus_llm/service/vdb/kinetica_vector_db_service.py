...
class KineticaVectorDBResourceService(VectorDBResourceService):
    ...
    def __init__(self, name: str, schema: str, client: "GPUdb") -> None:
        if IMPORT_EXCEPTION is not None:
            raise ImportError(
                IMPORT_ERROR_MESSAGE.format(package="gpudb")
        ...
        **kwargs : dict
        ...
        if query is None:
            raise GPUdbException(
                "'query' - a valid SQL query statement must be given ..."
            )

        logger.debug(
            "Searching in Kinetica table: %s, query=%s, kwargs=%s",
            self._name,
            query,
            kwargs,
        )
    ...
    def __query_collection(
        ...
        json_filter = (
            json.dumps(kinetica_filter) if kinetica_filter is not None else None
        )
        where_clause = (
    ...
    def similarity_search_by_vector(
        ...
        docs = self.similarity_search_with_score_by_vector(
            ...
            k=k,
            kinetica_filter=kinetica_filter,
        )
        return docs
    ...
    def similarity_search_with_score_by_vector(
        ...
        resp: dict = self.__query_collection(
            embedding, output_fields, k, kinetica_filter
        )
    ...
    async def similarity_search(
        ...
        **kwargs : dict[str, typing.Any]
        ...
        assert self._vector_field is not None, (
            "Cannot perform similarity search on a table without a vector field"
        )
        ...
        **kwargs : dict[str, typing.Any]
        ...
        if new_values_maps is not None:  # if given, remove from kwargs
            if not isinstance(new_values_maps, (list, dict)):
                raise GPUdbException(
                    "'new_value_maps' should either be a 'list of dicts' or a dict ..."
                )
        ...
        if len(expressions) != len(new_values_maps):
            raise GPUdbException(
                "'expression' and 'new_value_maps' must have the same number of elements"
            )
    ...
    def retrieve_by_keys(
        ...
        **kwargs :  dict[str, typing.Any]
        ...
        if keys is None:
            raise GPUdbException(
                "'keys' must be specified as either an 'int' "
        ...
        if expression == "":
            ...
            if pk_field_name == "":
                raise GPUdbException(
                    "No 'expression' given and no 'PK field' found cannot retrieve records ..."
                )
            ...
            if isinstance(keys, str):
                ...
                isinstance(keys, list) and is_list_of_type(keys, int) and len(keys) > 0
            ):
                # keys is a list of ints
                expression = f"{pk_field_name} in ({','.join(map(str, keys))})"
            elif (
                isinstance(keys, list) and is_list_of_type(keys, str) and len(keys) > 0
            ):
...
class KineticaVectorDBService(VectorDBService):
    ...
    def load_resource(
    ...
    ) -> KineticaVectorDBResourceService:
        """
        ...
        @return:
        """
        return KineticaVectorDBResourceService(
            name=name, schema=self._schema, client=self._client
        )
    ...
    def collection_name(self, name: str):
        ...
        name = (
            f"{self._schema}.{name}"
            if not _Utils.is_collection_name_fully_qualified(name)
            else name
        )
        return name
    ...
    def create(
        ...
        **kwargs : dict
        ...
        """
        logger.debug(
            "Creating Kinetica table: %s, overwrite=%s, kwargs=%s",
            name,
            overwrite,
            kwargs,
        )
        ...
        if not self.has_store_object(name) and (
            table_type is None or len(table_type) == 0
        ):
            raise GPUdbException(
                "Table must either be existing or a type must be given to create the table ..."
            )
