...
class NeMoLLMClient(LLMClient):
    ...
    def __init__(
        self, parent: "NeMoLLMService", *, model_name: str, **model_kwargs
    ) -> None:
        raise RuntimeError(
            "NeMoLLM is no longer supported, this class will be removed in future releases of Morpheus. "
    ...
    def generate(self, **input_dict) -> str:
        ...
        return self.generate_batch(
            {self._prompt_key: [input_dict[self._prompt_key]]}, return_exceptions=False
        )[0]
    ...
    async def generate_async(self, **input_dict) -> str:
        ...
        return (
            await self.generate_batch_async(
                {self._prompt_key: [input_dict[self._prompt_key]]},
    ...
    async def _process_one_async(self, prompt: str) -> str:
        ...
        while iterations < self._parent._retry_count:
            ...
            result: dict = nemollm.NemoLLM.post_process_generate_response(
                fut, return_text_completion_only=False
            )  # type: ignore
