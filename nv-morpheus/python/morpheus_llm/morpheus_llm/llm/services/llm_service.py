...
class LLMClient(ABC):
    ...
    @abstractmethod
    def generate_batch(
        self, inputs: dict[str, list], return_exceptions: typing.Literal[True]
    ) -> list[str | BaseException]: ...

    @typing.overload
    @abstractmethod
    def generate_batch(
        self, inputs: dict[str, list], return_exceptions: typing.Literal[False]
    ) -> list[str]: ...

    @typing.overload
    @abstractmethod
    def generate_batch(
        self, inputs: dict[str, list], return_exceptions: bool = False
    ) -> list[str] | list[str | BaseException]: ...

    @abstractmethod
    def generate_batch(
        self, inputs: dict[str, list], return_exceptions=False
    ) -> list[str] | list[str | BaseException]:
        """
...
class LLMService(ABC):
    ...
    @staticmethod
    def create(
        service_type: typing.Literal["nemo"], *service_args, **service_kwargs
    ) -> "NeMoLLMService":
        pass
    ...
    @staticmethod
    def create(
        service_type: typing.Literal["openai"], *service_args, **service_kwargs
    ) -> "OpenAIChatService":
        pass
    ...
    @staticmethod
    def create(
        service_type: str | typing.Literal["nemo"] | typing.Literal["openai"],
        *service_args,
        **service_kwargs,
    ):
        """
        ...
        module_name = (
            f"morpheus_llm.llm.services.{service_type.lower()}_{llm_or_chat}_service"
        )
        module = importlib.import_module(module_name)

        # Get all of the classes in the module to find the correct service class
        mod_classes = {
            name: cls for name, cls in module.__dict__.items() if callable(cls)
        }
        ...
        matching_classes = [
            name for name in mod_classes if name.lower() == class_name_lower
        ]

        assert len(matching_classes) == 1, (
            f"Expected to find exactly one class with name {class_name_lower} "
