...
class NVFoundationLLMClient(LLMClient):
    ...
    def __init__(
        self, parent: "NVFoundationLLMService", *, model_name: str, **model_kwargs
    ) -> None:
        if IMPORT_EXCEPTION is not None:
            raise ImportError(
                IMPORT_ERROR_MESSAGE.format(package="langchain-nvidia-ai-endpoints")
    ...
    async def generate_async(self, **input_dict) -> str:
        ...
        return (
            await self.generate_batch_async(
                inputs=inputs, **input_dict, return_exceptions=False
            )
        )[0]
    ...
    def generate_batch(
        self, inputs: dict[str, list], return_exceptions=False, **kwargs
    ) -> list[str] | list[str | BaseException]:
        """
        ...
        **kwargs
        ...
        if return_exceptions:
            warnings.warn(
                "return_exceptions==True is not currently supported by the "
        ...
        responses = []
        try:
            generated_responses = self._client.generate_prompt(
                prompts=prompts, **final_kwargs
            )  # type: ignore
    ...
    async def generate_batch_async(
        ...
        **kwargs
        ...
        responses = []
        try:
            generated_responses = await self._client.agenerate_prompt(
                prompts=prompts, **final_kwargs
            )  # type: ignore
            responses = [g[0].text for g in generated_responses.generations]
        except Exception as e:
