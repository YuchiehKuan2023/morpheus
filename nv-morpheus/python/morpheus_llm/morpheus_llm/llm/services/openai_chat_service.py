...
class OpenAIChatClient(LLMClient):
    ...
    def __init__(
        ...
        **model_kwargs,
        ...
        if IMPORT_EXCEPTION is not None:
            raise ImportError(
                IMPORT_ERROR_MESSAGE.format(package="openai")
    ...
    def _create_messages(
        self, prompt: str, assistant: str = None
    ...
    def _extract_completion(
        self, completion: "openai.types.chat.chat_completion.ChatCompletion"
    ) -> str:
    ...
    def _generate(self, prompt: str, assistant: str = None) -> str:
        ...
        output: openai.types.chat.chat_completion.ChatCompletion = (
    ...
    async def _generate_async(self, prompt: str, assistant: str = None) -> str:
        ...
        with self._api_logger(inputs=messages) as msg_logger:
            try:
                output = await self._client_async.chat.completions.create(
                    model=self._model_name, messages=messages, **self._model_kwargs
                )
    ...
    async def generate_async(self, **input_dict) -> str:
        ...
        return await self._generate_async(
            input_dict[self._prompt_key], input_dict.get(self._assistant_key, None)
        )
    ...
    def __init__(
        ...
        *,
        ...
        log_file = os.path.join(
            appdirs.user_log_dir(appauthor="NVIDIA", appname="morpheus"), "openai.log"
        )
