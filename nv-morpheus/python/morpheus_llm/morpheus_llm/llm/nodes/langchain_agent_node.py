...
class LangChainAgentNode(LLMNodeBase):
    ...
    def __init__(
        ...
        if IMPORT_EXCEPTION is not None:
            raise ImportError(
                IMPORT_ERROR_MESSAGE.format("langchain_core")
    ...
    @staticmethod
    def _transform_dict_of_lists(
        data: dict[str, typing.Any],
    ) -> list[dict[str, typing.Any]]:
        return [dict(zip(data, t)) for t in zip(*data.values())]

    async def _run_single(
        self, metadata: dict[str, typing.Any] = None, **kwargs
    ) -> dict[str, typing.Any]:
        ...
        if all_lists:
            ...
            if (
                ...
                and self._is_all_lists(metadata)
            ...
            # Run multiple again
            results_async = [
                self._run_single(metadata=metadata_list[i], **x)
