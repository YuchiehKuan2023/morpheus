...
class RetrieverNode(LLMNodeBase):
    ...
    def __init__(
        self,
        *,
        embedding: typing.Callable[
            [list[str]], typing.Coroutine[typing.Any, typing.Any, list[list[float]]]
        ]
        | None,
        service: VectorDBResourceService,
    ...
    async def execute(self, context: LLMContext):  # pylint: disable=invalid-overridden-method
        ...
        if self._embedding is not None:
            ...
            embeddings: list[list[float]] = typing.cast(
                list[list[float]], context.get_input()
            )

        # Query the vector database
        results = await self._service.similarity_search(
            embeddings=embeddings, **self._similarity_search_kwargs
        )
