...
class PromptTemplateNode(LLMNodeBase):
    ...
    def __init__(
        self,
        template: str,
        template_format: typing.Literal["f-string", "jinja"] = "f-string",
    ) -> None:
        super().__init__()
        ...
        if self._template_format == "f-string":
            ...
            self._input_names = list(
                meta.find_undeclared_variables(
                    self._template_jinja.environment.parse(self._template_str)
                )
            )
        else:
            raise ValueError(
                f"Invalid template format: {self._template_format}, must be one of: f-string, jinja"
            )
    ...
    async def execute(self, context: LLMContext):  # pylint: disable=invalid-overridden-method
        ...
        if self._template_format == "f-string":
            ...
            render_coros = [
                self._template_jinja.render_async(**inputs) for inputs in input_list
            ]
