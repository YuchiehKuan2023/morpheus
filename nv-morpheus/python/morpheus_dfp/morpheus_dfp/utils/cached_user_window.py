...
class CachedUserWindow:
    ...
    _trained_rows: pd.Series = dataclasses.field(
        init=False, repr=False, default_factory=pd.DataFrame
    )
    _df: pd.DataFrame = dataclasses.field(
        init=False, repr=False, default_factory=pd.DataFrame
    )

    def append_dataframe(self, incoming_df: pd.DataFrame) -> bool:
        # Filter the incoming df by epochs later than the current max_epoch
        filtered_df = incoming_df[
            incoming_df[self.timestamp_column] > np.datetime64(self.max_epoch)
        ]

        if len(filtered_df) == 0:
            # We have nothing new to add. Double check that we fit within the window
            before_history = incoming_df[
                incoming_df[self.timestamp_column] < np.datetime64(self.min_epoch)
            ]
