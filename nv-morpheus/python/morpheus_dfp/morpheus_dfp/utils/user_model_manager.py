...
class DFPDataLoader:
    ...
    def get_next_frame(self):
        ...
        while True:
            df_frame = self._filter_func(
                pd.read_pickle(self._batch_frames[self._current_index])
            )
...
class UserModelManager:
        ...
        try:
            for _ in tqdm(range(self._epochs), desc="Training"):
                ...
                while True:
                    ...
                    if (
                        df_batch.shape[0] < 10
                    ):  # If we've already trained on some data, make sure we can tts this.
                        break
            ...
            logger.exception(
                "Error during training for user: %s", self._user_id, exc_info=True
            )
            return None, None
    ...
    def train(self, df: pd.DataFrame) -> AutoEncoder:
        ...
        if self._max_history > 0:
            self._history = combined_df.iloc[
                max(0, len(combined_df) - self._max_history) :, :
            ]
        else:
