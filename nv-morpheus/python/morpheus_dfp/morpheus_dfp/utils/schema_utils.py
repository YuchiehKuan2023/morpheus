...
class SchemaBuilder:
    ...
    def _build_azure_schema(self) -> Schema:
        ...
        source_column_info = [
            DateTimeColumn(
                name=self._config.ae.timestamp_column_name,
                dtype="datetime64[ns]",
                input_name="time",
            ),
            RenameColumn(
                name=self._config.ae.userid_column_name,
                dtype=str,
        ...
        preprocess_column_info = [
            ColumnInfo(
                name=self._config.ae.timestamp_column_name, dtype="datetime64[ns]"
            ),
            ...
            DistinctIncrementColumn(
                name="locincrement", dtype=int, input_name="location"
            ),
            DistinctIncrementColumn(
                name="appincrement", dtype=int, input_name="appDisplayName"
            ),
        ]

        preprocess_schema = DataFrameInputSchema(
            column_info=preprocess_column_info,
    ...
    def _build_duo_schema(self) -> Schema:
        ...
        source_column_info = [
            ...
            RenameColumn(
                name="accessdevicebrowser",
                dtype=str,
                input_name="access_device.browser",
            ),
            RenameColumn(
                name="accessdeviceos", dtype=str, input_name="access_device.os"
            ),
            StringCatColumn(
                name="location",
                dtype=str,
            ...
            RenameColumn(
                name="authdevicename", dtype=str, input_name="auth_device.name"
            ),
            BoolColumn(
                name="result",
                dtype=bool,
