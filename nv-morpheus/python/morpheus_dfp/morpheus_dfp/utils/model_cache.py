...
class ModelCache:
    def __init__(
        self, reg_model_name: str, reg_model_version: str, model_uri: str
    ) -> None:
        ...
        with self._lock:
            if self._model is None:
                ...
                try:
                    ...
                    logger.error(
                        "Error downloading model for URI: %s",
                        self._model_uri,
                        exc_info=True,
                    )
                    raise
...
class UserModelMap:
    def __init__(
        self, manager: "ModelManager", user_id: str, fallback_user_ids: typing.List[str]
    ):
        ...
        try:
            with timed_acquire(self._lock, timeout=timeout):
                ...
                if (
                    ...
                    or (now - self._last_checked).seconds
                    < self._manager.cache_timeout_sec
                ):
                    # Save the last checked time
                    self._last_checked = now

                    # Try to load from the manager
                    model_cache = self._manager.load_model_cache(
                        client=client,
                    ...
                    if model_cache is None and len(self._fallback_user_ids) > 0:
                        ...
                        self._child_user_model_cache = (
                            self._manager.load_user_model_cache(
                                self._fallback_user_ids[0],
                                timeout,
                                fallback_user_ids=self._fallback_user_ids[1:],
                            )
                        )
                    else:
                        return model_cache
                ...
                if self._child_user_model_cache is not None:
                    return self._child_user_model_cache.load_model_cache(
                        client=client, timeout=timeout
                    )

                # Otherwise load the model
                model_cache = self._manager.load_model_cache(
                    client=client, reg_model_name=self._reg_model_name, timeout=timeout
                )

                if model_cache is None:
                    raise RuntimeError(
                        f"Model was found but now no longer exists. Model: {self._reg_model_name}"
                    )

                return model_cache
        except TimeoutError as e:
            logger.error(
                "Deadlock detected while loading model cache. Please report this to the developers."
            )
...
class ModelManager:
    ...
    def _model_exists(self, reg_model_name: str, timeout: float = 1.0) -> bool:
        ...
        if (now - self._existing_models_updated).seconds > self._cache_timeout_sec:
            try:
                with timed_acquire(self._model_cache_lock, timeout=timeout):
                    ...
                    while (
                        results := client.search_registered_models(
                            max_results=1000, page_token=results.token
                        )
                    ) is not None:
                ...
                logger.error(
                    "Deadlock detected checking for new models. Please report this to the developers."
                )
    ...
    def user_id_to_model(self, user_id: str):
        return user_to_model_name(
            user_id=user_id, model_name_formatter=self._model_name_formatter
        )
    ...
    def load_user_model(
        ...
        user_model_cache = self.load_user_model_cache(
            user_id=user_id, timeout=timeout, fallback_user_ids=fallback_user_ids
        )

        return user_model_cache.load_model_cache(client=client, timeout=timeout)

    def load_model_cache(
        self, client: MlflowClient, reg_model_name: str, timeout: float = 1.0
    ) -> ModelCache:
        now = datetime.now()
        ...
        try:
            with timed_acquire(self._model_cache_lock, timeout=timeout):
                ...
                if (
                    ...
                    and (now - model_cache.last_checked).seconds
                    < self._cache_timeout_sec
                ):
                    return model_cache
                ...
                try:
                    ...
                    if len(latest_versions) == 0:
                        ...
                        latest_versions = (
                            None
                            if reg_model_obj is None
                            else reg_model_obj.latest_versions
                        )
                    ...
                    if len(latest_versions) > 1:
                        logger.warning(
                            (
                                "Multiple models in different stages detected. "
                ...
                if len(self._model_cache) > self._model_cache_size_max:
                    time_sorted = sorted(
                        list(self._model_cache.items()), key=lambda x: x[1].last_used
                    )
            ...
            raise RuntimeError(
                "Deadlock when trying to acquire model cache lock"
            ) from e

    def load_user_model_cache(
        self, user_id: str, timeout: float, fallback_user_ids: typing.List[str]
    ) -> UserModelMap:
        try:
            with timed_acquire(self._user_model_cache_lock, timeout=timeout):
                if user_id not in self._user_model_cache:
                    self._user_model_cache[user_id] = UserModelMap(
                        manager=self,
                        user_id=user_id,
            ...
            logger.error(
                "Deadlock when trying to acquire user model cache lock", exc_info=True
            )
            raise RuntimeError(
                "Deadlock when trying to acquire user model cache lock"
            ) from e
