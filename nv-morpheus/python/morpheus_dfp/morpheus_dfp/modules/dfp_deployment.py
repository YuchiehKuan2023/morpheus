...
def dfp_deployment(builder: mrc.Builder):
    ...
    fsspec_loader_defaults = {
        "loaders": [{"id": FSSPEC_LOADER}],
    }

    fsspec_dataloader_conf = merge_dictionaries(
        supported_loaders, fsspec_loader_defaults
    )

    dfp_training_pipe_conf = module_config["training_options"]
    dfp_inference_pipe_conf = module_config["inference_options"]

    fsspec_dataloader_module = builder.load_module(
        DATA_LOADER, "morpheus", "fsspec_dataloader", fsspec_dataloader_conf
    )
    dfp_training_pipe_module = builder.load_module(
        DFP_TRAINING_PIPE, "morpheus", "dfp_training_pipe", dfp_training_pipe_conf
    )
    dfp_inference_pipe_module = builder.load_module(
        DFP_INFERENCE_PIPE, "morpheus", "dfp_inference_pipe", dfp_inference_pipe_conf
    )
    ...
    router = Router(
        builder, "router", router_keys=["training", "inference"], key_fn=router_key_fn
    )

    # Make an edge between modules
    builder.make_edge(fsspec_dataloader_module.output_port("output"), router)
    builder.make_edge(
        router.get_source("training"), dfp_training_pipe_module.input_port("input")
    )
    builder.make_edge(
        router.get_source("inference"), dfp_inference_pipe_module.input_port("input")
    )

    out_nodes = [
        dfp_training_pipe_module.output_port("output"),
