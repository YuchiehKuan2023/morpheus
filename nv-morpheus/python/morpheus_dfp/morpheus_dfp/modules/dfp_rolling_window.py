...
def dfp_rolling_window(builder: mrc.Builder):
    ...
    def get_user_cache(user_id: str):
        ...
        cache_location = (
            os.path.join(cache_dir, f"{user_id}.pkl") if cache_to_disk else None
        )
    ...
    def try_build_window(
        message: MessageMeta, user_id: str
    ) -> typing.Union[MessageMeta, None]:
        with get_user_cache(user_id) as user_cache:
            ...
            if not user_cache.append_dataframe(incoming_df=incoming_df):
                ...
                logger.warning(
                    (
                        "Incoming data preceeded existing history. "
            ...
            if cache_to_disk and cache_dir is not None:
                logger.debug(
                    "Saved rolling window cache for %s == %d items",
                    user_id,
                    user_cache.total_count,
                )
                user_cache.save()
            ...
            if cache_mode == "batch":
                ...
                incoming_hash = pd.util.hash_pandas_object(
                    incoming_df.iloc[[0, -1]], index=False
                )
                ...
                last_row_idx = (
                    df_window[df_window["_row_hash"] == incoming_hash.iloc[-1]]
                    .index[-1]
                    .item()
                )
                ...
                if found_count != len(incoming_df):
                    raise RuntimeError(
                        (
                            "Overlapping rolling history detected. "
    ...
    def on_data(control_message: ControlMessage):
        try:
            ...
            logger.error(
                "Error processing control message in rolling window: %s\nDiscarding control message.",
                exec_info,
            )
            return None
