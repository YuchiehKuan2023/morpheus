...
def dfp_inference(builder: mrc.Builder):
    ...
    if "model_name_formatter" not in config:
        raise ValueError(
            "Inference module requires model_name_formatter to be configured"
        )
    ...
    def get_model(user: str) -> ModelCache:
        ...
        return model_manager.load_user_model(
            client,
            user_id=user,
    ...
    def process_task(control_message: ControlMessage) -> ControlMessage:
        ...
        # TODO(Devin): Recovery strategy should be more robust/configurable in practice
            ...
            logger.exception(
                "Error retrieving model for user %s, discarding training message. %s",
                user_id,
                exec_info,
            )
            return None
        ...
        results_df = cudf.from_pandas(
            loaded_model.get_results(df_user, return_abs=True)
        )
