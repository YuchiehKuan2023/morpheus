...
def dfp_training_pipe(builder: mrc.Builder):
    ...
    mlflow_writer_options = config.get(
        "mlflow_writer_options",
        {
            "timestamp_column_name": ts_column_name,
        },
    ...
    training_monitor_module_conf = merge_dictionaries(
        training_monitor_options, monitor_options
    )

    mlflow_model_writer_defaults = {}
    mlflow_model_writer_conf = merge_dictionaries(
        mlflow_writer_options, mlflow_model_writer_defaults
    )

    mlflow_model_writer_monitor_options = {
        "description": "Train & Saved [training_pipe]"
    }
    mlflow_model_writer_module_conf = merge_dictionaries(
        mlflow_model_writer_monitor_options, monitor_options
    )
    ...
    dfp_data_prep_loader = MonitorLoaderFactory.get_instance(
        "data_prep_monitor", module_config=data_prep_monitor_module_conf
    )
    dfp_data_prep_monitor_module = dfp_data_prep_loader.load(builder=builder)

    dfp_training_module = builder.load_module(
        DFP_TRAINING, "morpheus", "dfp_training", dfp_training_conf
    )

    dfp_training_monitor_loader = MonitorLoaderFactory.get_instance(
        "training_monitor", module_config=training_monitor_module_conf
    )
    dfp_training_monitor_module = dfp_training_monitor_loader.load(builder=builder)

    mlflow_model_writer_module = builder.load_module(
        MLFLOW_MODEL_WRITER, "morpheus", "mlflow_model_writer", mlflow_model_writer_conf
    )

    mlflow_model_writer_loader = MonitorLoaderFactory.get_instance(
        "mlflow_model_writer_monitor", module_config=mlflow_model_writer_module_conf
    )
    mlflow_model_writer_monitor_module = mlflow_model_writer_loader.load(
        builder=builder
    )
    ...
    builder.make_edge(
        ...
        dfp_training_module.input_port("input"),
    )
    builder.make_edge(
        dfp_training_module.output_port("output"),
        dfp_training_monitor_module.input_port("input"),
    )
    builder.make_edge(
        dfp_training_monitor_module.output_port("output"),
        mlflow_model_writer_module.input_port("input"),
    )
    builder.make_edge(
        mlflow_model_writer_module.output_port("output"),
    ...
    builder.register_module_output(
        "output", mlflow_model_writer_monitor_module.output_port("output")
    )
