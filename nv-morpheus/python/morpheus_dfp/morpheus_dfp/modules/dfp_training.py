...
def dfp_training(builder: mrc.Builder):
    ...
    if validation_size < 0.0 or validation_size > 1.0:
        raise ValueError(
            f"validation_size={validation_size} should be a positive float in the "
            "(0, 1) range"
        )
        ...
        while control_message.has_task("training"):
            ...
            if validation_size > 0.0:
                train_df, validation_df = train_test_split(
                    train_df, test_size=validation_size, shuffle=False
                )
                run_validation = True
    ...
    def node_fn(obs: mrc.Observable, sub: mrc.Subscriber):
        obs.pipe(
            ops.map(on_data), ops.flatten(), ops.filter(lambda x: x is not None)
        ).subscribe(sub)
