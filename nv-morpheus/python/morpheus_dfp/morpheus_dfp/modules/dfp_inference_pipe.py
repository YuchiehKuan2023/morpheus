...
def dfp_inference_pipe(builder: mrc.Builder):
    ...
    stream_aggregation_options = merge_dictionaries(
        stream_aggregation_options,
        {
            "cache_dir": cache_dir,
            "timestamp_column_name": ts_column_name,
    ...
    data_prep_options = merge_dictionaries(
        data_prep_options,
        {
            "timestamp_column_name": ts_column_name,
        },
    ...
    dfp_rolling_window_conf = merge_dictionaries(
        stream_aggregation_options, stream_aggregation_defaults
    )
    ...
    data_prep_monitor_module_conf = merge_dictionaries(
        data_prep_monitor_options, monitor_options
    )

    inference_model_defaults = {}  # placeholder for future defaults
    dfp_inference_conf = merge_dictionaries(
        inference_model_options, inference_model_defaults
    )

    inference_monitor_options = {"description": "Inferenced [inference_pipe]"}
    inference_monitor_module_conf = merge_dictionaries(
        inference_monitor_options, monitor_options
    )
    ...
    filter_detections_conf = merge_dictionaries(
        detection_criteria, detection_criteria_defaults
    )

    post_processing_defaults = {}  # placeholder for future defaults
    dfp_post_proc_conf = merge_dictionaries(
        post_processing_options, post_processing_defaults
    )
    ...
    write_to_file_conf = merge_dictionaries(
        write_to_file_options, write_to_file_defaults
    )

    write_to_file_monitor_options = {"description": "Saved [inference_pipe]"}
    write_to_fm_conf = merge_dictionaries(
        write_to_file_monitor_options, monitor_options
    )

    # Load modules
    preproc_module = builder.load_module(
        DFP_PREPROC, "morpheus", "dfp_preproc", preproc_conf
    )
    dfp_rolling_window_module = builder.load_module(
        DFP_ROLLING_WINDOW, "morpheus", "dfp_rolling_window", dfp_rolling_window_conf
    )
    dfp_data_prep_module = builder.load_module(
        DFP_DATA_PREP, "morpheus", "dfp_data_prep", dfp_data_prep_conf
    )

    dfp_data_prep_loader = MonitorLoaderFactory.get_instance(
        "dfp_inference_data_prep_monitor", module_config=data_prep_monitor_module_conf
    )

    dfp_data_prep_monitor_module = dfp_data_prep_loader.load(builder=builder)
    dfp_inference_module = builder.load_module(
        DFP_INFERENCE, "morpheus", "dfp_inference", dfp_inference_conf
    )

    dfp_inference_monitor_loader = MonitorLoaderFactory.get_instance(
        "dfp_inference_monitor", module_config=inference_monitor_module_conf
    )
    dfp_inference_monitor_module = dfp_inference_monitor_loader.load(builder=builder)
    filter_detections_module = builder.load_module(
        FILTER_DETECTIONS, "morpheus", "filter_detections", filter_detections_conf
    )
    dfp_post_proc_module = builder.load_module(
        DFP_POST_PROCESSING, "morpheus", "dfp_post_processing", dfp_post_proc_conf
    )
    serialize_module = builder.load_module(
        SERIALIZE, "morpheus", "serialize", serialize_conf
    )
    write_to_file_module = builder.load_module(
        WRITE_TO_FILE, "morpheus", "write_to_file", write_to_file_conf
    )

    dfp_write_to_file_monitor_loader = MonitorLoaderFactory.get_instance(
        "dfp_inference_write_to_file_monitor", module_config=write_to_fm_conf
    )
    dfp_write_to_file_monitor_module = dfp_write_to_file_monitor_loader.load(
        builder=builder
    )

    # Make an edge between the modules.
    builder.make_edge(
        preproc_module.output_port("output"),
        dfp_rolling_window_module.input_port("input"),
    )
    builder.make_edge(
        dfp_rolling_window_module.output_port("output"),
        dfp_data_prep_module.input_port("input"),
    )
    builder.make_edge(
        dfp_data_prep_module.output_port("output"),
        dfp_data_prep_monitor_module.input_port("input"),
    )
    builder.make_edge(
        dfp_data_prep_monitor_module.output_port("output"),
        dfp_inference_module.input_port("input"),
    )
    builder.make_edge(
        dfp_inference_module.output_port("output"),
        dfp_inference_monitor_module.input_port("input"),
    )
    builder.make_edge(
        dfp_inference_monitor_module.output_port("output"),
        filter_detections_module.input_port("input"),
    )
    builder.make_edge(
        filter_detections_module.output_port("output"),
        dfp_post_proc_module.input_port("input"),
    )
    builder.make_edge(
        dfp_post_proc_module.output_port("output"), serialize_module.input_port("input")
    )
    builder.make_edge(
        serialize_module.output_port("output"), write_to_file_module.input_port("input")
    )
    builder.make_edge(
        write_to_file_module.output_port("output"),
        dfp_write_to_file_monitor_module.input_port("input"),
    )

    # Register input and output port for a module.
    builder.register_module_input("input", preproc_module.input_port("input"))
    builder.register_module_output(
        "output", dfp_write_to_file_monitor_module.output_port("output")
    )
