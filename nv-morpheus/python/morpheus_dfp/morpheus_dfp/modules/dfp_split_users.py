    ...
    def generate_split_dataframes(users_df: pd.DataFrame):
        ...
        if include_individual:
            ...
            split_dataframes.update(
                {
                    username: user_df
                    for username, user_df in users_df.groupby(
                        userid_column_name, sort=False
                    )
                }
            )
        ...
        try:
            ...
            with message_meta.mutable_dataframe() as dfm:
                with log_time(logger.debug):
                    ...
                    control_messages = generate_control_messages(
                        control_message, split_dataframes
                    )

            return control_messages
        except Exception as exec_info:
            logger.exception(
                "Error extracting users from message, discarding control message: %s",
                exec_info,
            )
            return []
