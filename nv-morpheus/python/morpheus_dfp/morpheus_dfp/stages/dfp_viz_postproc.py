...
class DFPVizPostprocStage(PassThruTypeMixin, SinglePortStage):
    ...
    def _postprocess(self, msg: ControlMessage) -> pd.DataFrame:
        ...
        viz_pdf[["user", "time"]] = pdf[
            [self._user_column_name, self._timestamp_column]
        ]
    ...
    def _write_to_files(self, msg: ControlMessage):
        ...
        for period in unique_periods:
            ...
            output_file = os.path.join(
                self._output_dir, self._output_prefix + str(period) + ".csv"
            )
            ...
            lines = serializers.df_to_csv(
                period_df, include_header=is_first, include_index_col=False
            )
    ...
    def _build_single(
        ...
        dfp_viz_postproc = builder.make_node(
            self.unique_name, ops.map(self._write_to_files)
        )
