...
class DFPFileBatcherStage(SinglePortStage):
    ...
    def on_data(
        self, file_objects: fsspec.core.OpenFiles
    ) -> typing.List[typing.Tuple[fsspec.core.OpenFiles, int]]:
        """
        ...
        for file_object in file_objects:
            ...
            if (self._start_time is not None and ts < self._start_time) or (
                self._end_time is not None and ts > self._end_time
            ):
                continue
        ...
        df = pd.DataFrame(
            ...
            data={"filename": full_names, "objects": file_objs},
        )
        ...
        for _, period_df in resampled:
            ...
            obj_list = fsspec.core.OpenFiles(
                file_list, mode=file_objects.mode, fs=file_objects.fs
            )
