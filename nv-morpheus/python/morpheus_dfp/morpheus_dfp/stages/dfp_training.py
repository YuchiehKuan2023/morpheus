...
class DFPTraining(SinglePortStage):
    ...
    def __init__(
        self, c: Config, model_kwargs: dict = None, epochs=30, validation_size=0.0
    ):
        super().__init__(c)
        ...
        if 0.0 <= validation_size < 1.0:
            ...
            raise ValueError(
                f"validation_size={validation_size} should be a positive float in the (0, 1) range"
            )
    ...
    def on_data(self, message: ControlMessage) -> ControlMessage:
        ...
        train_df = train_df[
            train_df.columns.intersection(self._config.ae.feature_columns)
        ]
        validation_df = None
        ...
        if self._validation_size > 0.0:
            train_df, validation_df = train_test_split(
                train_df, test_size=self._validation_size, shuffle=False
            )
            run_validation = True
