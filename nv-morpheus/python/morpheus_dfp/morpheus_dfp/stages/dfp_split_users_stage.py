...
class DFPSplitUsersStage(SinglePortStage):
    ...
    def extract_users(self, message: DataFrameType) -> list[ControlMessage]:
        ...
        with log_time(logger.debug) as log_info:
            ...
            if len(self._skip_users) > 0:
                message = message[
                    ~message[self._config.ae.userid_column_name].isin(self._skip_users)
                ]

            if len(self._only_users) > 0:
                message = message[
                    message[self._config.ae.userid_column_name].isin(self._only_users)
                ]
            ...
            if self._include_individual:
                ...
                split_dataframes.update(
                    {
                        username: user_df
                        for username, user_df in message.groupby(
                            self._config.ae.userid_column_name, sort=False
                        )
                    }
                )
            ...
            for user_id in sorted(split_dataframes.keys()):
                ...
                user_df.index = range(
                    current_user_count, current_user_count + len(user_df)
                )
    ...
    def _build_single(
        ...
        node = builder.make_node(
            self.unique_name, ops.map(self.extract_users), ops.flatten()
        )
