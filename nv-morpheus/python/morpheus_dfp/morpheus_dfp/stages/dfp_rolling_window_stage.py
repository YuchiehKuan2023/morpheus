...
class DFPRollingWindowStage(SinglePortStage):
    ...
    def _get_user_cache(
        self, user_id: str
    ) -> typing.Generator[CachedUserWindow, None, None]:
    ...
    def _build_window(self, message: ControlMessage) -> ControlMessage:
        ...
        with self._get_user_cache(user_id) as user_cache:
            ...
            if (
                user_cache.total_count - user_cache.last_train_count
                < self._min_increment
            ):
                return None
            ...
            last_row_idx = train_df[
                train_df["_row_hash"] == incoming_hash.iloc[-1]
            ].index[-1]
        ...
        with log_time(logger.debug) as log_info:
            ...
            if result is not None:
                log_info.set_log(
                    ...
                    result.payload()
                    .get_data(self._config.ae.timestamp_column_name)
                    .min(),
                    result.payload()
                    .get_data(self._config.ae.timestamp_column_name)
                    .max(),
    ...
    def _build_single(
        ...
        node = builder.make_node(
            self.unique_name, ops.map(self.on_data), ops.filter(lambda x: x is not None)
        )
