...
class LinearPipeline(_pipeline.Pipeline):
    ...
    def add_stage(self, stage: SinglePortStageT) -> SinglePortStageT:  # pylint:disable=arguments-differ
        ...
        assert len(self._linear_stages) > 0, (
            "A source must be set on a LinearPipeline before adding any stages"
        )
        assert isinstance(stage, _pipeline.SinglePortStage), (
            "Only `SinglePortStage` stages are accepted in `add_stage()`"
        )
    ...
    def add_segment_boundary(self, data_type=None, as_shared_pointer=False):
        ...
        from morpheus.stages.boundary.linear_boundary_stage import (
            LinearBoundaryEgressStage,
        )
        from morpheus.stages.boundary.linear_boundary_stage import (
        ...
        if len(self._linear_stages) == 0:
            raise RuntimeError(
                "Cannot create a segment boundary, current segment is empty."
            )

        empty_config = Config()
        empty_config.execution_mode = self._execution_mode
        boundary_egress = LinearBoundaryEgressStage(
            empty_config, boundary_port_id=self._current_segment_id, data_type=data_type
        )
        boundary_ingress = LinearBoundaryIngressStage(
            empty_config, boundary_port_id=self._current_segment_id, data_type=data_type
        )
        ...
        port_id_tuple = (
            (self._current_segment_id, object, False)
            if data_type
            else self._current_segment_id
        )
