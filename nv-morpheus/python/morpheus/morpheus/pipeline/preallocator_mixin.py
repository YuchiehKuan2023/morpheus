...
class PreallocatorMixin(ABC):
    ...
    def _preallocate_df(self, df: DataFrameType) -> DataFrameType:
        missing_columns = [
            col for col in self._needed_columns.keys() if col not in df.columns
        ]
        ...
        if len(self._needed_columns) > 0:
            ...
            if issubclass(out_type, (ControlMessage, MessageMeta)):
                ...
                if (
                    ...
                    and self._all_types_supported_in_cpp()
                ):
                    import morpheus._lib.stages as _stages

                    needed_columns = list(self._needed_columns.items())
                    if issubclass(out_type, ControlMessage):
                        node = _stages.PreallocateControlMessageStage(
                            builder, node_name, needed_columns
                        )
                    elif issubclass(out_type, MessageMeta):
                        node = _stages.PreallocateMessageMetaStage(
                            builder, node_name, needed_columns
                        )
                    else:
                    ...
                    if issubclass(out_type, ControlMessage):
                        node = builder.make_node(
                            node_name, ops.map(self._preallocate_control)
                        )
                    elif issubclass(out_type, MessageMeta):
                        node = builder.make_node(
                            node_name, ops.map(self._preallocate_meta)
                        )
                    else:
                        raise RuntimeError(f"Unsupported output type {pretty_type}")
                ...
                if issubclass(out_type, tuple(supported_df_types)):
                    ...
                    msg = (
                        "Additional columns were requested to be inserted into the Dataframe, but the output type "
