...
def _validate_keyword_arguments(
    ...
    for param in param_iter:
        ...
        if (
            ...
            raise ValueError(
                f"Wrapped function {fn_name} has keyword only parameter '{param.name}' that was not "
                "provided a value"
            )

        if param.kind is param.POSITIONAL_ONLY:
            raise ValueError(
                "Positional arguments are not supported for wrapped functions. "
...
def _fn_receives_subscription(signature: inspect.Signature) -> bool:
    ...
    try:
        ...
        if (
            first_param.annotation is mrc.Subscription
            or first_param.annotation == "mrc.Subscription"
...
class PreAllocatedWrappedFunctionStage(
    _pipeline.PreallocatorMixin, WrappedFunctionSourceStage
):
    """
    Source stage that wraps a generator function as the method for generating messages.
...
def source(
    ...
    *,
    ...
    @functools.wraps(
        gen_fn, assigned=("__module__", "__name__", "__qualname__", "__doc__")
    )
    def wrapper(config: Config, **kwargs: _P.kwargs) -> WrappedFunctionSourceStage:
        ...
        if return_type in pre_allocation_output_types:
            return PreAllocatedWrappedFunctionStage(
                config=config,
                name=name,
                gen_fn=bound_gen_fn,
                compute_schema_fn=compute_schema_fn,
        ...
        return WrappedFunctionSourceStage(
            config=config,
            name=name,
            gen_fn=bound_gen_fn,
            compute_schema_fn=compute_schema_fn,
...
DecoratedStageType = typing.Callable[
    typing.Concatenate[Config, _P], WrappedFunctionStage
]
...
def stage(
    ...
    *,
    ...
    # annotations to reflect that the returned function requires a config and returns a stage
    @functools.wraps(
        on_data_fn, assigned=("__module__", "__name__", "__qualname__", "__doc__")
    )
    def wrapper(config: Config, **kwargs: _P.kwargs) -> WrappedFunctionStage:
        ...
        try:
            ...
            if accept_type is signature.empty:
                raise ValueError(
                    f"{first_param.name} argument of {name} has no type annotation"
                )
        except StopIteration as e:
            raise ValueError(
                f"Stage function {name} must have at least one parameter"
            ) from e
