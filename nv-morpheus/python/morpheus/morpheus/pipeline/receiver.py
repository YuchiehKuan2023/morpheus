...
class Receiver:
        ...
        if self._input_node is None:
            ...
            if len(self._input_senders) == 1:
                ...
                self._input_node = builder.make_node_component(
                    f"{self.parent.unique_name}-reciever[{self.port_number}]",
                    mrc.core.operators.map(lambda x: x),
                )
    ...
    def _compute_input_schema(self):
        ...
        if great_ancestor is None:
            raise RuntimeError(
                (
                    f"Cannot determine single type for senders of input port for {self._parent}. "
