...
class SinglePortStage(_pipeline.Stage):
    ...
    def _pre_compute_schema(self, schema: _pipeline.StageSchema):
        ...
        if not typing_utils.issubtype(input_type, accepted_types):
            raise RuntimeError(
                (
                    f"The {self.name} stage cannot handle input of {input_type}. "
    ...
    def _build(
        self, builder: mrc.Builder, input_nodes: list[mrc.SegmentObject]
    ...
    def _post_build_single(
        self, _: mrc.Builder, out_node: mrc.SegmentObject
    ) -> mrc.SegmentObject:
        return out_node

    def _post_build(
        self, builder: mrc.Builder, out_ports_nodes: list[mrc.SegmentObject]
    ) -> list[mrc.SegmentObject]:
