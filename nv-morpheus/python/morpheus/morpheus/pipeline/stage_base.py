...
class StageBase(ABC, collections.abc.Hashable):
    ...
    def _pre_build(self, do_propagate: bool = True):
        ...
        if self._config.execution_mode not in self.supported_execution_modes():
            supported_modes = ", ".join(
                str(x) for x in self.supported_execution_modes()
            )
            raise RuntimeError(
                f"Unsupported execution mode {self._config.execution_mode} for stage {self.name}, "
    ...
    def build(self, builder: mrc.Builder, do_propagate: bool = True):
        ...
        assert self._pipeline is not None, (
            "Must be attached to a pipeline before building!"
        )
        ...
        out_ports_nodes = self._post_build(
            builder=builder, out_ports_nodes=out_ports_nodes
        )
    ...
    def _create_ports(self, input_count: int, output_count: int):
        assert len(self._input_ports) == 0 and len(self._output_ports) == 0, (
            "Can only create ports once!"
        )

        self._input_ports = [
            _pipeline.Receiver(parent=self, port_number=i) for i in range(input_count)
        ]
        self._output_ports = [
            _pipeline.Sender(parent=self, port_number=i) for i in range(output_count)
        ]
