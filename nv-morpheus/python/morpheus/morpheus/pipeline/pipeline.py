...
class Pipeline:
    ...
    def __init__(self, config: Config):
        ...
        self._segments: typing.Dict = defaultdict(
            lambda: {"nodes": set(), "ingress_ports": [], "egress_ports": []}
        )
    ...
    def _assert_not_built(self):
        assert self._state == PipelineState.INITIALIZED, (
            "Pipeline has already been built. Cannot modify pipeline."
        )
    ...
    def add_stage(self, stage: StageT, segment_id: str = "main") -> StageT:
        ...
        assert stage._pipeline is None or stage._pipeline is self, (
            "A stage can only be added to one pipeline at a time"
        )
        ...
        if isinstance(stage, _pipeline.Stage):
            ...
            raise NotImplementedError(
                f"add_stage() failed. Unknown node type: {type(stage)}"
            )
    ...
    def add_segment_edge(
        ...
        """
        self._assert_not_built()
        assert isinstance(egress_stage, _pipeline.BoundaryStageMixin), (
            "Egress stage must be a BoundaryStageMixin"
        )
        ...
        assert isinstance(ingress_stage, _pipeline.BoundaryStageMixin), (
            "Ingress stage must be a BoundaryStageMixin"
        )
    ...
    def _pre_build(self):
        ...
        for segment_id in self._segments.keys():
            ...
            if not all(x.is_pre_built for x in segment_graph.nodes()):
                logger.warning(
                    "Cyclic pipeline graph detected! Building with reduced constraints"
                )
                ...
                for stage in segment_graph.nodes():
                    if stage.can_pre_build(check_ports=True):
                        ...
                        raise RuntimeError(
                            "Could not build pipeline. Ensure all types can be determined"
                        )
    ...
    def build(self):
        ...
        """
        assert self._state == PipelineState.INITIALIZED, (
            "Pipeline can only be built once!"
        )
        assert len(self._sources) > 0, "Pipeline must have a source stage"
        ...
        exec_options.engine_factories.default_engine_type = (
            mrc.core.options.EngineType.Thread
        )
        ...
        def inner_build(builder: mrc.Builder, segment_id: str):
            ...
            asyncio.run_coroutine_threadsafe(
                self._async_start(segment_graph.nodes()), self._loop
            ).result()
    ...
    async def _start(self):
        assert self._state == PipelineState.BUILT, (
            "Pipeline must be built before starting"
        )
        ...
        def error_handler(_, context: dict):
            msg = (
                f"Unhandled exception in async loop! Exception: \n{context['message']}"
            )
    ...
    def stop(self):
        ...
        """
        assert self._state == PipelineState.STARTED, (
            "Pipeline must be running to stop it"
        )
    ...
    async def join(self):
        ...
        """
        assert self._post_start_future is not None, (
    ...
    async def build_and_start(self):
        if self._state == PipelineState.INITIALIZED:
            try:
                ...
                logger.exception(
                    "Error occurred during Pipeline.build(). Exiting.", exc_info=True
                )
                raise
    ...
    def visualize(self, filename: str = None, **graph_kwargs):
        ...
        if self._state == PipelineState.INITIALIZED:
            raise RuntimeError(
                "Pipeline.visualize() requires that the Pipeline has been started before generating "
        ...
        for segment_id in self._segments:
            ...
            for name, attrs in typing.cast(
                typing.Mapping[_pipeline.StageBase, dict],
                self._segment_graphs[segment_id].nodes,
            ).items():
                ...
                if show_in_ports:
                    tmp_str = " | ".join(
                        [
                            f"<u{x.port_number}> input_port: {x.port_number}"
                            for x in name.input_ports
                        ]
                    )
                ...
                if show_out_ports:
                    tmp_str = " | ".join(
                        [
                            f"<d{x.port_number}> output_port: {x.port_number}"
                            for x in name.output_ports
                        ]
                    )
        ...
        for segment_id in self._segments:
            gv_subgraph = gv_subgraphs[segment_id]
            for e, attrs in typing.cast(
                typing.Mapping[
                    typing.Tuple[_pipeline.StageBase, _pipeline.StageBase], dict
                ],
                self._segment_graphs[segment_id].edges(),
            ).items():  # noqa: E501
                edge_attrs = {}
                ...
                if (
                    len(in_port._input_senders) == 1
                    and len(out_port._output_receivers) == 1
                    ...
                    if rec_idx == 0 and out_port.output_schema is not None:
                        edge_attrs["taillabel"] = pretty_print_type_name(
                            out_port.output_type
                        )

                    if sen_idx == 0 and in_port.input_schema is not None:
                        edge_attrs["headlabel"] = pretty_print_type_name(
                            in_port.input_type
                        )
