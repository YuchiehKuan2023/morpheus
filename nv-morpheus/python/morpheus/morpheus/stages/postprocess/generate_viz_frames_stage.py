...
class GenerateVizFramesStage(GpuAndCpuMixin, PassThruTypeMixin, SinglePortStage):
    ...
    def _to_vis_df(self, msg: ControlMessage):
        ...
        df["si"] = pd.Series(
            np.choose(
                index_sens_info.get(), list(idx2label.values()) + ["none"]
            ).tolist()
        )
    ...
    def _write_viz_file(self, x: typing.List[typing.Tuple[int, pd.DataFrame]]):
        ...
        in_df = pd.concat([df for _, df in x], ignore_index=True).sort_values(
            by=["timestamp"]
        )
    ...
    async def start_async(self):
        ...
        async def run_server():
            ...
            try:
                async with serve(
                    client_connected, self._server_url, self._server_port
                ) as server:
                    listening_on = [
                        ":".join([str(y) for y in x.getsockname()])
                        for x in server.sockets
                    ]
                    listening_on_str = [f"'{x}'" for x in listening_on]

                    logger.info(
                        "Websocket server listening at: %s", ", ".join(listening_on_str)
                    )
                ...
                logger.info(
                    "Server shut down. Is queue empty: %s", self._buffer_queue.empty()
                )
    ...
    def _build_single(
        ...
        def node_fn(input_obs, output_obs):
            def write_batch(msg: ControlMessage):
                ...
                try:
                    ...
                    asyncio.run_coroutine_threadsafe(
                        self._buffer_queue.put(out_buf), loop=self._loop
                    ).result()
                except Closed:
            ...
            shutdown_future = asyncio.run_coroutine_threadsafe(
                self._stop_server(), loop=self._loop
            )
