...
class _UserTimeSeries:
    ...
    def __init__(
        ...
        # Stateful members
        self._pending_messages: deque[ControlMessage] = (
            deque()
        )  # Holds the existing messages pending
        self._timeseries_data: pd.DataFrame = pd.DataFrame(
            columns=[self._timestamp_col]
    ...
    def _calc_bin_series(self, timeseries: pd.Series) -> pd.Series:
        return (
            round(
                (timeseries.dt.round(freq="S") - self._t0_epoch).dt.total_seconds()
            ).astype(int)
    ...
    def _calc_outliers(self, action: _TimeSeriesAction):
        ...
        signal_cp, signal_bins = cp.histogram(
            cp.array(action.window["event_bin"]),
            bins=cp.arange(window_start, window_end + 1, 1),
        )

        # TODO(MDD): Take this out after testing
        assert cp.sum(signal_cp) == len(action.window), (
            "All points in window are not accounted for in histogram"
        )

        is_anomaly = fftAD(
            signal_cp, percentile=self._filter_percent, zthresh=self._zscore_threshold
        )
        ...
        if len(is_anomaly) > 0:
            ...
            action.message.payload().set_data(
                "ts_anomaly", anomalies[0 : action.message.payload().count]
            )
            ...
            paired_anomalies = anomalies[anomalies == True].index.intersection(
                idx.to_pandas()
            )  # noqa: E712
            ...
            if len(paired_anomalies) > 0:
                return self._t0_epoch + pd.to_timedelta(
                    (
                        action.window.loc[paired_anomalies]["event_bin"].unique()
                        * self._resolution_sec
                    ),
                    unit="s",
                )
    ...
    def _determine_action(
    ...
    ) -> typing.Optional[_TimeSeriesAction]:
        ...
        message_start = calc_bin(
            pd.Timestamp(x.payload().get_data(self._timestamp_col).iloc[0]),
            self._t0_epoch,
            self._resolution_sec,
        )
        message_end = calc_bin(
            pd.Timestamp(x.payload().get_data(self._timestamp_col).iloc[-1]),
            self._t0_epoch,
            self._resolution_sec,
        )
        ...
        if timeseries_start > window_start:
            logger.debug(
                "Warming up.    TS: %s, WS: %s, MS: %s, ME: %s, WE: %s, TE: %s. Delta: %s",
                timeseries_start,
                window_start,
                message_start,
                message_end,
            ...
            if not self._is_warm and not is_complete:
                return _TimeSeriesAction(
                    send_message=True, message=self._pending_messages.popleft()
                )
        ...
        if timeseries_end < window_end:
            logger.debug(
                "Filling front. TS: %s, WS: %s, MS: %s, ME: %s, WE: %s, TE: %s. Delta: %s",
                timeseries_start,
                window_start,
                message_start,
                message_end,
        ...
        logger.debug(
            "Perform Calc.  TS: %s, WS: %s, MS: %s, ME: %s, WE: %s, TE: %s.",
            timeseries_start,
            window_start,
            message_start,
            message_end,
        ...
        self._timeseries_data.drop(
            self._timeseries_data[
                self._timeseries_data["event_bin"] < window_start
            ].index,
            inplace=True,
        )

        # Now grab timestamps within the window
        window_timestamps_df = self._timeseries_data[
            self._timeseries_data["event_bin"] <= window_end
        ]
    ...
    def _calc_timeseries(self, x: ControlMessage, is_complete: bool):
        if x is not None:
            ...
            self._timeseries_data = pd.concat(
                [self._timeseries_data, new_timedata]
            ).sort_index()
            ...
            if (
                len(
                    pd.RangeIndex(
                        start=self._processed_index,
                        stop=self._timeseries_data.index[-1] + 1,
                        step=1,
                    ).difference(self._timeseries_data.index)
                )
                > 0
            ):
                return []
        ...
        self._timeseries_data["event_bin"] = self._calc_bin_series(
            self._timeseries_data[self._timestamp_col]
        )
...
class TimeSeriesStage(PassThruTypeMixin, SinglePortStage):
    ...
    def _call_timeseries_user(self, x: ControlMessage):
        ...
        if user_id not in self._timeseries_per_user:
            self._timeseries_per_user[user_id] = _UserTimeSeries(
                user_id=user_id,
