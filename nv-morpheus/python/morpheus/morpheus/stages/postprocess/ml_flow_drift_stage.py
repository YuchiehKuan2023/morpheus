...
@register_stage(
    "mlflow-drift", modes=[PipelineModes.FIL, PipelineModes.NLP, PipelineModes.OTHER]
)
class MLFlowDriftStage(PassThruTypeMixin, SinglePortStage):
    ...
    def __init__(
        ...
        if self._batch_size > c.pipeline_batch_size:
            logger.warning(
                (
                    "Warning: MLFlowDriftStage batch_size (%d) is greater than pipeline_batch_size (%d). "
        ...
        if run_id is None and not force_new_run:
            ...
            if len(active_runs) > 0 and "tags.morpheus.type" in active_runs:
                morpheus_runs = active_runs[
                    active_runs["tags.morpheus.type"] == "drift"
                ]
        ...
        mlflow.start_run(
            run_id, run_name="Model Drift", tags={"morpheus.type": "drift"}
        )
