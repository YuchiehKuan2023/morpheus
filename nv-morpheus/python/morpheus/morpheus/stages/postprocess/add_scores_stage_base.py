...
class AddScoresStageBase(PassThruTypeMixin, GpuAndCpuMixin, SinglePortStage):
    ...
    def __init__(
        ...
        *,
        ...
        self._labels = (
            labels if labels is not None and len(labels) > 0 else c.class_labels
        )
        self._prefix = prefix
        ...
        for label in self._labels:
            ...
            if label not in self._class_labels:
                logger.warning(
                    "The label '%s' is not in Config.class_labels and will be ignored",
                    label,
                )
                continue
    ...
    def _add_labels(
        ...
        if probs.shape[1] <= max(idx2label.keys()):
            raise RuntimeError(
                (
                    "Model output did not contain enough columns to fufill the requested labels. "
