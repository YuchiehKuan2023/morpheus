...
class ValidationStage(CompareDataFrameStage):  # pylint: disable=too-many-ancestors
    ...
    def __init__(
        ...
        if self._results_file_name is not None and os.path.exists(
            ...
            if overwrite:
                ...
                raise FileExistsError(
                    f"Cannot output validation results to '{self._results_file_name}'. "
    ...
    def _build_single(
        ...
        node = builder.make_node(
            self.unique_name,
            ops.map(self._append_message),
            ops.on_completed(self._do_comparison),
        )
