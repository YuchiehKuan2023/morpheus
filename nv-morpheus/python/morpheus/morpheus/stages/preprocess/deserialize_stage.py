...
class DeserializeStage(GpuAndCpuMixin, ControlMessageStage):
    ...
    def __init__(
        ...
        *,
        ...
        if (self._task_type is None) != (self._task_payload is None):
            raise ValueError(
                "Both `task_type` and `task_payload` must be specified if either is specified."
            )
    ...
    def _build_single(
        ...
        if self._build_cpp_node():
            ...
            out_node = _stages.DeserializeStage(
                builder,
                self.unique_name,
            ...
            module_loader = DeserializeLoaderFactory.get_instance(
