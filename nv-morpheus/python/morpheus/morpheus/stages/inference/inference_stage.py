...
class InferenceWorker:
    ...
    def build_output_message(self, msg: ControlMessage) -> ControlMessage:
        ...
        memory = TensorMemory(
            count=output_dims[0], tensors={"probs": cp.zeros(output_dims)}
        )
    ...
    def process(
        self, batch: ControlMessage, callback: typing.Callable[[TensorMemory], None]
    ):
        """
...
class InferenceStage(ControlMessageStage):
    ...
    def _get_inference_worker(
        self, inf_queue: ProducerConsumerQueue
    ) -> InferenceWorker:
        """
        ...
        def py_inference_fn(obs: mrc.Observable, sub: mrc.Subscriber):
            ...
            def on_next(message: ControlMessage):
                ...
                for batch in batches:
                    ...
                    def set_output_fut(
                        resp: TensorMemory, inner_batch, batch_future: mrc.Future
                    ):
                        nonlocal outstanding_requests
                        nonlocal batch_offset
                        mess = self._convert_one_response(
                            output_message, inner_batch, resp, batch_offset
                        )
                    ...
                    worker.process(
                        ...
                        partial(
                            set_output_fut,
                            inner_batch=batch,
                            batch_future=completion_future,
                        ),
                    )
            ...
            assert outstanding_requests == 0, (
                "Not all inference requests were completed"
            )
    ...
    @staticmethod
    def _split_batches(
        msg: ControlMessage, max_batch_size: int
    ) -> typing.List[ControlMessage]:
        out_batches = []

        id_array = cp.concatenate(
            [cp.array([-1]), msg.tensors().get_tensor("seq_ids")[:, 0], cp.array([-1])]
        )
    ...
    @staticmethod
    def _convert_one_response(
        output: ControlMessage,
        inf: ControlMessage,
        res: TensorMemory,
        batch_offset: int,
    ) -> ControlMessage:  # pylint:disable=unused-argument
