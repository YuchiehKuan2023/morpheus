...
@lru_cache(None)
def _notify_dtype_once(
    model_name: str, input_name: str, triton_dtype: cp.dtype, data_dtype: cp.dtype
):
...
class ResourcePool(typing.Generic[_T]):
    ...
    def __init__(self, create_fn: typing.Callable[[], _T], max_size: int = 1000):
        ...
        self._queue: ProducerConsumerQueue[_T] = ProducerConsumerQueue(
...
class InputWrapper:
    ...
    def build_input(
        self, name: str, data: cp.ndarray, force_convert_inputs: bool
        ...
        triton_input = tritonclient.InferInput(
            name, list(data.shape), self._config[name].datatype
        )
...
class ShmInputWrapper(InputWrapper):
    ...
    def __init__(
        ...
        for key in self._config.keys():
            self._config[key].ptr = cp.cuda.MemoryPointer(
                self._memory, self._config[key].offset
            )
        ...
        client.register_cuda_shared_memory(
            self.region_name, base64.b64encode(self._ipc_handle), 0, self._total_bytes
        )
    ...
    def build_input(
        ...
        triton_input.set_shared_memory(
            self.region_name, data.nbytes, self.get_offset(name)
        )
...
class TritonInferenceWorker(InferenceWorker):
    ...
    def init(self):
        ...
        self._triton_client = tritonclient.InferenceServerClient(
            url=self._server_url, verbose=False
        )

        try:
            assert (
                self._triton_client.is_server_live()
                and self._triton_client.is_server_ready()
            ...
            model_meta = self._triton_client.get_model_metadata(
            ...
            if int(model_meta["inputs"][0]["shape"][-1]) != self._fea_length:
                raise RuntimeError(
                    f"Mismatched Sequence Length. Config specified {self._fea_length} but model"
            ...
            def build_inout(x: dict, mapping: dict[str, str]):
                ...
                mapped_name = (
                    x["name"] if x["name"] not in mapping else mapping[x["name"]]
                )
            ...
            for x in model_meta["outputs"]:
                assert x["name"] not in self._inputs, (
                    "Input/Output names must be unique from eachother"
                )
            ...
            if self._use_shared_memory:
                ...
                def create_wrapper():
                    return ShmInputWrapper(
                        self._triton_client, self._model_name, shm_config
                    )
            else:
            ...
            logger.exception(
                "Exception occurred while coordinating with Triton. Exception message: \n%s\n",
                ex,
                exc_info=ex,
            )
            raise ex
    ...
    def calc_output_dims(self, msg: ControlMessage) -> typing.Tuple:
        return (
            msg.tensor_count(),
            self._outputs[list(self._outputs.keys())[0]].shape[1],
        )
    ...
    def _build_response(
        ...
        return TensorMemory(
            ...
            tensors={
                "probs": cp.array(output["probs"])
            },  # For now, only support one output
        )
    ...
    def process(
        ...
        outputs = [
            tritonclient.InferRequestedOutput(output.name)
            for output in self._outputs.values()
        ]
...
@register_stage(
    "inf-triton", modes=[PipelineModes.NLP, PipelineModes.FIL, PipelineModes.OTHER]
)
class TritonInferenceStage(InferenceStage):
    ...
    def __init__(
        ...
        input_mapping_ = self._INFERENCE_WORKER_DEFAULT_INOUT_MAPPING.get(
            c.mode, {}
        ).get("inputs", {})
        output_mapping_ = self._INFERENCE_WORKER_DEFAULT_INOUT_MAPPING.get(
            c.mode, {}
        ).get("outputs", {})
        ...
        if inout_mapping:
            ...
            warnings.warn(
                (
                    "TritonInferenceStage's `inout_mapping` argument has been deprecated. "
    ...
    def supports_cpp_node(self) -> bool:
        ...
        if TritonInferenceWorker.supports_cpp_node():
            ...
            logger.warning(
                "The C++ implementation of TritonInferenceStage does not support the use_shared_memory "
