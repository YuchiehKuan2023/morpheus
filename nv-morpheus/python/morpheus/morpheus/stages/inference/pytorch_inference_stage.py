...
try:
    ...
    print(
        (
            "PyTorch Not Found! PyTorch must be installed to use the PyTorchInferenceStage. "
...
class _PyTorchInferenceWorker(InferenceWorker):
    ...
    def __init__(
        self, inf_queue: ProducerConsumerQueue, c: Config, model_filename: str
    ):
    ...
    def calc_output_dims(self, msg: ControlMessage) -> typing.Tuple:
        ...
        if not self._output_size:
            ...
            test_outputs = self._model(
                torch.randint(65000, (test_intput_ids_shape), dtype=torch.long).cuda(),
                token_type_ids=None,
    ...
    def process(
        ...
        attention_mask = from_dlpack(input_mask.astype(cp.float).toDlpack()).type(
            torch.long
        )
...
@register_stage(
    "inf-pytorch", modes=[PipelineModes.FIL, PipelineModes.NLP, PipelineModes.OTHER]
)
class PyTorchInferenceStage(InferenceStage):
    ...
    def _get_inference_worker(
        ...
        return _PyTorchInferenceWorker(
            inf_queue, self._config, model_filename=self._model_filename
        )
