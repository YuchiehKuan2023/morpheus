...
class AppShieldSourceStage(PreallocatorMixin, GpuAndCpuMixin, SingleOutputSource):
    ...
    def read_file_to_df(file: io.TextIOWrapper, cols_exclude: list[str]):
        ...
        try:
            ...
            plugin_df.drop(
                columns=plugin_df.columns.difference(features_plugin), inplace=True
            )
    ...
    def load_df(filepath: str, cols_exclude: list[str], encoding: str) -> pd.DataFrame:
        ...
        try:
            ...
            logger.error(
                "Unable to load %s to dataframe with %s encoding : %s",
                filepath,
                encoding,
                decode_error,
            )
            ...
            logger.info(
                "Retrying... Attempting to load %s with utf-8 encoding", filepath
            )
    ...
    @staticmethod
    def load_meta_cols(
        filepath_split: list[str], plugin: str, plugin_df: pd.DataFrame
    ) -> pd.DataFrame:
        """
        ...
        if len(filepath_split) < 3:
            raise ValueError(
                f"Invalid filepath_split {filepath_split}. Length should be greater than 2"
            )
    ...
    def batch_source_split(
        x: list[pd.DataFrame], source: str
        ...
        if len(unique_sources) > 1:
            for source_name in unique_sources:
                source_dfs[source_name] = combined_df[
        ...
        for filepath in x:
            try:
                ...
                if plugin in plugins_include:
                    plugin_df = AppShieldSourceStage.load_df(
                        filepath, cols_exclude, encoding
                    )
                    plugin_df = AppShieldSourceStage.fill_interested_cols(
                        plugin_df, cols_include
                    )
                    plugin_df = AppShieldSourceStage.load_meta_cols(
                        filepath_split, plugin, plugin_df
                    )
                    plugin_dfs.append(plugin_df)

            except JSONDecodeError as decode_error:
                logger.error(
                    "Unable to decode json file %s: %s", filepath, decode_error
                )

        df_per_source = AppShieldSourceStage.batch_source_split(
            plugin_dfs, source="source"
        )
    ...
    def _post_build_single(
        self, builder: mrc.Builder, out_node: mrc.SegmentObject
    ) -> mrc.SegmentObject:
