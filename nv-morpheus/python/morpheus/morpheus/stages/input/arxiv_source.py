...
class ArxivSource(GpuAndCpuMixin, PreallocatorMixin, SingleOutputSource):
    ...
    def _build_source(self, builder: mrc.Builder) -> mrc.SegmentObject:
        download_pages = builder.make_source(
            self.unique_name + "-download", self._generate_frames
        )
        process_pages = builder.make_node(
            self.unique_name + "-process", ops.map(self._process_pages)
        )
        process_pages.launch_options.pe_count = 6

        builder.make_edge(download_pages, process_pages)

        splitting_pages = builder.make_node(
            self.unique_name + "-split", ops.map(self._splitting_pages)
        )
    ...
    def _process_pages(self, pdf_path: str):
        ...
        for _ in range(5):
            try:
                ...
                logger.debug(
                    "Processing %s/%s: %s", len(documents), self._total_pages, pdf_path
                )
