...
class CloudTrailSourceStage(AutoencoderSourceStage):
    ...
    def read_file(filename: str, file_type: FileTypes) -> pd.DataFrame:
        ...
        if (
            determine_file_type(filename) == FileTypes.JSON
            and len(df) == 1
            and list(df) == ["Records"]
        ):
            # Reread with lines=False
            df = read_file_to_df(
                filename, file_type, df_type="pandas", parser_kwargs={"lines": False}
            )
    ...
    def cleanup_df(df: pd.DataFrame, feature_columns: typing.List[str]):
        ...
        def clean_column(cloudtrail_df):
            ...
            if col_name in cloudtrail_df:
                cloudtrail_df[col_name] = cloudtrail_df[col_name].apply(
        ...
        if "userIdentityaccountId" in df and df[
            "userIdentityaccountId"
        ].dtype != np.dtype("O"):
            df["userIdentityaccountId"] = "Account-" + df[
                "userIdentityaccountId"
            ].astype(str)
    ...
    def files_to_dfs_per_user(
        ...
        df_per_user = CloudTrailSourceStage.batch_user_split(
            dfs, userid_column_name, userid_filter
        )
