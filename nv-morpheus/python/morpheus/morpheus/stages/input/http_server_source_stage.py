...
class HttpServerSourceStage(
    GpuAndCpuMixin, PreallocatorMixin, ConfigurableOutputSource
):
    """
    Source stage that starts an HTTP server and listens for incoming requests on a specified endpoint.
    ...
    def __init__(
        ...
        for url, http_method in [
            (endpoint, method),
            (live_endpoint, live_method),
            (ready_endpoint, ready_method),
        ]:
            check_supported_method(
                url,
                http_method,
                SUPPORTED_METHODS if url == endpoint else HEALTH_SUPPORTED_METHODS,
            )
    ...
    def _parse_payload(self, payload: str, headers: dict = None) -> HttpParseResponse:
        ...
        try:
            ...
            self._queue_size += 1
            return HttpParseResponse(
                status_code=self._accept_status.value,
                content_type=MimeTypes.TEXT.value,
                body="",
            )
    ...
    def _liveliness_check(self, _: str) -> HttpParseResponse:
        ...
        return HttpParseResponse(
            status_code=HTTPStatus.OK.value, content_type=MimeTypes.TEXT.value, body=""
        )
    ...
    def _generate_frames(
        self, subscription: mrc.Subscription
    ) -> typing.Iterator[ControlMessage | MessageMeta]:
        from morpheus.common import FiberQueue
        ...
        msg = HttpEndpoint(
            ...
            self._method.name,
            include_headers=(
                self._message_type is SupportedMessageTypes.CONTROL_MESSAGE
            ),
        )
        live = HttpEndpoint(
            self._liveliness_check,
            self._live_endpoint,
            self._live_method.name,
        ...
        with (
            FiberQueue(self._max_queue_size) as self._queue,
            HttpServer(
                endpoints=[msg, live, ready],
            ...
            while self._processing:
                ...
                try:
                    ...
                    if (
                        not self._http_server.is_running()
                        or self.is_stop_requested()
                ...
                if df is not None:
                    ...
                    if (
                        self._stop_after > 0
                        and self._records_emitted >= self._stop_after
                    ):
                        self._processing = False

    def _set_default_payload_to_df_fn(self):
        reader = get_json_reader(self._config.execution_mode)
        self._payload_to_df_fn = lambda payload, lines: reader(
            StringIO(initial_value=payload), lines=lines
        )
    ...
    def _build_source(self, builder: mrc.Builder) -> mrc.SegmentObject:
        if self._build_cpp_node() and self._payload_to_df_fn is None:
            ...
            if self._message_type is SupportedMessageTypes.CONTROL_MESSAGE:
                http_server_kwargs.update(
                    {"task_type": self._task_type, "task_payload": self._task_payload}
                )
...
def check_supported_method(
    endpoint: str, method: HTTPMethod, supported_methods: typing.Tuple[HTTPMethod, ...]
) -> None:
