...
class DataBricksDeltaLakeSourceStage(
    GpuAndCpuMixin, PreallocatorMixin, SingleOutputSource
):
    """
    ...
    def source_generator(self, subscription: mrc.Subscription):
        try:
            ...
            spark_df = spark_df.select("*").withColumn(
                "_id", sf.row_number().over(window)
            )
            count = spark_df.count()
            while self.offset <= count and subscription.is_subscribed():
                df = spark_df.where(
                    sf.col("_id").between(
                        self.offset, self.offset + self.items_per_page
                    )
                )
                logger.debug(
                    "Reading next iteration data between index: \
                    %s and %s",
                    str(self.offset),
