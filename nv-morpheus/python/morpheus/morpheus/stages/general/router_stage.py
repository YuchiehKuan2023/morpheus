...
class RouterStage(GpuAndCpuMixin, _pipeline.Stage):
    ...
    def __init__(
        ...
        *,
        ...
        if self._processing_engines < 0:
            raise ValueError(
                "Invalid number of processing engines. Must be greater than or equal to 0."
            )
    ...
    def _build(
        ...
        if self._processing_engines > 0:
            self._router = Router(
                builder, self.unique_name, router_keys=self._keys, key_fn=self._key_fn
            )

            self._router.launch_options.engines_per_pe = self._processing_engines
        else:
            self._router = RouterComponent(
                builder, self.unique_name, router_keys=self._keys, key_fn=self._key_fn
            )
