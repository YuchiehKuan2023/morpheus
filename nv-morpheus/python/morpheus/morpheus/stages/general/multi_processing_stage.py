    ...
    def __init__(
        ...
        *,
        ...
        if max_in_flight_messages is None:
            ...
            self._max_in_flight_messages = int(
                self._shared_process_pool.total_max_workers * 1.5
            )
        else:
        ...
        for param in signature.parameters.values():
            if param.name not in bound_args:
                if input_arg is not None:
                    raise ValueError(
                        "Found more than one unbound arguments in partial function"
                    )
                input_arg = param
...
class MultiProcessingStage(MultiProcessingBaseStage[InputT, OutputT]):
    ...
    def create(
        *,
        ...
        return MultiProcessingStage[input_t, output_t](
            c=c,
