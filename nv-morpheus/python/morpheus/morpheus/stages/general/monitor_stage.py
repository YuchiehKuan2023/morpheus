...
class MonitorStage(PassThruTypeMixin, GpuAndCpuMixin, SinglePortStage):
    ...
    def _build_single(
        ...
        if self._build_cpp_node() and self._schema.input_type in (
            ControlMessage,
            MessageMeta,
        ):
            if self._schema.input_type == ControlMessage:
                node = _stages.MonitorControlMessageStage(
                    builder,
                    self.unique_name,
                ...
                node = _stages.MonitorMessageMetaStage(
                    builder,
                    self.unique_name,
                    self._mc._description,
                    self._mc._unit,
            ...
            # results
            node = builder.make_node_component(
                self.unique_name,
