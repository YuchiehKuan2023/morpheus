...
class DataBricksDeltaLakeSinkStage(PassThruTypeMixin, SinglePortStage):
    ...
    def _build_single(
        ...
        def write_to_deltalake(meta: MessageMeta):
            ...
            spark_df.write.format("delta").option("mergeSchema", "true").mode(
                self.delta_table_write_mode
    ...
    @staticmethod
    def _extract_schema_from_pandas_dataframe(
        df: pd.DataFrame,
        ...
        for col, dtype in df.dtypes.items():
            try:
                ...
                logger.error(
                    "Encountered error %s while converting columns %s with data type %s",
                    e,
                    col,
                    dtype,
                )
