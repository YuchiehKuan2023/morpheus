...
class HttpServerSinkStage(GpuAndCpuMixin, PassThruTypeMixin, SinglePortStage):
    ...
    async def start_async(self):
        ...
        http_endpoint = HttpEndpoint(
            py_parse_fn=self._request_handler,
            url=self._endpoint,
    ...
    def _request_callback(
        self, df: DataFrameType, num_tasks: int, has_error: bool, error_msg: str
    ) -> None:
        try:
            if has_error:
    ...
    def _request_handler(self, _: str) -> HttpParseResponse:
        ...
        data_frames = []
        try:
            while num_rows == 0 or num_rows < (
                self._max_rows_per_response * self._overflow_pct
            ):
        ...
        if len(data_frames) > 0:
            ...
            return HttpParseResponse(
                ...
                on_complete_callback=partial(
                    self._request_callback, df, len(data_frames)
                ),
            )
