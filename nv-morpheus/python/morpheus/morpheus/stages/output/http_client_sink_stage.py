...
class HttpClientSinkStage(GpuAndCpuMixin, PassThruTypeMixin, SinglePortStage):
    ...
    **request_kwargs : dict
    ...
    def __init__(
        ...
        lines: bool = False,
        df_to_request_kwargs_fn: typing.Optional[
            typing.Callable[[str, str, DataFrameType], dict]
        ] = None,
        **request_kwargs,
    ):
        super().__init__(c)
    ...
    def _chunk_requests(self, df: DataFrameType) -> typing.Iterable[dict]:
        ...
        while slice_start < len(df):
            ...
            if self._df_to_request_kwargs_fn is not None:
                yield self._df_to_request_kwargs_fn(
                    self._base_url, self._endpoint, df_slice
                )
            else:
