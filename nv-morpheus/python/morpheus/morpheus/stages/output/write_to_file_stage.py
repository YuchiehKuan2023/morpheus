...
class WriteToFileStage(GpuAndCpuMixin, PassThruTypeMixin, SinglePortStage):
    ...
    def __init__(
        ...
        self._controller = WriteToFileController(
            filename=filename,
            overwrite=overwrite,
            file_type=file_type,
    ...
    def _build_single(
        ...
        if self._build_cpp_node():
            ...
            to_file_node = _stages.WriteToFileStage(
                builder,
                self.unique_name,
            ...
            to_file_node = builder.make_node(
                self.unique_name, ops.build(self._controller.node_fn)
            )
