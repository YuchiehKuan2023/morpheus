...
class WriteToElasticsearchStage(PassThruTypeMixin, SinglePortStage):
    ...
    def __init__(
        ...
        try:
            ...
            raise FileNotFoundError(
                f"The specified connection configuration file '{connection_conf_file}' does not exist."
            ) from exc
        except Exception as exc:
            raise RuntimeError(
                f"An error occurred while loading the configuration file: {exc}"
            ) from exc

        if connection_kwargs_update_func:
            connection_kwargs = connection_kwargs_update_func(connection_kwargs)

        self._controller = ElasticsearchController(
    ...
    def _build_single(
        ...
        to_elasticsearch = builder.make_node(
            self.unique_name,
            ops.map(on_data),
