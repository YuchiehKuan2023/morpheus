...
def write_to_elasticsearch(builder: mrc.Builder):
    ...
    if not isinstance(connection_kwargs, dict):
        raise ValueError(
            f"Expects `connection_kwargs` as a dictionary, but it is of type {type(connection_kwargs)}"
        )
    ...
    if pickled_func_config:
        ...
        if pickled_func_str and encoding:
            connection_kwargs_update_func = pickle.loads(
                bytes(pickled_func_str, encoding)
            )
    ...
    node = builder.make_node(
        ...
        ops.map(on_data),
        ops.on_completed(controller.close_client),
    )
