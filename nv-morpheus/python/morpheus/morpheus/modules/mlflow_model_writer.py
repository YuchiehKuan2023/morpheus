...
from morpheus.controllers.mlflow_model_writer_controller import (
    MLFlowModelWriterController,
)
...
def mlflow_model_writer(builder: mrc.Builder):
    ...
    controller = MLFlowModelWriterController(
    ...
    def node_fn(obs: mrc.Observable, sub: mrc.Subscriber):
        obs.pipe(
            ops.map(controller.on_data), ops.filter(lambda x: x is not None)
        ).subscribe(sub)
