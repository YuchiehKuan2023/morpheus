    ...
    if not message.has_sliceable_index():
        if ensure_sliceable_index:
            ...
            if old_index_name:
                logger.warning(
                    (
                        "Incoming MessageMeta does not have a unique and monotonic index. "
...
def _process_dataframe_to_control_message(
    message: MessageMeta,
    batch_size: int,
...
def _deserialize(builder: mrc.Builder):
    ...
    if (task_type is not None) != (task_payload is not None):
        raise ValueError(
            "task_type and task_payload must be both specified or both None"
        )
    ...
    map_func = partial(
        _process_dataframe_to_control_message,
