...
def from_control_message(builder: mrc.Builder):
    ...
    def control_message_to_meta(x: ControlMessage) -> MessageMeta:
        if not isinstance(x, ControlMessage):
            raise TypeError(
                f"Expected 'x' to be of type ControlMessage, but instead got {type(x).__name__}."
            )

        message_meta = x.payload()
        if message_meta is None:
            logger.debug(
                "ControlMessage does not contain a payload, it cannot be converted to a MessageMeta object."
