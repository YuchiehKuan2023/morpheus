...
def monitor(builder: mrc.Builder):
    ...
    if determine_count_fn_schema is not None:
        if hasattr(determine_count_fn_schema, "schema_str") and hasattr(
            determine_count_fn_schema, "encoding"
        ):
            determine_count_fn = pickle.loads(
                bytes(
                    determine_count_fn_schema.get("schema_str"),
                    determine_count_fn_schema.get("encoding"),
                )
            )
    ...
    def node_fn(obs: mrc.Observable, sub: mrc.Subscriber):
        obs.pipe(
            ops.map(controller.progress_sink),
            ops.on_completed(
                partial(controller.sink_on_completed, tqdm_class=tqdm_class)
            ),
        ).subscribe(sub)
