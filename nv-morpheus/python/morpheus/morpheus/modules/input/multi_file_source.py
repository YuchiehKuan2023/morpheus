...
MultiFileSourceLoaderFactory = ModuleLoaderFactory(
    "multi_file_source", "morpheus", MultiFileSourceSchema
)
...
def _multi_file_source(builder: mrc.Builder):
    ...
    def generate_frames_fsspec():
        ...
        if not filenames:
            logger.warning(
                "Multi-file-source was provided with no filenames for processing this is probably not what"
                "you want"
            )
            return
