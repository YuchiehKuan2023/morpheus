...
def filter_detections(builder: mrc.Builder):
    ...
    filter_source_dict = {
        "AUTO": FilterSource.Auto,
        "DATAFRAME": FilterSource.DATAFRAME,
        "TENSOR": FilterSource.TENSOR,
    }
    ...
    controller = FilterDetectionsController(
        threshold=threshold,
    ...
    if use_cpp:
        node = _stages.FilterDetectionsStage(
            builder,
            FILTER_DETECTIONS,
        ...
        if copy:
            ...
            node = builder.make_node(
                FILTER_DETECTIONS, ops.map(controller.filter_slice), ops.flatten()
            )
