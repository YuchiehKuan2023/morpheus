...
def file_batcher(builder: mrc.Builder):
    ...
    iso_date_regex_pattern = config.get(
        "iso_date_regex_pattern", DEFAULT_ISO_DATE_REGEX_PATTERN
    )
    ...
    if sampling_rate_s is not None and sampling_rate_s > 0:
        assert sampling is None, (
            "Cannot set both sampling and sampling_rate_s at the same time"
        )

        # Show the deprecation message
        warnings.warn(
            (
                "The `sampling_rate_s` argument has been deprecated. "
    ...
    def validate_control_message(control_message: ControlMessage):
        if control_message.has_metadata("batching_options") and not isinstance(
            ...
            raise ValueError(
                "Invalid or missing 'batching_options' metadata in control message"
            )

        data_type = control_message.get_metadata("data_type")
        if data_type not in {"payload", "streaming"}:
            raise ValueError(
                f"Invalid 'data_type' metadata in control message: {data_type}"
            )

    def build_period_batches(
        files: typing.List[str], params: typing.Dict[any, any]
    ) -> typing.List[typing.Tuple[typing.List[str], int]]:
        ...
        try:
            ...
            if start_time is not None:
                start_time = datetime.datetime.fromisoformat(start_time).replace(
                    tzinfo=datetime.timezone.utc
                )

            if end_time is not None:
                end_time = datetime.datetime.fromisoformat(end_time).replace(
                    tzinfo=datetime.timezone.utc
                )
        ...
        for file_object in file_objects:
            ...
            if (start_time is not None and time_stamp < start_time) or (
                end_time is not None and time_stamp > end_time
            ):
                continue
        ...
        # Build the dataframe
        df = pd.DataFrame(
            index=pd.DatetimeIndex(timestamps), data={"filename": full_names}
        )
    ...
    def generate_cms_for_batch_periods(
        ...
        batch_periods: typing.List[typing.Tuple[typing.List[str], int]],
    ) -> typing.List[ControlMessage]:
        data_type = control_message.get_metadata("data_type")
            ...
            if filenames:
                load_task = {
                    ...
                    "batcher_config": {  # TODO(Devin): Remove this when we're able to attach config to the loader
                        "timestamp_column_name": file_to_df_params.get(
    ...
    def on_data(control_message: ControlMessage) -> typing.List[ControlMessage]:
        try:
            ...
            control_messages = generate_cms_for_batch_periods(
                control_message, batch_periods
            )
            ...
            logger.error(
                "Error building file list, discarding control message %s", exec_info
            )
            return []
