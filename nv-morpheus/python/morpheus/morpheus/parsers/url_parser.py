...
def _handle_unknown_suffix(
    unknown_suffix_df: DataFrameType, col_dict: dict[str, bool]
) -> DataFrameType:
...
def _extract_tld(
    ...
    suffix_df: DataFrameType,
    col_len: int,
    col_dict: dict[str, bool],
) -> DataFrameType:
    tmp_dfs = []
...
def _create_col_dict(
    allowed_output_cols: set[str], req_cols: set[str]
) -> dict[str, bool]:
...
def _generate_tld_cols(
    hostname_split_df: DataFrameType, hostnames: SeriesType, col_len: int
) -> DataFrameType:
...
def _extract_hostnames(urls: SeriesType) -> SeriesType:
    hostnames = urls.str.extract("([\\w]+[\\.].*[^/]|[\\-\\w]+[\\.].*[^/])")[
        0
    ].str.extract("([\\w\\.\\-]+)")[0]
    return hostnames
...
def parse(urls: SeriesType, req_cols: set[str] = None) -> DataFrameType:
    ...
    if is_cudf_type(hostname_split_df):
        hostname_split_df = df_pkg.DataFrame(
            hostname_split_df[0].to_arrow().to_pylist()
        )
    else:
