...
def _mask_pandas(df_cols: tuple[int], mask_: int, series_name: str) -> pd.Series:
    outputs = [
        int(mask_ / 16777216) % 256,
        int(mask_ / 65536) % 256,
        int(mask_ / 256) % 256,
        int(mask_) % 256,
    ]
    return pd.Series(
        [df_cols.idx, ".".join(map(str, outputs))], index=["idx", series_name]
    )
...
def _mask_series_pandas(
    df_cols: tuple[int], mask_series_name: str, output_series_name: str
) -> pd.Series:
