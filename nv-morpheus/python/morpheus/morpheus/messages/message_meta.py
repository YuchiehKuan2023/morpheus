...
class MutableTableCtxMgr:
    ...
    ussage_error = (
        "Error attempting to use mutable_dataframe outside of context manager. Intended usage :\n"
...
class MessageMeta(MessageBase, cpp_class=_messages.MessageMeta):
    ...
    def _get_col_indexers(
        self, df, columns: typing.Union[None, str, typing.List[str]] = None
    ):
        if columns is None:
    ...
    def has_sliceable_index(self) -> bool:
        ...
        return self._df.index.is_unique and (
            self._df.index.is_monotonic_increasing
    ...
    def ensure_sliceable_index(self) -> str:
        ...
        if not self.has_sliceable_index():
            ...
            with self.mutable_dataframe() as df:
                ...
                if not df.index.is_unique or not (
                    df.index.is_monotonic_increasing or df.index.is_monotonic_decreasing
                ):
                    logger.info(
                        "Non unique index found in dataframe, generating new index."
                    )
    ...
    def get_data(self, columns: typing.Union[None, str, typing.List[str]] = None):
        ...
        with self.mutable_dataframe() as df:
            ...
            if -1 in column_indexer:
                missing_columns = [
                    columns[i]
                    for i, index_value in enumerate(column_indexer)
                    if index_value == -1
                ]
                raise KeyError(
                    f"Requested columns {missing_columns} does not exist in the dataframe"
                )
