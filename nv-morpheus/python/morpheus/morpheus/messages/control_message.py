...
class ControlMessage(MessageBase, cpp_class=_messages.ControlMessage):
    def __init__(self, config_or_message: typing.Union["ControlMessage", dict] = None):
        ...
        if isinstance(config_or_message, dict):
            ...
            raise ValueError(
                f"Invalid argument type {type(config_or_message)}, value must be a dict or ControlMessage"
            )
            ...
            if cm_type is not None:
                if isinstance(cm_type, str):
                    try:
                        ...
                        raise ValueError(
                            f"Invalid ControlMessageType: {cm_type}, supported types: {enum_names}"
                        ) from exc
    ...
    def add_task(self, task_type: str, task: dict):
        if isinstance(task_type, str):
            cm_type = get_enum_members(ControlMessageType).get(
                task_type, ControlMessageType.NONE
            )
            if cm_type != ControlMessageType.NONE:
                if self._type == ControlMessageType.NONE:
                    ...
                    raise ValueError(
                        "Cannot mix different types of tasks on the same control message"
                    )
    ...
    def get_metadata(
        self, key: str = None, default_value: typing.Any = None
    ) -> typing.Any:
        """
    ...
    def get_timestamp(
        self, key: str, fail_if_nonexist: bool = False
    ) -> datetime | None:
        try:
            ...
            if fail_if_nonexist:
                raise ValueError(
                    "Timestamp for the specified key does not exist."
                ) from e
            return None
    ...
    def filter_timestamp(self, regex_filter: str) -> dict[str, datetime]:
        ...
        return {
            key: value for key, value in self._timestamps.items() if re_obj.match(key)
        }
    ...
    @classmethod
    def _copy_impl(
        cls, src: "ControlMessage", dst: "ControlMessage" = None
    ) -> "ControlMessage":
