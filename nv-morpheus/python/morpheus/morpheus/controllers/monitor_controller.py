...
class MonitorController:
    ...
    def ensure_progress_bar(self):
        ...
        if self._progress is None:
            self._progress = self._tqdm_class(
    ...
    def auto_count_fn(
        self, msg: SupportedTypes
    ) -> typing.Callable[[SupportedTypes], int] | None:
        """
        ...
        if isinstance(msg, list):
            ...
            return lambda y: reduce(
                lambda sum, z, item_count_fn=item_count_fn: sum + item_count_fn(z), y, 0
            )
    ...
    def sink_on_completed(self):
        ...
        if (
            MonitorController.controller_count <= 0
            and self._tqdm_class.monitor is not None
        ):
