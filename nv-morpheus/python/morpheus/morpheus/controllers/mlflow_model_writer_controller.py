...
def _infer_pandas_column(col: pd.Series):
    try:
        ...
        # compatibility.
        if (
            "Failed to infer schema for pandas.Series" in ex.message
            and pd.api.types.is_string_dtype(col.dtype)
        ):
...
class MLFlowModelWriterController:
    ...
    def _apply_model_permissions(self, reg_model_name: str):
        ...
        if databricks_host is None or databricks_token is None:
            raise RuntimeError(
                "Cannot set Databricks model permissions. "
        ...
        try:
            ...
            get_registered_model_url = urllib.parse.urljoin(
                url_base, "/api/2.0/mlflow/databricks/registered-models/get"
            )

            get_registered_model_response = requests.get(
                url=get_registered_model_url,
                headers=headers,
            ...
            reg_model_id = registered_model_response["registered_model_databricks"][
                "id"
            ]
            ...
            patch_registered_model_permissions_url = urllib.parse.urljoin(
                url_base,
                f"/api/2.0/preview/permissions/registered-models/{reg_model_id}",
            )

            patch_registered_model_permissions_body = {
                "access_control_list": [
                    {"group_name": group, "permission_level": permission}
                    for group, permission in self._databricks_permissions.items()
                ]
            }

            requests.patch(
                url=patch_registered_model_permissions_url,
                headers=headers,
            ...
            logger.exception(
                "Error occurred trying to apply model permissions to model: %s",
                reg_model_name,
                exc_info=True,
            )
    ...
    def on_data(self, message: ControlMessage) -> ControlMessage:
        ...
        try:
            ...
            with mlflow.start_run(
                run_name="autoencoder model training run",
                ...
                mlflow.log_params(
                    {
                        ...
                        "Epochs": model.learning_rate_decay.state_dict().get(
                            "last_epoch", "unknown"
                        ),
                        "Learning rate": model.learning_rate,
                        "Batch size": model.batch_size,
                        "Start Epoch": message.payload()
                        .get_data(self._timestamp_column_name)
                        .min(),
                        "End Epoch": message.payload()
                        .get_data(self._timestamp_column_name)
                        .max(),
                ...
                for key, value in model.categorical_fts.items():
                    ...
                    metrics_dict[f"embedding-{key}-num_embeddings"] = (
                        embedding.num_embeddings
                    )
                    metrics_dict[f"embedding-{key}-embedding_dim"] = (
                ...
                input_schema = Schema(
                    [
                        ColSpec(
                            type=_infer_pandas_column(input_df[col_name]), name=col_name
                        )
                ...
                try:
                    create_model_response = client.create_registered_model(
                        reg_model_name
                    )
                    logger.debug(
                        "Successfully registered model '%s'.",
                ...
                if (
                    self._databricks_permissions is not None
                ...
                model_src = RunsArtifactRepository.get_underlying_uri(
                ...
                mv_obj = client.create_model_version(
                    name=reg_model_name,
                    source=model_src,
                ...
                logger.debug(
                    "ML Flow model upload complete: %s:%s:%s",
                    user,
                    reg_model_name,
                    mv_obj.version,
                )
