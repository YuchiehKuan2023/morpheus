...
IMPORT_ERROR_MESSAGE = (
    "ElasticsearchController requires the elasticsearch package to be installed."
)
...
class ElasticsearchController:
    ...
    def __init__(
        ...
        logger.debug(
            "Creating Elasticsearch client with configuration: %s", connection_kwargs
        )
    ...
    def refresh_client(self, force: bool = False) -> bool:
        ...
        if (
            ...
            or time_now - self._last_refresh_time >= self._refresh_period_secs
        ):
            if self._client:
                try:
    ...
    def parallel_bulk_write(self, actions) -> None:
        ...
        for success, info in parallel_bulk(
            self._client, actions=actions, raise_on_exception=self._raise_on_exception
        ):
            if not success:
