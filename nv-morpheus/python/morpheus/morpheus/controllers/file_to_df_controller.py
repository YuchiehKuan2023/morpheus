...
def single_object_to_dataframe(
...
class FileToDFController:
    ...
    def __init__(
        ...
        cache_dir: str,
        timestamp_column_name: str,
        download_method: typing.Union[
            DownloadMethods, str
        ] = DownloadMethods.DASK_THREAD,
    ):
        self._schema = schema
    ...
    def _get_or_create_dataframe_from_batch(
        self, file_object_batch: typing.Tuple[fsspec.core.OpenFiles, int]
    ) -> typing.Tuple[pd.DataFrame, bool]:
        ...
        hash_data = [
            {"ukey": file_system.ukey(file_object.path)} for file_object in file_list
        ]

        # Convert to base 64 encoding to remove - values
        objects_hash_hex = hashlib.md5(
            json.dumps(hash_data, sort_keys=True).encode()
        ).hexdigest()

        batch_cache_location = os.path.join(
            self._cache_dir, "batches", f"{objects_hash_hex}.pkl"
        )
        ...
        try:
            ...
            logger.warning(
                "Failed to save batch cache. Skipping cache for this batch.",
                exc_info=True,
            )
    ...
    def convert_to_dataframe(
        ...
        try:
            output_df, cache_hit = self._get_or_create_dataframe_from_batch(
                file_object_batch
            )
            ...
            if output_df is not None and logger.isEnabledFor(logging.DEBUG):
                logger.debug(
                    "S3 objects to DF complete. Rows: %s, Cache: %s, Duration: %s ms, Rate: %s rows/s",
                    len(output_df),
