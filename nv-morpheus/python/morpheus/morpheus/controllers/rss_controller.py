...
IMPORT_ERROR_MESSAGE = (
    "RSSController requires the bs4 and feedparser packages to be installed"
)
...
class RSSController:
    ...
    def __init__(
        ...
        self._previous_entries = (
            set()
        )  # Stores the IDs of previous entries to prevent the processing of duplicates.
        ...
        if stop_after > 0 and run_indefinitely:
            raise ValueError(
                "Cannot set both `stop_after` and `run_indefinitely` to True."
            )
        ...
        if enable_cache:
            self._session = requests_cache.CachedSession(
                os.path.join(cache_dir, "RSSController.sqlite"), backend="sqlite"
            )
        else:
    ...
    def get_feed_stats(self, feed_url: str) -> FeedStats:
        ...
        if feed_url not in self._feed_stats_dict:
            raise ValueError(
                "The feed URL is not part of the feed url provided to the constructor."
            )
    ...
    def _try_parse_feed_with_beautiful_soup(
        self, feed_input: str
        ...
        if soup.find("item"):
            ...
            if logger.getEffectiveLevel() == logging.DEBUG:
                ...
                err_msg = (
                    f"Unable to find item or entry tags in response from {feed_input}."
                )
            else:
                # If not DEBUG, truncate feed_input to 256 characters
                truncated_input = (
                    (feed_input[:253] + "...") if len(feed_input) > 256 else feed_input
                )
                err_msg = (
    ...
    def _try_parse_feed(self, url: str) -> "feedparser.FeedParserDict":
        ...
        logger.debug(
            "Parsed feed: %s. Cache hit: %s. Fallback: %s", url, cache_hit, fallback
        )
            ...
            if field_value is not None:
                if isinstance(field_value, list):
                    for field_item in field_value:
                        ...
                        field_item["value"] = self._strip_markup_from_field(
                            field_item["value"], mime_type
                        )
    ...
    def parse_feeds(self):
        ...
        for url in self._feed_input:
            ...
            try:
                ...
                logger.warning(
                    "Failed to parse feed: %s Feed stats: %s\n%s.",
                    url,
                    asdict(feed_stats),
                    ex,
                )
            ...
            for feed in self.parse_feeds():
                for entry in feed.entries:
                    ...
                    if entry_id not in self._previous_entries:
                        ...
                        if (
                            self._batch_size > 0
                            and len(entry_accumulator) >= self._batch_size
                        ):
        ...
        while (
            ...
            try:
                ...
                if not self.run_indefinitely:
                    logger.error(
                        "Failed either in the process of fetching or processing entries: %s.",
                        exc,
                    )
                    raise
                logger.error(
            ...
            logger.info(
                "Waiting for %d seconds before fetching again...", self._interval_secs
            )
            sleep_until = datetime.now() + self._interval_td
            while (
                datetime.now() < sleep_until
                and not self._should_stop_fn()
                and subscription.is_subscribed()
            ):
                time.sleep(1)
