...
class WriteToFileController:
    ...
    def __init__(
        ...
        if os.path.exists(self._output_file):
            ...
            os.makedirs(
                os.path.realpath(os.path.dirname(self._output_file)), exist_ok=True
            )
    ...
    def _convert_to_strings(self, df: DataFrameType):
        if self._file_type in (FileTypes.JSON, "JSON"):
            output_strs = serializers.df_to_json(
                df, include_index_col=self._include_index_col
            )
        elif self._file_type in (FileTypes.CSV, "CSV"):
            output_strs = serializers.df_to_csv(
                df,
