...
class SharedProcessPool:
    ...
    def __new__(cls):
        ...
        with cls._lock:
            if cls._instance is None:
                ...
                logger.debug(
                    "SharedProcessPool.__new__: SharedProcessPool has been initialized."
                )
    ...
    def _initialize(self):
        ...
        if cpu_usage is not None:
            ...
            cpu_usage = 0.5
        self._total_max_workers = math.floor(
            max(1, len(os.sched_getaffinity(0)) * cpu_usage)
        )
        self._processes = []
    ...
    def _launch_workers(self):
        for i in range(self.total_max_workers):
            ...
            logger.debug(
                "SharedProcessPool._lanch_workers(): Process %s/%s has been started.",
                i + 1,
    ...
    def _worker(cancellation_token, task_queues, stage_semaphores):
        logger.debug(
            "SharedProcessPool._worker: Worker process %s has been started.",
            os.getpid(),
        )
        ...
        while True:
            if cancellation_token.is_cancelled():
                logger.debug(
                    "SharedProcessPool._worker: Worker process %s has terminated the worker loop.",
                    os.getpid(),
                )
                return
            ...
            for stage_name, task_queue in task_queues.items():
                ...
                if task is None:
                    logger.debug(
                        "SharedProcessPool._worker: Worker process %s has received a None task.",
                        os.getpid(),
                    )
                    semaphore.release()
                    continue
    ...
    def submit_task(
        self, stage_name: str, process_fn: Callable, *args, **kwargs
    ) -> Task:
        """
        ...
        if self._status != PoolStatus.RUNNING:
            raise RuntimeError(
                "Cannot submit a task to a SharedProcessPool that is not running."
            )

        if stage_name not in self._stage_usage:
            raise ValueError(
                f"Stage {stage_name} has not been set in SharedProcessPool."
            )
    ...
    def set_usage(self, stage_name: str, percentage: float):
        ...
        if self._status != PoolStatus.RUNNING:
            raise RuntimeError(
                "Cannot set usage to a SharedProcessPool that is not running."
            )
        ...
        new_total_usage = (
            self._total_usage - self._stage_usage.get(stage_name, 0.0) + percentage
        )
        ...
        self._stage_semaphores[stage_name] = self._manager.Semaphore(
            allowed_processes_num
        )
        ...
        logger.debug(
            "SharedProcessPool.set_usage(): stage_usage: %s", self._stage_usage
        )
        logger.debug(
            "SharedProcessPool.set_usage(): stage semaphores: %s", allowed_processes_num
        )
    ...
    def wait_until_ready(self, timeout: float | None = None):
        ...
        if self._status == PoolStatus.RUNNING:
            logger.debug(
                "SharedProcessPool.wait_until_ready(): SharedProcessPool is already running."
            )
            return
        ...
        if self._status == PoolStatus.INITIALIZING:
            ...
            raise RuntimeError(
                "Cannot wait for a pool that is not initializing or running."
            )

        logger.debug(
            "SharedProcessPool.wait_until_ready(): SharedProcessPool is ready."
        )
    ...
    def reset(self):
        ...
        if self._status != PoolStatus.SHUTDOWN:
            raise RuntimeError(
                "Cannot reset a SharedProcessPool that is not already shut down."
            )
    ...
    def stop(self):
        ...
        if self._status not in (PoolStatus.RUNNING, PoolStatus.INITIALIZING):
            logger.debug(
                "SharedProcessPool.stop(): Cannot stop a SharedProcessPool that is not running."
            )
            return
    ...
    def join(self, timeout: float | None = None):
        ...
        if self._status != PoolStatus.STOPPED:
            if self._status == PoolStatus.SHUTDOWN:
                logger.debug(
                    "SharedProcessPool.join(): process pool is already shut down."
                )
                return
    ...
    def terminate(self):
        ...
        for i, p in enumerate(self._processes):
            p.terminate()
            logger.debug(
                "Process %s/%s has been terminated.", i + 1, self._total_max_workers
            )

        logger.debug(
            "SharedProcessPool.terminate(): SharedProcessPool has been terminated."
        )
        self._status = PoolStatus.SHUTDOWN
