...
class EnvConfigValue(ABC):
    ...
    def __init__(self, value: str | None = None, use_env: bool = True):
        ...
        if use_env:
            ...
            if (
                self.__class__._ENV_KEY_OVERRIDE is not None
                and self.__class__._ENV_KEY_OVERRIDE in os.environ
            ):
            ...
            if not self.__class__._ALLOW_NONE and value is None:
                message = (
                    "value must not be None, but provided value was None and no environment-based default or "
                    "override was found."
                )
