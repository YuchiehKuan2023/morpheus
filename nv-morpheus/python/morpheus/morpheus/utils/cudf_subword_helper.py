...
def get_cached_tokenizer(vocab_hash_file: str, do_lower_case: bool):
    ...
    if hashed_inputs not in cached_tokenizers:
        cached_tokenizers[hashed_inputs] = create_tokenizer(
            vocab_hash_file, do_lower_case
        )

    return cached_tokenizers[hashed_inputs]


def tokenize_text_series(
    vocab_hash_file: str,
    do_lower_case: bool,
    ...
    assert tokenizer is not None, (
        "Must create tokenizer first using `create_tokenizer()`"
    )
    ...
    output_f = Feature(
        input_ids=input_ids, input_mask=attention_mask, segment_ids=metadata
    )
