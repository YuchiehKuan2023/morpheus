...
def create_increment_col(
    ...
    cat_col: pd.Series = df.groupby([per_day, groupby_column])[column_name].transform(
        lambda x: pd.factorize(x.fillna("nan"))[0] + 1
    )

    increment_col = (
        pd.concat([cat_col, df[groupby_column]], axis=1)
        .groupby([per_day, groupby_column])[column_name]
        .expanding(1)
        .max()
        .droplevel(0)
        .droplevel(0)
...
class ColumnInfo:
    ...
    def convert_pandas_dtype(dtype: str | type) -> str:
        ...
        if (isinstance(dtype, str) and dtype.startswith("datetime")) or (
            isinstance(dtype, type) and issubclass(dtype, datetime)
        ):
...
class BoolColumn(RenameColumn):
    ...
    value_map: typing.Dict[str, bool] = dataclasses.field(
    ...
    def __post_init__(
        ...
        assert self.dtype == ColumnInfo.convert_pandas_dtype(bool), (
            "BoolColumn must have dtype 'bool'"
        )
...
class StringCatColumn(ColumnInfo):
    ...
    def _process_column(self, df: pd.DataFrame) -> pd.Series:
        ...
        return first_col.str.cat(
            others=df[self.input_columns[1:]].astype(str), sep=self.sep
        )
...
class DistinctIncrementColumn(RenameColumn):
    ...
    def _process_column(self, df: pd.DataFrame) -> pd.Series:
        ...
        cat_col: pd.Series = df.groupby([per_period, self.groupby_column])[
            self.input_name
        ].transform(lambda x: pd.factorize(x.fillna("nan"))[0] + 1)
...
def _json_flatten(
    ...
    if preserve_re:
        columns_to_preserve.update(
            col for col in df_input.columns if re.match(preserve_re, col)
        )

    # Early exit
    if json_cols is None or len(json_cols) == 0:
        return PreparedDFInfo(
            df=df_input, columns_to_preserve=list(columns_to_preserve)
        )
...
def _resolve_json_output_columns(
    json_cols: list[str], input_cols: dict[str, str]
) -> list[tuple[str, str]]:
    """
...
class DataFrameInputSchema:
    ...
    json_output_columns: typing.List[tuple[str, str]] = dataclasses.field(
        init=False, repr=False
    )
    input_columns: typing.Dict[str, str] = dataclasses.field(init=False, repr=False)
    output_columns: typing.List[tuple[str, str]] = dataclasses.field(
        init=False, repr=False
    )

    prep_dataframe: typing.Callable[[pd.DataFrame], typing.List[str]] = (
    ...
    def __post_init__(self):
        ...
        for col_info in self.column_info:
            ...
            overlapping_keys = set(col_input_types.keys()).intersection(
                input_columns_dict.keys()
            )

            for key in overlapping_keys:
                if col_input_types[key] != input_columns_dict[key]:
                    raise ValueError(
                        f"Column input '{key}' is defined with conflicting types: "
        ...
        self.json_output_columns = _resolve_json_output_columns(
            self.json_columns, self.input_columns
        )

        self.prep_dataframe = partial(
            _json_flatten,
