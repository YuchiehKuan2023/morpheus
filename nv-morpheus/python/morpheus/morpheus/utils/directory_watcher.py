...
class DirectoryWatcher:
    ...
    def __init__(
        ...
        if len(glob_split) == 1:
            raise RuntimeError(
                (
                    "When watching directories, input_glob must have a wildcard. "
    ...
    def _get_filename_queue(self) -> FiberQueue:
        ...
        if self._watch_directory:
            ...
            self._watcher.schedule(
                event_handler, self._dir_to_watch, recursive=self._recursive
            )
    ...
    def _generate_via_polling(self, subscription: mrc.Subscription):
        ...
        while not self._should_stop_fn() and subscription.is_subscribed():
            ...
            new_snapshot = DirectorySnapshot(
                self._dir_to_watch, recursive=self._recursive
            )
            ...
            files_to_process = filter_paths(
                diff_events.files_created, included_patterns=[self._input_glob]
            )
