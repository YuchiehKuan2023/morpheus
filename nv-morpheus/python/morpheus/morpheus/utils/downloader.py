...
class Downloader:
    ...
    def get_dask_cluster(self):
        ...
        with Downloader._mutex:
            if Downloader._dask_cluster is None:
                ...
                Downloader._dask_cluster = dask.distributed.LocalCluster(
                    start=True, processes=self.download_method != "dask_thread"
                )

                logger.debug(
                    "Creating dask cluster... Done. Dashboard: %s",
                    Downloader._dask_cluster.dashboard_link,
                )
    ...
    def download(
        ...
        download_fn: typing.Callable[[fsspec.core.OpenFile], pd.DataFrame],
