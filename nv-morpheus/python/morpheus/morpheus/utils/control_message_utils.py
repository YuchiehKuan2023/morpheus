...
def cm_skip_processing_if_failed(
    func: Callable[CM_SKIP_P, T],
) -> Callable[CM_SKIP_P, T]:
    """
    ...
    def wrapper(
        ...
        *args: CM_SKIP_P.args,
        **kwargs: CM_SKIP_P.kwargs,
    ) -> T:
        if control_message.has_metadata("cm_failed") and control_message.get_metadata(
            "cm_failed"
        ):
...
def cm_default_failure_context_manager(
    ...
    def decorator(func):
        ...
        def wrapper(control_message: ControlMessage, *args, **kwargs):
            ...
            with CMDefaultFailureContextManager(
                control_message=control_message, raise_on_failure=raise_on_failure
            ) as ctx_mgr:
