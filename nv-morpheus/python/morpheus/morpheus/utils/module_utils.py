...
mrc_version = [
    int(j.group(1))
    for i in mrc.__version__.split(".")
    if (j := re.compile(r"(\d+)").match(i)) is not None
]
...
def verify_module_registration(func):
    ...
    def inner_func(config, **kwargs):
        ...
        if module_id is None or namespace is None:
            raise TypeError(
                "TypeError: a string-like object is required for module_id and namespace, not 'NoneType'"
            )

        if not Registry.contains(module_id, namespace):
            raise ValueError(
                f"Module '{module_id}' doesn't exist in the namespace '{namespace}'"
            )
...
def register_module(module_id, namespace):
    ...
    def inner_func(func):
        ...
        if not Registry.contains(module_id, namespace):
            ...
            logger.debug(
                "Module '%s' was successfully registered with '%s' namespace.",
                module_id,
                namespace,
            )
        else:
            logger.debug(
                "Module: '%s' already exists in the given namespace '%s'",
                module_id,
                namespace,
            )
...
def load_module(config: typing.Dict, builder: mrc.Builder):
    ...
    logger.debug(
        "Module '%s' with namespace '%s' is successfully loaded.", module_id, namespace
    )
...
def verify_module_meta_fields(config: typing.Dict):
    ...
    if "module_id" not in config:
        raise KeyError(
            "Required attribute 'module_id' is missing in the module configuration."
        )
    if "namespace" not in config:
        raise KeyError(
            "Required attribute 'namespace' is missing in the module configuration."
        )
    if "module_name" not in config:
        raise KeyError(
            "Required attribute 'module_name' is missing in the module configuration."
        )
...
def to_period_approximation(data_df: DataFrameType, period: str) -> DataFrameType:
    ...
    if period not in period_to_strptime:
        raise ValueError(
            f"Unknown period: {period}. Supported period: {period_to_strptime}"
        )

    strptime_format = period_to_strptime[period]

    df_pkg = get_df_pkg_from_obj(data_df)
    data_df["period"] = df_pkg.to_datetime(
        data_df["ts"].dt.strftime(strptime_format) + "-1",
...
def get_config_with_overrides(
    config, module_id, module_name=None, module_namespace="morpheus"
):
    ...
    try:
        ...
        raise KeyError(
            f"'module_name' is not set in the '{module_id}' module configuration."
        ) from exc_info
...
def make_nested_module(
    ...
    def module_init(builder: mrc.Builder):
        ...
        for config in ordered_modules_meta:
            ...
            if prev_module:
                builder.make_edge(
                    prev_module.output_port("output"), curr_module.input_port("input")
                )
            else:
...
class ModuleLoader:
    ...
    def load(self, builder: mrc.Builder):
        ...
        logger.debug(
            "Module '%s' with namespace '%s' is successfully loaded.",
            self._module_interface.identity,
            self._module_interface.namespace,
        )
