...
def filter_df(
    ...
    if replace_idx is not None and replace_idx in filtered_df:
        ...
        if replace_idx.startswith("_index_"):
            filtered_df.index.name = str(filtered_df.index.name).replace(
                "_index_", "", 1
            )
...
def compare_df(
    ...
    """
    df_a_filtered = filter_df(
        df_a, include_columns, exclude_columns, replace_idx=replace_idx
    )
    df_b_filtered = filter_df(
        df_b, include_columns, exclude_columns, replace_idx=replace_idx
    )
    ...
    if comparison.matches():
        ...
        mismatched_idx = match_columns[
            match_columns.apply(lambda r: not r.all(), axis=1)
        ].index

        merged = (
            pd.concat([df_a_filtered, df_b_filtered], keys=[dfa_name, dfb_name])
            .swaplevel()
            .sort_index()
        )
        ...
        if diff_rows > 0 or diff_cols > 0:
            logger.debug(
                "Results do not match. Diff %d/%d (%f %%). First 10 mismatched rows:",
                diff_rows,
                total_rows,
