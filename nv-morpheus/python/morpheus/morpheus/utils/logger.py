    ...
    *extra_handlers: List of additional handlers which will handle entries placed on the queue
    ...
    if not getattr(morpheus_logger, "_configured_by_morpheus", False):
        ...
        log_file = os.path.join(
            appdirs.user_log_dir(appauthor="NVIDIA", appname="morpheus"), "morpheus.log"
        )
        ...
        file_handler = logging.handlers.RotatingFileHandler(
            filename=log_file, backupCount=5, maxBytes=1000000
        )
        file_handler.setLevel(logging.DEBUG)
        file_handler.setFormatter(
            logging.Formatter(
                "%(asctime)s - [%(levelname)s]: %(message)s {%(name)s, %(threadName)s}"
            )
        )
        ...
        raise RuntimeError(
            "Logging has already been configured. Use `set_log_level` to change the log level or reset "
...
def configure_logging(
    *extra_handlers: logging.Handler, log_level: int = None, log_config_file: str = None
):
    """
...
def deprecated_message_warning(cls, new_cls):
    ...
    message = (
        f"The '{cls.__name__}' message has been deprecated and will be removed "
