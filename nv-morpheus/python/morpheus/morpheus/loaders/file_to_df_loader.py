...
def file_to_df_loader(control_message: ControlMessage, task: dict):
    ...
    if task.get("strategy", "aggregate") != "aggregate":
        raise RuntimeError(
            "Only 'aggregate' strategy is supported for file_to_df loader."
        )
    ...
    try:
        ...
        raise ValueError(
            f"Invalid input file type '{file_type}'. Available file types are: CSV, JSON."
        ) from exec_info

    try:
        controller = FileToDFController(
            schema=schema,
        ...
        pdf = controller.convert_to_dataframe(
            file_object_batch=(fsspec.open_files(files), n_groups)
        )
