...
def _aggregate_df(
    df_aggregate: typing.Optional[cudf.DataFrame], df: pd.DataFrame
) -> cudf.DataFrame:
    """
...
def sql_loader(
    control_message: ControlMessage, task: typing.Dict[str, typing.Any]
) -> ControlMessage:
    """
    ...
    with CMDefaultFailureContextManager(control_message):
        ...
        try:
            for query_data in queries:
                ...
                read_sql = functools.partial(
                    _read_sql, engine_obj=engine_registry[conn_str]
                )
                execution_chain = ExecutionChain(
                    function_chain=[_parse_query_data, read_sql, aggregate_df]
                )
