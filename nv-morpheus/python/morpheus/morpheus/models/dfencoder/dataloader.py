...
class DFEncoderDataLoader(DataLoader):
    ...
    def get_distributed_training_dataloader_from_path(
        ...
        dataloader = (
            DFEncoderDataLoader.get_distributed_training_dataloader_from_dataset(
                dataset=dataset,
                rank=rank,
    ...
    def __init__(
        ...
        if preload_data_into_memory:
            self._preloaded_data = {
                fn: self._load_data_fn(f"{self._data_folder}/{fn}")
                for fn in self._filenames
            }

        self._file_sizes = {
            fn: self._get_file_len(fn) - 1
            if not self._preloaded_data
            else len(self._preloaded_data[fn])
            for fn in self._filenames
        }
