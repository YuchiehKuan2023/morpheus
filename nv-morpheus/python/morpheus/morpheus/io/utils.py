...
def cudf_string_cols_exceed_max_bytes(
    df: "cudf.DataFrame", column_max_bytes: dict[str, int]
) -> bool:
    """
...
def truncate_string_cols_by_bytes(
    df: DataFrameType, column_max_bytes: dict[str, int], warn_on_truncate: bool = True
) -> bool:
    """
    ...
    for col, max_bytes in column_max_bytes.items():
        ...
        if encoded_series.str.len().max() > max_bytes:
            ...
            decoded_series = truncated_series.str.decode(
...
def _get_df_method(
    selector: DataFrameModule | ExecutionMode, method_name: str
) -> typing.Callable[..., DataFrameType]:
    """
...
def get_csv_reader(
    selector: DataFrameModule | ExecutionMode,
) -> typing.Callable[..., DataFrameType]:
    """
