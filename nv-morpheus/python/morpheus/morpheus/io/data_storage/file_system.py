...
class FileSystemStorage(RecordStorageInterface):
    ...
    def __init__(self, file_path: str, file_format: str):
        ...
        if self._file_format == "csv":
            ...
            self._data_writer = lambda df, path: df.to_csv(
            ...
            raise NotImplementedError(
                f"File format {self._file_format} is not supported."
            )
    ...
    def delete(self) -> None:
        ...
        if (
            self._owner
            and self._backing_source is not None
            and self._fs.exists(self._backing_source)
        ):
    ...
    def store(
        ...
        if isinstance(data_source, (cudf.DataFrame, pd.DataFrame)):
            ...
            if copy_from_source:
                ...
                self._num_rows = row_count_from_file(
                    self._backing_source, self._file_format
                )
                self._owner = False
