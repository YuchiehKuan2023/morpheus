...
def df_to_stream_csv(
    df: DataFrameType, stream: IOBase, include_header=False, include_index_col=True
):
    """
...
def df_to_stream_json(
    df: DataFrameType, stream: IOBase, include_index_col=True, lines=True
):
    """
...
def df_to_stream_parquet(
    df: DataFrameType, stream: IOBase, include_index_col: bool = True
) -> IOBase:
    """
...
def df_to_json(
    df: DataFrameType, strip_newlines=False, include_index_col=True
) -> typing.List[str]:
    """
...
def df_to_parquet(
    df: DataFrameType, strip_newlines: bool = False, include_index_col: bool = True
) -> typing.List[bytes]:
    """
...
def write_df_to_file(
    df: DataFrameType, file_name: str, file_type: FileTypes = FileTypes.Auto, **kwargs
):
    """
    ...
    if CppConfig.get_should_use_cpp():
        if is_cudf_type(df):
            ...
            write_df_to_file_cpp(
                df=df, filename=file_name, file_type=file_type, **kwargs
            )
            return
