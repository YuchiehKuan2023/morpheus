...
class DataManager:
    ...
    def __repr__(self):
        return (
            f"DataManager(records={self.num_rows}, "
    ...
    def __str__(self):
        return (
            f"DataManager with {self.num_rows} records, "
    ...
    def manifest(self) -> dict:
        ...
        if self._dirty:
            self._manifest = {
                source_id: data_record.backing_source
                for source_id, data_record in self._records.items()
            }
    ...
    def store(
        ...
        if self._storage_type == "filesystem":
            data_label = os.path.join(
                self._storage_dir, f"{tracking_id}.{self._file_format}"
            )
        else:
