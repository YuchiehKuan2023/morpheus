...
def get_reader(
    file_type: FileTypes, df_type: DataFrameModule
...
def _read_file_to_df_py(
    *,
    ...
    if mode == FileTypes.Auto:
        ...
        if isinstance(file_name, io.IOBase):
            if hasattr(file_name, "path"):  # This attr is not in the base
                ...
                raise ValueError(
                    "Unable to determine file type from instance of io.IOBase,"
    ...
    if mode == FileTypes.CSV:
        if (
            len(df.columns) > 1
            and df.columns[0] == "Unnamed: 0"
            and df.iloc[:, 0].dtype == np.dtype(int)
        ):
...
def read_file_to_df(
    ...
    df_type: DataFrameModule = "pandas",
) -> DataFrameType:
    """
