...
class StageInfo:
    ...
    qualified_name: (
        str  # The fully qualified name of the stage. Only used for comparison
    )
    build_command: typing.Callable[[], click.Command] = dataclasses.field(
...
class LazyStageInfo(StageInfo):
    ...
    def __init__(
        self, name: str, stage_qualified_name: str, modes: typing.List[PipelineModes]
    ):
        super().__init__(
            name=name,
            modes=modes,
    ...
    def _lazy_build(self):
        ...
        if stage_class is None:
            raise RuntimeError(
                f"Could not import {self.class_name} from {self.package_name}"
            )

        # Now get the stage info from the class (it must have been registered during the import)
        stage_class_info: StageInfo = getattr(
            stage_class, "_morpheus_registered_stage", None
        )

        if stage_class_info is None:
            raise RuntimeError(
                f"Class {self.qualified_name} did not have attribute '_morpheus_registered_stage'. \
...
class StageRegistry:
    def __init__(self) -> None:
        ...
        self._registered_stages: typing.Dict[
            PipelineModes, typing.Dict[str, StageInfo]
        ] = {}
    ...
    def _add_stage_info(self, mode: PipelineModes, stage: StageInfo):
        ...
        if stage.name in mode_stages:
            ...
            logging.debug(
                "The stage %s has already been added for mode: %s", stage.name, mode
            )
    ...
    def get_stage_info(
        self, stage_name: str, mode: PipelineModes = None, raise_missing=False
    ) -> StageInfo:
        ...
        if raise_missing:
            raise RuntimeError(
                "Found stage '{stage_name}' in registry, but it does not support pipeline mode: {mode}"
            )
    ...
    def get_registered_names(self, mode: PipelineModes = None) -> typing.List[str]:
        ...
        stage_names: typing.List[str] = [
            name
            for name, stage_info in self._get_stages_for_mode(mode).items()
            if stage_info.supports_mode(mode)
        ]
