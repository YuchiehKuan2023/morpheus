...
def get_pipeline_from_ctx(ctx: click.Context) -> "LinearPipeline":
    ...
    assert "pipeline" in ctx_dict, (
        "Inconsistent configuration. Pipeline accessed before created"
    )
...
def parse_log_level(ctx, param, value):
    ...
    if x is None:
        raise click.BadParameter(
            f"Must be one of {', '.join(logging._nameToLevel.keys())}. Passed: {value}"
        )
    return x
...
class MorpheusRelativePath(click.Path):
    ...
    def convert(
        ...
        if package_relative != value:
            logger.debug(
                (
                    "Parameter, '%s', with relative path, '%s', does not exist. "
