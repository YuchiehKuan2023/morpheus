...
def partial_pop_kwargs(function, input_dict: dict):
    ...
    bound_args = {
        ...
        for input_name in list(input_dict.keys())
        if input_name in fn_sig.parameters
    }
...
def set_options_param_type(options_kwargs: dict, annotation, doc_type: str):
    ...
    if issubtype(annotation, pathlib.Path):
        ...
        options_kwargs["type"] = partial_pop_kwargs(click.Choice, doc_type_kwargs)(
            get_enum_keys(annotation)
        )

        _convert_enum_default(options_kwargs, annotation)
        options_kwargs["callback"] = functools.partial(
            parse_enum, enum_class=annotation, case_sensitive=case_sensitive
        )
        ...
        if has_matching_kwargs(click.IntRange, doc_type_kwargs):
            options_kwargs["type"] = partial_pop_kwargs(
                click.IntRange, doc_type_kwargs
            )()
        else:
        ...
        if has_matching_kwargs(click.FloatRange, doc_type_kwargs):
            options_kwargs["type"] = partial_pop_kwargs(
                click.FloatRange, doc_type_kwargs
            )()
        else:
            options_kwargs["type"] = annotation
        ...
        issubtype(annotation, typing.List)
        or issubtype(annotation, typing.Tuple)
        or issubtype(annotation, typing.Iterable)
    ) and not (issubtype(annotation, str)):
...
def compute_option_name(
    stage_arg_name: str, rename_options: typing.Dict[str, str] = None
) -> tuple:
...
def register_stage(
    ...
    def register_stage_inner(stage_class: _DecoratorType) -> _DecoratorType:
        ...
        if (
            not hasattr(stage_class, "_morpheus_registered_stage")
            ...
            def build_command():
                ...
                if (
                    hasattr(stage_class, "__init__")
                    and stage_class.__init__.__doc__ is not None
                ):
                ...
                for p_name, p_value in class_init_sig.parameters.items():
                    try:
                        ...
                        set_options_param_type(
                            option_kwargs,
                            p_value.annotation,
                            get_param_type(numpy_doc, p_name),
                        )
                        ...
                        raise RuntimeError(
                            (
                                f"Error auto registering CLI command '{command_name}' with "
                                f"class '{stage_class}' and parameter '{p_name}'. Error: {ex}"
                            )
                        ) from ex

                if config_param_name is None:
                    raise RuntimeError(
                        "All stages must take on argument of morpheus.Config. Ensure your stage "
                ...
                command_kwargs["help"] = "\n".join(
                    numpy_doc["Extended Summary"] or [command_kwargs["short_help"]]
                )
            ...
            for mode in modes:
                registered_stage = GlobalStageRegistry.get().get_stage_info(
                    command_name, mode
                )
                ...
                if registered_stage is not None:
                    ...
                    if isinstance(registered_stage, LazyStageInfo):
                        ...
                        if registered_stage.qualified_name != get_full_qualname(
                            stage_class
                        ):
                            raise RuntimeError(
            ...
            if len(existing_registrations) > 0:
                ...
                if len(diff_modes) > 0:
                    raise RuntimeError(
                        "Mismatch between LazyStageInfo modes and register_stage() modes"
                    )
            else:
