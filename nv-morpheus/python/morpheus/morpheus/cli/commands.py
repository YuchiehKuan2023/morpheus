...
def add_command(
    name: str, stage_module: str, modes: typing.Tuple[PipelineModes, ...] = None
):
...
class PluginGroup(AliasedGroup):
    ...
    def __init__(
        ...
        commands: typing.Optional[
            typing.Union[
                typing.Dict[str, click.Command], typing.Sequence[click.Command]
            ]
        ] = None,
        **attrs: typing.Any,
    ):
        ...
        assert self._pipeline_mode is not None, (
            "Must specify `pipeline_mode` when using `PluginGroup`"
        )
    ...
    def list_commands(self, ctx: click.Context) -> typing.List[str]:
        ...
        plugin_command_list = registered_stages.get_registered_names(
            self._pipeline_mode
        )
        ...
        if len(duplicate_commands) > 0 and "COMP_WORDS" not in os.environ:
            raise RuntimeError(
                f"Plugins registered the following duplicate commands: {', '.join(duplicate_commands)}"
            )
...
@click.option(
    ...
    help=(
        "Config file to use to configure logging. Use only for advanced situations. "
...
@click.option(
    "--input_model", type=click.Path(exists=True, readable=True), required=True
)
@click.option(
    "--output_model", type=click.Path(exists=False, writable=True), required=True
)
...
@tools.group(
    short_help="Utility for installing/updating/removing shell completion for Morpheus",
    no_args_is_help=True,
)
...
def show(shell):
    ...
    click.secho(
        f"To add {shell} completion, write the following code to '{path}':\n", fg="blue"
    )
    click.echo(code)
...
@click.option(
    "--append/--overwrite", help="Append the completion code to the file", default=False
)
@click.option(
    "--shell",
    required=False,
...
@cli.group(
    short_help="Run one of the available pipelines",
    no_args_is_help=True,
    cls=AliasedGroup,
)
@click.option(
...
@click.option(
    ...
    help=(
        "The size of buffered channels to use between nodes in a pipeline. Larger values reduce "
...
@click.option(
    ...
    type=bool,
    help=(
        "[Deprecated] Whether or not to use C++ node and message types or to prefer python. "
...
@click.option(
    ...
    is_flag=True,
    help=(
        "Whether or not to run in CPU only mode, setting this to True will disable C++ mode. "
...
@click.option(
    ...
    help=(
        "Manually seed the random number generators used by Morpheus, useful for testing."
    ),
)
@prepare_command(parse_config=True)
def run(ctx: click.Context, **kwargs):
    ...
    if (
        ctx.get_parameter_source("use_cpu_only")
        is not click.core.ParameterSource.DEFAULT
        and ctx.get_parameter_source("use_cpp")
        is not click.core.ParameterSource.DEFAULT
    ):
        # If the user set explicit values for both use_cpu_only and use_cpp raise an error
        raise click.UsageError(
            "Cannot set both --use_cpp and --use_cpu_only. The --use_cpp flag is deprecated. "
    ...
    if ctx.get_parameter_source("use_cpp") is not click.core.ParameterSource.DEFAULT:
        logger.warning(
            "The --use_cpp flag is deprecated and will be removed in a future release"
        )
...
@click.option(
    ...
    type=click.IntRange(min=1),
    help=(
        "Limits the length of the sequence returned. If tokenized string is shorter than max_length, "
...
@click.option(
    "--label", type=str, default=None, multiple=True, help=("Specify output labels.")
)
@click.option(
    "--labels_file",
    default="data/labels_nlp.txt",
    type=MorpheusRelativePath(
        dir_okay=False, exists=True, file_okay=True, resolve_path=True
    ),
    help=(
        "Specifies a file to read labels from in order to convert class IDs into labels. "
...
@click.option(
    ...
    help=(
        "Set the direction for the Graphviz pipeline diagram, "
...
def pipeline_nlp(ctx: click.Context, **kwargs):
    ...
    if len(labels):
        ...
        logger.debug(
            "Loaded labels file. Current labels: [%s]", str(config.class_labels)
        )
...
def pipeline_fil(ctx: click.Context, **kwargs):
    ...
    if "columns_file" in kwargs and kwargs["columns_file"] is not None:
        ...
        logger.debug(
            "Loaded columns. Current columns: [%s]", str(config.fil.feature_columns)
        )
...
def post_pipeline(ctx: click.Context, *args, **kwargs):
    ...
    if "viz_file" in kwargs and kwargs["viz_file"] is not None:
        ...
        click.secho(
            f"Pipeline visualization saved to {kwargs['viz_file']}", fg="yellow"
        )
...
add_command(
    "deserialize",
    "morpheus.stages.preprocess.deserialize_stage.DeserializeStage",
    modes=ALL,
)
add_command(
    "dropna", "morpheus.stages.preprocess.drop_null_stage.DropNullStage", modes=ALL
)
add_command(
    "filter",
...
add_command(
    "from-databricks-deltalake",
    "morpheus.stages.input.databricks_deltalake_source_stage.DataBricksDeltaLakeSourceStage",
    modes=ALL,
)
add_command(
    "from-file", "morpheus.stages.input.file_source_stage.FileSourceStage", modes=ALL
)
add_command(
    "from-kafka", "morpheus.stages.input.kafka_source_stage.KafkaSourceStage", modes=ALL
)
add_command(
    "from-http",
...
add_command(
    "from-rss", "morpheus.stages.input.rss_source_stage.RSSSourceStage", modes=ALL
)
add_command(
    "gen-viz",
...
add_command(
    "mlflow-drift",
    "morpheus.stages.postprocess.ml_flow_drift_stage.MLFlowDriftStage",
    modes=ALL,
)
...
add_command(
    "serialize", "morpheus.stages.postprocess.serialize_stage.SerializeStage", modes=ALL
)
add_command(
    "to-elasticsearch",
...
add_command(
    "to-file", "morpheus.stages.output.write_to_file_stage.WriteToFileStage", modes=ALL
)
add_command(
    "to-kafka",
