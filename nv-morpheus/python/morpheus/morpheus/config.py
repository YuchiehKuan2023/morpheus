...
def auto_determine_bootstrap():
    ...
    bootstrap_servers = ",".join(
        [
            c.ports["9092/tcp"][0]["HostIp"] + ":" + c.ports["9092/tcp"][0]["HostPort"]
            for c in kafka_net.containers
...
class ConfigOnnxToTRT(ConfigBase):
    ...
    batches: typing.List[typing.Tuple[int, int]] = dataclasses.field(
...
class CppConfig:
    ...
    __allow_cpp: bool = not os.getenv("MORPHEUS_NO_CPP", "False").lower() in (
        "true",
        "1",
        "t",
    )
...
class Config(ConfigBase):
    ...
    def _validate_config(self):
        if self._pipeline_batch_size < self._model_max_batch_size:
            logging.warning(
                "Config has `pipeline_batch_size < model_max_batch_size` which effectively limits "
