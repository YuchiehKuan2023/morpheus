...
class MorpheusLibImportChecker(BaseChecker):
    ...
    msgs = {
        "W9901": (
            "Incorrect import of %(name)s as %(alias)s. Imports from morpheus._lib should be in the form of: "
    ...
    def visit_importfrom(self, node) -> None:
        ...
        for name, alias in node.names:
            for lib_module in self._LIB_MODULES:
                if (
                    node.modname.startswith(lib_module)
                    and os.path.basename(node.root().file) != "__init__.py"
                ):
                    self.add_message(
                        "morpheus-incorrect-lib-from-import",
                        node=node,
