...
def _run_cmd(exe: str, *args: str):
    ...
    try:
        ...
        logging.warning(
            "Running %s command [ERRORED]: `%s`, Output: '%s'",
            exe,
            cmd_str,
...
class GitWrapper:
    ...
    def get_repo_version():
        ...
        if match is None:
            logging.debug(
                "Could not determine repo major minor version. Full repo version: %s.",
                full_repo_version,
            )
            return None
    ...
    def get_repo_owner_name():
        ...
        return "nv-morpheus/" + _run_cmd(
            "git remote -v | grep -oP '/\K\w*(?=\.git \(fetch\))' | head -1"
        )  # noqa: W605
    ...
    def get_remote_branch(local_branch_ref: str, *, repo_owner_and_name: str = None):
        ...
        logging.info(
            "Remote branch '%s' for repo '%s' does not exist. Falling back to rev-parse",
            remote_branch_ref,
            repo_owner_and_name,
        )

        remote_branch_ref = _git(
            "rev-parse",
            "--abbrev-ref",
            "--symbolic-full-name",
            local_branch_ref + "@{upstream}",
        )
    ...
    def get_target_remote_branch():
        try:
            ...
            if base_ref == GitWrapper.get_current_branch():
                logging.warning(
                    "Current branch is the same as the tagged branch: %s. Falling back to 'main'",
                    base_ref,
                )
                base_ref = "main"
    ...
    def diff(
        ...
        assert target_ref is not None or target_ref != "", (
...
def determine_merge_commit(current_branch="HEAD"):
    ...
    logging.info(
        "Determined TARGET_BRANCH as: '%s'. With merge-commit: %s",
        remote_branch,
        common_commit,
    )
...
def filter_files(
    ...
    for file in files:
        ...
        assert _is_repo_relative(file, git_root=git_root), (
...
def changed_files(
    ...
    *,
    ...
    logging.info(
        "Comparing %s..%s with merge_base: %s, staged: %s",
        target_ref,
        base_ref,
        merge_base,
        staged,
...
def modified_files(
    ...
    *,
    ...
    logging.info(
        "Comparing index to %s with merge_base: %s, staged: %s",
        target_ref,
        merge_base,
        staged,
    )
