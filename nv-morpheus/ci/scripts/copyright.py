...
def check_copyright(
        ...
        if update_start_year:
            try:
                ...
                e = [
                    f,
                    line_num,
                    f"Error determining start year from git: {excp}",
                    None,
                ]
                errs.append(e)
                continue
    ...
    if update_current_year or update_start_year or do_insert_license:
        ...
        if len(errs_update) > 0:
            logging.info(
                "File: %s. Changing line(s) %s",
                f,
                ", ".join(str(x[1]) for x in errs if x[-1] is not None),
            )
...
def _main():
    ...
    argparser.add_argument(
        ...
        action="store_true",
        required=False,
        help="If set, any files auto-fixed will have `git add` run on them. ",
    )
    ...
    if len(errors) > 0:
        ...
        if n_fixable > 0:
            logging.info(
                (
                    "You can run `python %s --git-modified-only "
