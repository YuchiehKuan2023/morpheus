...
class ModelManager:
    ...
    def _load_from_mlflow(self, client: MlflowClient, user_id: str, timeout: float) -> ModelCache | None:
        ...
        try:
            ...
            if elapsed > timeout:
                logger.warning(f"Model search timeout exceeded: {elapsed:.2f}s > {timeout}s")
                return None
...
class DFPInference:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        logger.info(
            f"DFPInference initialized: tracking_uri={self.tracking_uri}, fallback_user={self.fallback_username}"
        )
        ...
        try:
            ...
            if len(results_df) > 0:
                ...
                if z_cols and len(results_df) > 0:
                    first_row_results = results_df.iloc[0]
                    last_row_input = inference_data.iloc[-1]
                    z_scores = {col: first_row_results[col] for col in z_cols}
                    ...
                    for feat, z in sorted_z:
                        ...
                        loss_val = first_row_results.get(loss_col, "N/A")
                        actual_val = last_row_input.get(feat_name, "N/A")
                        pred_val = first_row_results.get(pred_col, "N/A")
                    ...
                    reported_mean = first_row_results.get("mean_abs_z", "N/A")
                    debug_msg += "\nZ-SCORE STATISTICS:\n"
